...
这个插件展示研究阶段冻结的四层市场状态，而不是重新在浏览器请求中计算或训练模型：

- 战略状态：1D / 4H，偏多、中性、偏空；
- 战术状态：4H / 1H，向上过程、整理、向下过程；
- 入场状态：30m 至 1m，局部向上、中性、局部向下；
- 活跃度状态：低活跃/压缩、普通活跃、高活跃/扩张。

先完成 R03.3.3.1 研究并生成状态缓存，再预构建 2024、2025 两个完整 OOS 活跃持续概率轨道：

```bash
python tools\prebuild_ai_market_state_timeline.py
```

随后启动 Analyze Tool，在插件列表选择 `AI多周期市场状态 R03.3.3.1`。建议优先使用 1m 或 15m K 线查看 2024、2025：

- 四条状态色带来自完整 15m 因果状态缓存；
- `活跃状态持续3h概率` 是对应年份 Walk-forward 模型的完整 OOS 预测；
- `活跃状态3h转换风险` 等于 `1 - 持续概率`；
- `历史结果审计（未来3h）` 色带使用未来真实结果，只为了肉眼核对预测是否正确，绝不是模型输入或实盘可用状态；
- 2021—2023 可以查看状态，但没有正式 OOS 持续概率；
- 2026 仍然封存，不显示该研究模型结果。

状态和概率都只是后续方向、入场和持仓模型的上下文，不可单独作为开仓、平仓或反向依据。
