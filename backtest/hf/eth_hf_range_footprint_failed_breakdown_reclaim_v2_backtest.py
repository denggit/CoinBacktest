#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Range Footprint Failed Breakdown Reclaim V2.
...
STRATEGY_NAME = "ETH_HF_RangeFootprint_FailedBreakdownReclaim_V2"
FILE_STEM = "eth_hf_range_footprint_failed_breakdown_reclaim_v2"
...
class Config:
    ...
    max_pending_bars: int = 60
    skip_ambiguous_entry_bar: bool = True

    # V2 confirmation / exits.
    hold_required_bars: int = 1
    hold_buffer_pct: float = 0.0006
    tp1_r: float = 1.0
    ...
    out_dir: Path = Path("data/reports/hf/eth_hf_range_footprint_failed_breakdown_reclaim_v2")
...
def generate_signals(features: pd.DataFrame, cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Generate V2 signals with an explicit reclaim-hold-break state machine.

    V1 placed the buy stop immediately after the trap bar.  V2 first requires a
    later bar to hold above the breakdown level, then places a fresh buy stop
    above that hold bar.  This avoids buying every small reflex bounce.
    """
    ...
    print(
        f"[{STRATEGY_NAME}][signals] start scan V2 | loop_start_i={loop_start_i:,} "
    ...
    hold_low = np.full(n, np.nan)
    hold_high = np.full(n, np.nan)
    ...
    def place_pending_from_hold(i: int, setup: dict[str, Any]) -> dict[str, Any] | None:
        nonlocal setup_seq
        trigger = float(max(high_a[i], setup["trap_high"]) * (1.0 + cfg.entry_buffer_pct))
        stop = float(setup["stop"])
        ok, risk_info = _setup_passes_risk_filters(trigger, stop, cfg)
        info = {
            **setup,
            "hold_i": int(i),
            "hold_low": float(low_a[i]),
            "hold_high": float(high_a[i]),
            "trigger": trigger,
            **risk_info,
        }
        if not ok:
            log(i, "reject_hold_risk_filter", info)
            return None
        setup_seq += 1
        pending_order = {
            **info,
            ...
            "tp1": float(trigger + cfg.tp1_r * (trigger - stop)),
        ...
        log(i, "buy_stop_placed_after_hold", pending_order)
    ...
    for i in range(loop_start_i, n):
        ...
        # Pending buy stop after hold confirmation.  It can only trigger after the hold bar.
        if pending is not None and i > pending["active_from_i"]:
            if i - pending["active_from_i"] > cfg.max_pending_bars:
                ...
                hold_low[i] = pending["hold_low"]
                hold_high[i] = pending["hold_high"]
                breakdown_level[i] = pending["breakdown_level"]
                signal_reason[i] = "failed_breakdown_reclaim_hold_break"
        ...
        # Active trap must first prove it can hold the reclaimed breakdown level.
        if active is not None and pending is None:
            if i <= active["trap_i"]:
                continue
            if i - active["trap_i"] > cfg.max_pending_bars:
                log(i, "cancel_active_hold_timeout", active)
                active = None
                cooldown_until = i + cfg.cooldown_bars
            elif low_a[i] <= active["stop"]:
                log(i, "cancel_active_invalidated_before_hold", active)
                active = None
                cooldown_until = i + cfg.cooldown_bars
            else:
                breakdown = float(active["breakdown_level"])
                hold_ok = (
                    close_a[i] >= breakdown * (1.0 + cfg.reclaim_buffer_pct)
                    and low_a[i] >= breakdown * (1.0 - cfg.hold_buffer_pct)
                )
                if hold_ok:
                    active["hold_count"] = int(active.get("hold_count", 0)) + 1
                else:
                    active["hold_count"] = 0
                if active["hold_count"] >= cfg.hold_required_bars:
                    pending = place_pending_from_hold(i, active)
                    active = None if pending is not None else active
                    if pending is None:
                        cooldown_until = i + max(1, cfg.cooldown_bars // 2)

        if pending is not None or active is not None or i < cooldown_until:
            continue
        ...
        log(i, "active_waiting_for_hold", active)
    ...
    out["hold_high"] = hold_high
    out["breakdown_level"] = breakdown_level
    out["signal_reason"] = signal_reason
    return out, pd.DataFrame(events)

def _breakeven_stop_price(entry: float, qty_open: float, entry_fee_remaining: float, cfg: Config) -> float:
    """Return stop trigger price whose conservative fill approximately covers fees."""
    if qty_open <= 0:
        return entry
    # fill * (1 - fee) - entry - entry_fee_per_unit = 0
    needed_fill = (entry + entry_fee_remaining / qty_open) / max(1.0 - cfg.fee_rate, 1e-12)
    # apply_exit_slippage(stop, long) = stop * (1 - slippage)
    return needed_fill / max(1.0 - cfg.slippage_pct, 1e-12)
...
def run_backtest(signals: pd.DataFrame, cfg: Config) -> tuple[list[dict[str, Any]], pd.DataFrame, dict[str, Any]]:
    ...
    tp1_a = d["tp1_price"].to_numpy(float) if "tp1_price" in d.columns else np.full(n, np.nan)
    ...
    def close_position(i: int, exit_price: float, exit_reason: str) -> None:
        ...
        exit_fee = abs(pos["qty_open"] * fill) * cfg.fee_rate
        final_pnl = (fill - pos["entry"]) * pos["qty_open"] - pos["entry_fee_remaining"] - exit_fee
        total_pnl = pos["realized_pnl"] + final_pnl
        total_fee = pos["fee_paid"] + pos["entry_fee_remaining"] + exit_fee
        before = capital
        capital += total_pnl
        ...
        trades.append(
            {
                ...
                "fee": total_fee,
                "capital": capital,
                "return_pct": total_pnl / max(before, 1e-12),
                ...
                "tp1_realized_pnl": float(pos["realized_pnl"]),
            }
        )
        pos = None
    ...
    for i in range(n):
        ...
        if pos is None:
            ...
            eq = capital + pos["realized_pnl"] + (close_a[i] - pos["entry"]) * pos["qty_open"]
        peak = max(peak, eq)
        ...
        if pos is not None:
            ...
            # Conservative same-bar priority: stop first, then final target, then partial TP1.
            if low_a[i] <= pos["stop"]:
                reason = "stop_be_after_tp1" if pos["tp1_done"] else "stop_trap_low"
                close_position(i, pos["stop"], reason)
                continue
            if high_a[i] >= pos["target"]:
                close_position(i, pos["target"], "target_runner")
                continue
            if (not pos["tp1_done"]) and high_a[i] >= pos["tp1"]:
                part_qty = pos["qty_open"] * cfg.tp1_fraction
                if part_qty > 0:
                    fill = apply_exit_slippage(float(pos["tp1"]), 1, cfg.slippage_pct)
                    part_entry_fee = pos["entry_fee_remaining"] * (part_qty / max(pos["qty_open"], 1e-12))
                    exit_fee = abs(part_qty * fill) * cfg.fee_rate
                    part_pnl = (fill - pos["entry"]) * part_qty - part_entry_fee - exit_fee
                    pos["realized_pnl"] += part_pnl
                    pos["fee_paid"] += part_entry_fee + exit_fee
                    pos["entry_fee_remaining"] -= part_entry_fee
                    pos["qty_open"] -= part_qty
                    pos["tp1_done"] = True
                    be_stop = _breakeven_stop_price(pos["entry"], pos["qty_open"], pos["entry_fee_remaining"], cfg)
                    pos["stop"] = max(pos["stop"], be_stop)
            if pos is not None and i - pos["entry_i"] >= cfg.max_holding_bars:
                close_position(i, close_a[i], "time_bars")
                continue
        ...
        if pos is None and sig_a[i] == 1:
            ...
            tp1 = float(tp1_a[i])
            if not (np.isfinite(trigger) and np.isfinite(stop) and np.isfinite(target) and np.isfinite(tp1)):
                continue
            ...
            if not (entry > stop_fill_est and target_fill_est > entry and tp1 > entry):
                continue
            ...
            pos = {
                ...
                "entry_fee_remaining": float(entry_fee),
                "fee_paid": 0.0,
                "realized_pnl": 0.0,
                "entry_time": ts,
                "entry_i": int(i),
    ...
    if pos is not None:
        close_position(n - 1, close_a[-1], "final")
    ...
    summary.update(
        {
            ...
            "tp1_done_trades": int(sum(1 for x in trades if x.get("tp1_done"))),
            "range_pct": cfg.range_pct,
            "price_step": cfg.price_step,
            ...
            "hold_required_bars": cfg.hold_required_bars,
            "hold_buffer_pct": cfg.hold_buffer_pct,
...
def write_outputs(signals: pd.DataFrame, events: pd.DataFrame, trades: list[dict[str, Any]], equity: pd.DataFrame, summary: dict[str, Any], cfg: Config) -> None:
    ...
    signal_cols = [
        ...
        "prior_low", "entry_trigger", "initial_stop", "tp1_price", "target_price", "planned_net_reward_risk",
        "target_distance_pct", "raw_stop_pct", "trap_low", "trap_high", "hold_low", "hold_high", "breakdown_level",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-pending-bars", type=int, default=60)
    p.add_argument("--hold-required-bars", type=int, default=1)
    p.add_argument("--hold-buffer-pct", type=float, default=0.0006)
    p.add_argument("--tp1-r", type=float, default=1.0)
    p.add_argument("--tp1-fraction", type=float, default=0.40)
...
def main() -> None:
    ...
    cfg = Config(
        ...
        hold_required_bars=args.hold_required_bars,
        hold_buffer_pct=args.hold_buffer_pct,
        tp1_r=args.tp1_r,
