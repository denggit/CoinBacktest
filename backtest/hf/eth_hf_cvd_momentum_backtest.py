...
# -*- coding: utf-8 -*-
"""
ETH HF CVD Momentum Backtest
============================

放置位置：backtest/eth_hf_cvd_momentum_backtest.py

运行示例：
    python backtest/eth_hf_cvd_momentum_backtest.py --start-date 2026-06-01 --end-date 2026-06-07
...
STRATEGY_NAME = "eth_hf_cvd_momentum"
...
class StrategyConfig:
    ...
    signal_seconds: int = 5
    context_seconds: int = 300
    cvd_quantile: float = 0.85
    buy_ratio_min: float = 0.65
    sell_ratio_min: float = 0.65
    min_ret_pct: float = 0.0004
    stop_loss_pct: float = 0.0012
    take_profit_pct: float = 0.0022
    max_hold_seconds: int = 180
    exit_on_cvd_flip: bool = True
...
def build_features(bars: pd.DataFrame, cfg: StrategyConfig) -> pd.DataFrame:
    """5秒 CVD / 主动成交进攻延续。"""
    df = bars.copy()
    win = int(cfg.signal_seconds)
    df["buy_notional_5s"] = rolling_sum(df["buy_notional"], win)
    df["sell_notional_5s"] = rolling_sum(df["sell_notional"], win)
    df["cvd_5s"] = rolling_sum(df["cvd_notional"], win)
    df["volume_5s"] = df["buy_notional_5s"] + df["sell_notional_5s"]
    df["buy_ratio_5s"] = df["buy_notional_5s"] / df["volume_5s"].replace(0, pd.NA)
    df["sell_ratio_5s"] = df["sell_notional_5s"] / df["volume_5s"].replace(0, pd.NA)
    df["ret_5s"] = df["close"] / df["close"].shift(win) - 1.0
    df["cvd_long_q"] = rolling_quantile_shifted(df["cvd_5s"], cfg.context_seconds, cfg.cvd_quantile)
    df["cvd_short_q"] = rolling_quantile_shifted(df["cvd_5s"], cfg.context_seconds, 1.0 - cfg.cvd_quantile)

    long_signal = (
        (df["cvd_5s"] > df["cvd_long_q"])
        & (df["buy_ratio_5s"] >= cfg.buy_ratio_min)
        & (df["ret_5s"] >= cfg.min_ret_pct)
    )
    short_signal = (
        (df["cvd_5s"] < df["cvd_short_q"])
        & (df["sell_ratio_5s"] >= cfg.sell_ratio_min)
        & (df["ret_5s"] <= -cfg.min_ret_pct)
    )
    df["signal"] = 0
    ...
    df["signal_reason"] = ""
    df.loc[long_signal, "signal_reason"] = "CVD_BUY_MOMENTUM"
    df.loc[short_signal, "signal_reason"] = "CVD_SELL_MOMENTUM"
        ...
        if state.in_pos:
            ...
            if state.side == 1:
                ...
                if low <= state.stop_price:
                    ...
                    reason = "TAKE_PROFIT"
                elif cfg.exit_on_cvd_flip and hasattr(row, "cvd_5s") and pd.notna(row.cvd_5s) and float(row.cvd_5s) < 0:
                    exit_now = True
                    exit_price = apply_exit_slippage(close, state.side, cfg.slippage_pct)
                    reason = "CVD_FLIP_EXIT"
            else:
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 高频 CVD 主动成交动量策略回测")
    ...
    p.add_argument("--out-dir", default="data/reports/eth_hf_cvd_momentum")
    p.add_argument("--write-full-audit", action="store_true", help="写出 1秒全量特征审计，文件可能很大")
    p.add_argument("--min-ret-pct", type=float, default=0.0004)
    p.add_argument("--stop-loss-pct", type=float, default=0.0012)
    p.add_argument("--take-profit-pct", type=float, default=0.0022)
    return p.parse_args()
