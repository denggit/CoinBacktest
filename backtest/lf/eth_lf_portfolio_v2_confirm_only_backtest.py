...
# -*- coding: utf-8 -*-
"""
ETH LF Portfolio V2 Confirm-Only Backtest
========================================

放置位置建议：backtest/lf/eth_lf_portfolio_v2_confirm_only_backtest.py
运行示例：
    python backtest/lf/eth_lf_portfolio_v2_confirm_only_backtest.py --start-date 2023-01-01 --end-date 2026-06-15 --preset turbo

组合定位：
    低频组合调度器 V2。V1 复盘发现 Turtle 单独交易为负贡献，因此 V2 改为确认器，不再允许 Turtle 单独开仓：
        1) 同一时刻只允许一个组合持仓；
        2) V8 Trend Rider 是主引擎；
        3) Turtle V2 只做超级趋势确认器；
        4) V8 与 Turtle 同方向共振时提高质量权重；
        5) V8 与 Turtle 冲突时按 V8 执行，但记录 conflict；
        6) 只有 Turtle 信号、V8 没信号时不交易，避免负贡献引擎抢仓；
        7) 入场/加仓/退出沿用 V8 SAFE 执行，不用同 bar 新 stop 触发。
...
STRATEGY_NAME = "eth_lf_portfolio_v2_confirm_only"
REPORT_STRATEGY_NAME = "ETH_LF_Portfolio_V2_ConfirmOnly"
...
def make_turtle_config(args: argparse.Namespace, v8_cfg: V8Config) -> TurtleConfig:
    return TurtleConfig(
        symbol=args.symbol,
        initial_capital=args.initial_capital,
        # Turtle 在 V2 中只作为确认器；这里保留配置是为了生成 Turtle 信号。
...
def select_portfolio_signals(v8: pd.DataFrame, turtle: pd.DataFrame, cfg: V8Config, args: argparse.Namespace) -> pd.DataFrame:
    ...
    V2 rule set:
    1. V8 is the only standalone trading engine.
    2. If V8 and Turtle agree on direction, choose confluence and boost quality.
    3. If they conflict, V8 wins and the conflict is only recorded for diagnostics.
    4. If only Turtle signals, ignore it. V1 report showed Turtle standalone was negative contribution.
    5. Only one final signal column is passed into the execution engine.
    """
    out = v8.copy()
    ...
    out["turtle_long_exit_channel"] = turtle["long_exit_channel"].astype("boolean").fillna(False).astype(bool)
    out["turtle_short_exit_channel"] = turtle["short_exit_channel"].astype("boolean").fillna(False).astype(bool)
    ...
    # V2: Turtle is confirmation-only. Turtle-only bars are explicitly ignored.
    # This prevents a negative standalone engine from taking the single active slot.
    ...
    out.loc[only_turtle, "selected_engine"] = "TURTLE_V2_IGNORED"
    out.loc[only_turtle, "selected_priority"] = PRIORITY["TURTLE_V2"]
    out.loc[only_turtle, "turtle_only_ignored"] = True

    # 风险调整：只对 V8+Turtle 共振加质量权重。
    # boost 不能过大；目前共振样本很少，默认只设为 1.30，避免按单笔历史收益过拟合。
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "selected_engine", "selected_priority", "portfolio_conflict", "portfolio_agreement", "turtle_only_ignored",
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH LF Portfolio V2 Confirm-Only Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH LF Portfolio V2: one-position scheduler; V8 trades, Turtle only confirms.")
    ...
    p.add_argument("--turtle-unit-risk-scale", type=float, default=0.45, help="仅用于生成 Turtle 信号；V2 不允许 Turtle 单独开仓。")
    p.add_argument("--turtle-max-units", type=int, default=3, help="仅用于生成 Turtle 信号；V2 不允许 Turtle 单独开仓。")
    p.add_argument("--confluence-quality-boost", type=float, default=1.30)
    p.add_argument("--out-dir", default="data/reports/lf/eth_lf_portfolio_v2_confirm_only")
    return p.parse_args()
...
def main() -> int:
    ...
    print("Signal counts:", {
        ...
        "turtle_only_ignored": int(features.turtle_only_ignored.sum()),
    })
    ...
    summary["conflict_rule"] = "V8 wins conflict; same-direction V8+Turtle gets confluence boost; Turtle-only signals are ignored."
