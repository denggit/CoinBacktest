...
# -*- coding: utf-8 -*-
"""
ETH LF Portfolio V8 Micro Confirmation Scaled
===========================================

组合定位：
    在 V7B micro confirmation 基础上新增全局风险缩放层。
    LF 引擎仍然负责方向；range/footprint 只用于在 4H 信号确认时降低逆向订单流信号的 entry risk，
    默认不增加仓位，只尝试降低坏入场风险。

设计原则：
    - 不把其他 fallback 硬塞进来；Bull V2 只补 V5 空档，不做叠加收益。
    ...
    - 默认 global-risk-scale=1.13：把 V7B soft_05 的低回撤空间转化为真实执行层风险放大。
"""
...
STRATEGY_NAME = "eth_lf_portfolio_v8_micro_confirm_scaled"
REPORT_STRATEGY_NAME = "ETH_LF_Portfolio_V8_MicroConfirmScaled"
        ...
        if in_pos:
            ...
            if in_pos and units < active_cfg.max_units:
                ...
                if add_triggered:
                    ...
                    add_q = unit_qty(capital, add_price, add_stop_dist, qty, active_cfg, add_risk_mult)
...
def attach_engine_to_trades(trades: list[dict[str, Any]], features: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    for trade in trades:
        ...
        if signal_time in features.index:
            ...
            item["global_risk_scale"] = float(row.get("global_risk_scale", 1.0))
            item["final_entry_risk_scale"] = float(row.get("micro_entry_risk_scale", 1.0)) * float(row.get("global_risk_scale", 1.0))
            ...
            item["global_risk_scale"] = 1.0
            item["final_entry_risk_scale"] = 1.0
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    trades_df.to_csv(out_dir / f"{STRATEGY_NAME}_trades.csv", index=False)
    ...
    audit_cols = [
        ...
        "micro_context_available", "micro_aligned", "micro_contra", "micro_entry_risk_scale",
        "global_risk_scale", "final_entry_risk_scale", "micro_filter_action",
        "rf_bar_count", "rf_micro_return_pct", "rf_close_pos", "rf_delta_sum", "rf_imbalance", "rf_taker_buy_ratio",
        "rf_max_sell_bucket_share", "rf_max_buy_bucket_share",
        "momentum_long_exit_channel", "momentum_short_exit_channel", "bear_short_exit_channel", "bull_long_exit_channel",
    ]
    audit = features[[c for c in audit_cols if c in features.columns]]
    audit.to_csv(out_dir / f"{STRATEGY_NAME}_signal_audit.csv")
    audit.to_csv(out_dir / "signal_audit.csv")
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 92)
    print("ETH LF Portfolio V8 Micro Confirmation Scaled Backtest Summary")
    print("=" * 92)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH LF Portfolio V8: V7B soft micro confirmation + global risk scale.")
    ...
    p.add_argument("--global-risk-scale", type=float, default=1.13, help="V8 global risk multiplier applied to all new entry/add sizing after V7B micro risk scaling.")
...
def main() -> int:
    ...
    if args.out_dir:
        ...
        if args.micro_filter_mode == "soft":
            micro_tag = f"soft_{_tag_float(args.micro_not_aligned_risk_scale).replace('0p', '0')}"
        else:
            micro_tag = args.micro_filter_mode
        scale_tag = f"gs_{_tag_float(args.global_risk_scale)}"
        out_dir = Path(PROJECT_ROOT) / "data/reports/lf" / STRATEGY_NAME / f"{micro_tag}_{scale_tag}"
    ...
    features["global_risk_scale"] = float(args.global_risk_scale)
    features["final_entry_risk_scale"] = features["micro_entry_risk_scale"].astype(float) * float(args.global_risk_scale)
    ...
    trades, equity = run_priority_backtest(
        ...
        global_risk_scale=float(args.global_risk_scale),
    )
    ...
    summary["global_risk_scale"] = float(args.global_risk_scale)
    summary["v8_design_note"] = "V7B micro confirmation is unchanged; global_risk_scale only scales real execution sizing."
    ...
    summary["conflict_rule"] = "Momentum V3 first; Bear V3 standalone second; Bull Reclaim V2 long only when Momentum/Bear silent; range/footprint micro confirmation only risk-filters entry; V8 global risk scale multiplies sizing; no hedge."
