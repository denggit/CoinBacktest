...
# -*- coding: utf-8 -*-
"""
ETH 1D+4H Bear Short Engine V2
==============================

放置位置建议：backtest/lf/eth_1d_4h_bear_short_engine_v2_backtest.py
运行示例：
    python backtest/lf/eth_1d_4h_bear_short_engine_v2_backtest.py --start-date 2023-01-01 --end-date 2026-06-15 --preset high

定位：
    第二个独立引擎的第二版。V2 在 V1 基础上加入 bear permission 与追空过度过滤，目标是降低 2023/2025 震荡反抽亏损，同时保留 2026 熊市爆发力。
...
STRATEGY_NAME = "eth_1d_4h_bear_short_engine_v2"
REPORT_STRATEGY_NAME = "ETH_1D_4H_BearShortEngine_V2"
...
PRESETS: dict[str, dict[str, float | int | str]] = {
    ...
    "stable": {"unit_risk_per_trade": 0.018, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "bear_permission_v2"},
    "high": {"unit_risk_per_trade": 0.022, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "bear_permission_v2"},
    "turbo": {"unit_risk_per_trade": 0.030, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "bear_permission_v2"},
    "ultra": {"unit_risk_per_trade": 0.040, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "bear_permission_v2"},
    "max": {"unit_risk_per_trade": 0.055, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "bear_permission_v2"},
}
...
def build_bear_features(base_4h: pd.DataFrame, cfg: BearConfig) -> pd.DataFrame:
    ...
    # 3) ret_12 < 2% avoids shorting immediately after a 4H rebound squeeze.
    d1_major_bear = (
    ...
    elif cfg.style == "bear_permission_v2":
    ...
    out["bear_permission_v2"] = bear_permission_v2.fillna(False).astype(bool)
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH 1D+4H Bear Short Engine V2 Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH short-only Bear/Crash engine V2 with bear-permission filter and safe execution.")
    ...
    p.add_argument("--style", choices=["breakdown", "crash_continuation", "bear_permission_v2", "combo"], default=None)
    ...
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_bear_short_engine_v2")
    return p.parse_args()
