...
# -*- coding: utf-8 -*-
"""
ETH 1D+4H Pullback Continuation V3
==================================

定位：
    第三个独立引擎的第三版。它不是为了替代 V4B 这种低胜率高盈亏比趋势系统，
...
STRATEGY_NAME = "eth_1d_4h_pullback_continuation_v3"
REPORT_STRATEGY_NAME = "ETH_1D_4H_PullbackContinuation_V3"

PRESETS: dict[str, dict[str, float | int]] = {
    "stable": {"unit_risk_per_trade": 0.016, "max_total_notional_mult": 7.0, "max_units": 1, "max_risk_mult": 1.8},
    "high": {"unit_risk_per_trade": 0.024, "max_total_notional_mult": 8.0, "max_units": 1, "max_risk_mult": 1.8},
    "turbo": {"unit_risk_per_trade": 0.030, "max_total_notional_mult": 9.0, "max_units": 1, "max_risk_mult": 1.8},
}
...
def build_features(base_4h: pd.DataFrame, cfg: PullbackConfig) -> pd.DataFrame:
    ...
    # V3: two-bucket long-only continuation.
    # A bucket keeps the high-quality V2 setup.
    # B bucket adds more trades but uses lower quality_mult, so it supplements without taking full risk.
    base_long = out["trend_long"] & out["recent_long_pullback"] & out["long_reclaim"] & out["long_not_extended"]
    out["quality_bucket_a"] = (
        base_long
        & (out["rsi"] > 55.0)
        & (out["d1_distance"].between(0.030, 0.100))
        & (out["adx"] < 25.0)
    )
    out["quality_bucket_b"] = (
        base_long
        & (~out["quality_bucket_a"])
        & (out["rsi"] > 52.0)
        & (out["d1_distance"].between(0.020, 0.100))
        & (out["adx"] < 30.0)
    )
    out["continuation_quality"] = out["quality_bucket_a"] | out["quality_bucket_b"]
    out["long_signal"] = out["continuation_quality"]
    ...
    out.loc[out["quality_bucket_b"], "quality_mult"] = 0.35
    out.loc[out["quality_bucket_a"] & (out["low"] <= out["ema50"] * (1.0 + cfg.pullback_band)), "quality_mult"] *= 1.05
    out["quality_mult"] = out["quality_mult"].clip(0.25, 1.30)
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "long_reclaim", "short_reject", "long_not_extended", "short_not_extended", "quality_bucket_a", "quality_bucket_b", "continuation_quality",
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 90)
    print("ETH 1D+4H Pullback Continuation V3 Backtest Summary")
    print("=" * 90)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 1D+4H Pullback Continuation V3: higher win-rate trend pullback engine.")
    ...
    p.add_argument("--enable-short", action="store_true", help="V2 默认 long-only。此参数仅保留接口，当前信号仍不启用 short。")
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_pullback_continuation_v3")
    return p.parse_args()
