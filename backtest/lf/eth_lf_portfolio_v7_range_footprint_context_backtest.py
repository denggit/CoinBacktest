...
# -*- coding: utf-8 -*-
"""
ETH LF Portfolio V7 Range/Footprint Context Risk Filter
===========================================
...
设计原则：
    ...
    - 默认 micro-filter-mode=soft：只降低逆向 micro context 的入场风险，不跳过信号、不放大信号。
"""
...
STRATEGY_NAME = "eth_lf_portfolio_v7_range_footprint_context"
REPORT_STRATEGY_NAME = "ETH_LF_Portfolio_V7_RangeFootprintContext"
...
def apply_micro_context_filter(features: pd.DataFrame, micro_ctx: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    out.loc[long_contra | short_contra, "micro_entry_risk_scale"] = float(args.micro_contra_risk_scale)
    out.loc[long_contra | short_contra, "micro_filter_action"] = "RISK_REDUCED"

    if args.micro_filter_mode == "strict":
        blocked = long_contra | short_contra
        ...
        out.loc[blocked, "micro_filter_action"] = "BLOCKED"
    elif args.micro_filter_mode == "soft":
        # Default V7 behavior: do not boost aligned signals; only reduce contradicted entries.
        pass
    else:
    ...
    print("Micro context counts:", {
        ...
        "risk_reduced": int((out["micro_filter_action"] == "RISK_REDUCED").sum()),
        "blocked": int((out["micro_filter_action"] == "BLOCKED").sum()),
    }, flush=True)
    return out
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 92)
    print("ETH LF Portfolio V7 Range/Footprint Context Risk Filter Backtest Summary")
    print("=" * 92)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH LF Portfolio V7: V6 base + range/footprint context risk filter.")
    ...
    p.add_argument("--micro-contra-risk-scale", type=float, default=0.50, help="Soft mode risk scale for contradicted micro context. Default only reduces risk, never boosts.")
...
def main() -> int:
    ...
    summary["conflict_rule"] = "Momentum V3 first; Bear V3 standalone second; Bull Reclaim V2 long only when Momentum/Bear silent; range/footprint context only risk-filters entry; no hedge."
