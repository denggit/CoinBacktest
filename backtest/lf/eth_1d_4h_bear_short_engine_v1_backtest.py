...
# -*- coding: utf-8 -*-
"""
ETH 1D+4H Bear Short Engine V1
==============================

放置位置建议：backtest/lf/eth_1d_4h_bear_short_engine_v1_backtest.py
运行示例：
    python backtest/lf/eth_1d_4h_bear_short_engine_v1_backtest.py --start-date 2023-01-01 --end-date 2026-06-15 --preset high

定位：
    第二个独立引擎的第一版。它不是 V8 的复制品，而是专门研究 ETH 大级别熊市 / 崩盘延续时的 short-only 引擎。
...
STRATEGY_NAME = "eth_1d_4h_bear_short_engine_v1"
REPORT_STRATEGY_NAME = "ETH_1D_4H_BearShortEngine_V1"
...
PRESETS: dict[str, dict[str, float | int | str]] = {
    ...
    # 默认：Bear/Crash continuation，PF 较好，但非熊市年份可能小亏。
    "high": {"unit_risk_per_trade": 0.022, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "crash_continuation"},
    "turbo": {"unit_risk_per_trade": 0.030, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "crash_continuation"},
    "ultra": {"unit_risk_per_trade": 0.040, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3, "style": "crash_continuation"},
}
...
def build_bear_features(base_4h: pd.DataFrame, cfg: BearConfig) -> pd.DataFrame:
    ...
    if cfg.style == "breakdown":
        ...
        short_signal = breakdown | crash_continuation
    else:
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH 1D+4H Bear Short Engine V1 Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH short-only Bear/Crash engine V1 with safe execution.")
    ...
    p.add_argument("--style", choices=["breakdown", "crash_continuation", "combo"], default=None)
    ...
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_bear_short_engine_v1")
    return p.parse_args()
