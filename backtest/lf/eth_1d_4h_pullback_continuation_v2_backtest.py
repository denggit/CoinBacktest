...
# -*- coding: utf-8 -*-
"""
ETH 1D+4H Pullback Continuation V2
==================================

定位：
    第三个独立引擎的第二版。它不是为了替代 V4B 这种低胜率高盈亏比趋势系统，
...
STRATEGY_NAME = "eth_1d_4h_pullback_continuation_v2"
REPORT_STRATEGY_NAME = "ETH_1D_4H_PullbackContinuation_V2"

PRESETS: dict[str, dict[str, float | int]] = {
    "stable": {"unit_risk_per_trade": 0.010, "max_total_notional_mult": 5.0, "max_units": 1, "max_risk_mult": 1.8},
    "high": {"unit_risk_per_trade": 0.020, "max_total_notional_mult": 8.0, "max_units": 1, "max_risk_mult": 1.8},
...
def build_features(base_4h: pd.DataFrame, cfg: PullbackConfig) -> pd.DataFrame:
    ...
    # V2: long-only high-quality continuation. It waits for the reclaim to be stronger,
    # not just the first weak bounce after pullback.
    out["continuation_quality"] = (
        (out["rsi"] > 55.0)
        & (out["d1_distance"].between(0.030, 0.100))
        & (out["adx"] < 25.0)
    )
    out["long_signal"] = out["trend_long"] & out["recent_long_pullback"] & out["long_reclaim"] & out["long_not_extended"] & out["continuation_quality"]
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "long_reclaim", "short_reject", "long_not_extended", "short_not_extended", "continuation_quality",
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 90)
    print("ETH 1D+4H Pullback Continuation V2 Backtest Summary")
    print("=" * 90)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 1D+4H Pullback Continuation V2: higher win-rate trend pullback engine.")
    ...
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_pullback_continuation_v2")
    return p.parse_args()
