...
# -*- coding: utf-8 -*-
"""
ETH 1D+4H Momentum Breakout V2
==============================

定位：
    新的独立引擎 V2。V1 收益弹性强，但 2023 假突破亏损明显。
    V2 增加周线确认和 long quality 分桶：强多头突破正常风险，弱多头突破只给小风险。
...
STRATEGY_NAME = "eth_1d_4h_momentum_breakout_v2"
REPORT_STRATEGY_NAME = "ETH_1D_4H_MomentumBreakout_V2"
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "long_breakout_setup", "short_breakout_setup", "long_quality_full", "long_quality_weak", "long_signal", "short_signal", "signal",
    ]
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 92)
    print("ETH 1D+4H Momentum Breakout V2 Backtest Summary")
    print("=" * 92)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ETH 1D+4H Momentum Breakout V2")
