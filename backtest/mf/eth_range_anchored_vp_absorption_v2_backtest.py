#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Range Anchored Volume Profile Absorption Breakout V2.
...
No dynamic stop in V2. Dynamic structural trailing can be added only after this basic
...
STRATEGY_NAME = "ETH_Range_AnchoredVP_AbsorptionBreakout_V2"
...
class Config:
    ...
    max_absorption_zone_width_pct: float = 0.020
    ...
    out_dir: Path = Path("data/reports/mf/eth_range_anchored_vp_absorption_v2")
...
def generate_signals(bars: pd.DataFrame, fp_store: FootprintStore, cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for i in range(loop_start_i, n):
        ...
        # If active setup exists, keep scanning for post-break bubbles or cancellation.
        if active is not None:
            if i - active["created_i"] > cfg.max_active_setup_bars:
                ...
                if cluster is not None:
                    ...
                    trigger = active["absorption_zone_high"] * (1.0 + cfg.entry_buffer_pct)
                    ...
                    if risk > 0 and reward > 0 and rr >= cfg.min_reward_risk:
                        setup_seq += 1
                        pending = {**active, "setup_id": setup_seq, "entry_trigger": float(trigger), "initial_stop": float(stop), "target_price": float(target), "reward_risk": float(rr), "active_from_i": i}
                        log(i, "buy_stop_placed", pending)
                    else:
                        info = {**active, "entry_trigger": float(trigger), "initial_stop": float(stop), "target_price": float(target), "reward_risk": float(rr) if np.isfinite(rr) else np.nan}
                        log(i, "reject_order_low_rr", info)
                        active = None
                        cooldown_until = i + cfg.cooldown_bars
        ...
        if risk > 0 and reward > 0 and rr >= cfg.min_reward_risk:
            setup_seq += 1
            pending = {**setup, "setup_id": setup_seq, "entry_trigger": float(trigger), "initial_stop": float(stop), "target_price": float(target), "reward_risk": float(rr), "active_from_i": i}
            active = setup
            log(i, "buy_stop_placed", pending)
        else:
            info = {**setup, "entry_trigger": float(trigger), "initial_stop": float(stop), "target_price": float(target), "reward_risk": float(rr) if np.isfinite(rr) else np.nan}
            log(i, "reject_order_low_rr", info)
            active = None
            cooldown_until = i + cfg.cooldown_bars
...
def run_backtest(signals: pd.DataFrame, cfg: Config) -> tuple[list[dict[str, Any]], pd.DataFrame, dict[str, Any]]:
    ...
    for i in range(n):
        ...
        if pos is None and sig[i] == 1:
            ...
            if risk <= 0 or reward <= 0 or reward / risk < cfg.min_reward_risk:
                continue
    ...
    summary.update({"signal_count": int((d["signal"] == 1).sum()), "range_pct": cfg.range_pct, "price_step": cfg.price_step, "fee_rate_per_side": cfg.fee_rate, "slippage_pct": cfg.slippage_pct, "unit_risk_per_trade": cfg.unit_risk_per_trade})
...
def write_outputs(signals: pd.DataFrame, events: pd.DataFrame, trades: list[dict[str, Any]], equity: pd.DataFrame, summary: dict[str, Any], cfg: Config) -> None:
    ...
    pd.DataFrame(trades).to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v2_trades.csv", index=False)
    equity.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v2_equity.csv")
    events.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v2_setup_events.csv", index=False)
    events["event"].value_counts().rename_axis("event").reset_index(name="count").to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v2_event_counts.csv", index=False)
    sigs = signals[signals["signal"] == 1].copy()
    cols = ["end_ts", "bar_id", "open", "high", "low", "close", "setup_id", "entry_trigger", "initial_stop", "target_price", "profile_vah", "profile_val", "profile_poc", "lower_poc", "absorption_low", "absorption_high", "bubble_count", "signal_reason"]
    sigs[[c for c in cols if c in sigs.columns]].to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v2_signal_audit.csv", index=False)
    if cfg.write_full_audit:
        signals.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v2_full_audit.csv", index=False)
    with (cfg.out_dir / "eth_range_anchored_vp_absorption_v2_summary.json").open("w", encoding="utf-8") as f:
...
def main() -> None:
    args = parse_args()
    cfg = Config(symbol=args.symbol, start_date=args.start_date, end_date=args.end_date, warmup_start_date=args.warmup_start_date, initial_capital=args.initial_capital, fee_rate=args.fee_rate, slippage_pct=args.slippage_pct, range_pct=args.range_pct, price_step=args.price_step, data_dir=args.data_dir, write_full_audit=args.write_full_audit)
    for k, v in PRESETS[args.preset].items():
        setattr(cfg, k, v)
    cfg.out_dir = Path(args.out_dir) if args.out_dir else Path(f"data/reports/mf/eth_range_anchored_vp_absorption_v2/{args.preset}_r{int(cfg.range_pct*10000):04d}_step{cfg.price_step:g}")
