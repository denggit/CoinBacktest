#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Range Anchored Volume Profile Absorption Breakout V8.
...
No dynamic stop in V8. Dynamic structural trailing can be added only after this basic
...
STRATEGY_NAME = "ETH_Range_AnchoredVP_AbsorptionBreakout_V8"
...
class Config:
    ...
    # V8: absorption should happen in the lower part of the VAL -> lower-POC band.
    # This is a structural rule, not a result-fitted width optimization.
    absorption_location_mode: str = "lower_band"  # any | lower_band
    ...
    out_dir: Path = Path("data/reports/mf/eth_range_anchored_vp_absorption_v8")
...
def _cluster_absorption(bars_slice: pd.DataFrame, profile: dict[str, float], cfg: Config) -> dict[str, Any] | None:
    ...
    if cfg.absorption_location_mode == "lower_band":
        # Fabio-style long absorption should be a lower acceptance/absorption area,
        # not any sell bubble scattered across the full VAL -> lower-POC band.
        # The position is known at setup time because profile and bubbles are frozen
        # using historical bars only.
...
def write_outputs(signals: pd.DataFrame, events: pd.DataFrame, trades: list[dict[str, Any]], equity: pd.DataFrame, summary: dict[str, Any], cfg: Config) -> None:
    ...
    pd.DataFrame(trades).to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v8_trades.csv", index=False)
    equity.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v8_equity.csv")
    events.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v8_setup_events.csv", index=False)
    events["event"].value_counts().rename_axis("event").reset_index(name="count").to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v8_event_counts.csv", index=False)
    ...
    sigs[[c for c in cols if c in sigs.columns]].to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v8_signal_audit.csv", index=False)
    if cfg.write_full_audit:
        signals.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v8_full_audit.csv", index=False)
    with (cfg.out_dir / "eth_range_anchored_vp_absorption_v8_summary.json").open("w", encoding="utf-8") as f:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--absorption-location-mode", choices=["any", "lower_band"], default="lower_band")
...
def main() -> None:
    args = parse_args()
    cfg = Config(symbol=args.symbol, start_date=args.start_date, end_date=args.end_date, warmup_start_date=args.warmup_start_date, initial_capital=args.initial_capital, fee_rate=args.fee_rate, slippage_pct=args.slippage_pct, range_pct=args.range_pct, price_step=args.price_step, data_dir=args.data_dir, max_holding_hours=args.max_holding_hours, min_reward_risk=args.min_reward_risk, min_target_distance_pct=args.min_target_distance_pct, max_absorption_zone_width_pct=args.max_absorption_zone_width_pct, absorption_location_mode=args.absorption_location_mode, absorption_max_band_position=args.absorption_max_band_position, target_mode=args.target_mode, lower_poc_mode=args.lower_poc_mode, lower_poc_hvn_min_prominence=args.lower_poc_hvn_min_prominence, rr_filter_target=args.rr_filter_target, write_full_audit=args.write_full_audit)
    for k, v in PRESETS[args.preset].items():
        setattr(cfg, k, v)
    cfg.out_dir = Path(args.out_dir) if args.out_dir else Path(f"data/reports/mf/eth_range_anchored_vp_absorption_v8/{args.preset}_r{int(cfg.range_pct*10000):04d}_step{cfg.price_step:g}_{cfg.target_mode}_rr{cfg.rr_filter_target}_{cfg.lower_poc_mode}_struct")
