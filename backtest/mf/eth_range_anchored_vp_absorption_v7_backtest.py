#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Range Anchored Volume Profile Absorption Breakout V7.
...
No dynamic stop in V7. Dynamic structural trailing can be added only after this basic
...
STRATEGY_NAME = "ETH_Range_AnchoredVP_AbsorptionBreakout_V7B"
...
class Config:
    ...
    max_holding_bars: int = 260  # kept for compatibility; V7 uses max_holding_hours.
    max_holding_hours: float = 48.0
    cooldown_bars: int = 15

    out_dir: Path = Path("data/reports/mf/eth_range_anchored_vp_absorption_v7")
...
def _lower_poc(prof: pd.DataFrame, val: float, cfg: Config) -> dict[str, float] | None:
    ...
    V7 change: default mode is local HVN instead of blindly taking the maximum
...
def generate_signals(bars: pd.DataFrame, fp_store: FootprintStore, cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _prepare_order_or_retest(i: int, setup: dict[str, Any], cluster: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None, bool]:
        ...
        V7 fix: if the absorption-zone trigger has already been crossed at the
        ...
        # Key V7 realism guard. A buy stop must be above the market when it is
    ...
    for i in range(loop_start_i, n):
        ...
        if active is not None:
            if i - active["created_i"] > cfg.max_active_setup_bars:
                ...
                # V7 retest mode: old trigger was already crossed. Wait for price
...
def write_outputs(signals: pd.DataFrame, events: pd.DataFrame, trades: list[dict[str, Any]], equity: pd.DataFrame, summary: dict[str, Any], cfg: Config) -> None:
    ...
    pd.DataFrame(trades).to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v7_trades.csv", index=False)
    equity.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v7_equity.csv")
    events.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v7_setup_events.csv", index=False)
    events["event"].value_counts().rename_axis("event").reset_index(name="count").to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v7_event_counts.csv", index=False)
    sigs = signals[(signals["signal"] == 1) & (signals["end_ts"] >= pd.Timestamp(cfg.start_date))].copy()
    cols = ["end_ts", "bar_id", "open", "high", "low", "close", "setup_id", "entry_trigger", "initial_stop", "target_price", "rr_filter_target_price", "planned_reward_risk", "target_distance_pct", "absorption_zone_width_pct", "profile_vah", "profile_val", "profile_poc", "lower_poc", "absorption_low", "absorption_high", "bubble_count", "signal_reason"]
    sigs[[c for c in cols if c in sigs.columns]].to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v7_signal_audit.csv", index=False)
    if cfg.write_full_audit:
        signals.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v7_full_audit.csv", index=False)
    with (cfg.out_dir / "eth_range_anchored_vp_absorption_v7_summary.json").open("w", encoding="utf-8") as f:
...
def main() -> None:
    args = parse_args()
    cfg = Config(symbol=args.symbol, start_date=args.start_date, end_date=args.end_date, warmup_start_date=args.warmup_start_date, initial_capital=args.initial_capital, fee_rate=args.fee_rate, slippage_pct=args.slippage_pct, range_pct=args.range_pct, price_step=args.price_step, data_dir=args.data_dir, max_holding_hours=args.max_holding_hours, min_reward_risk=args.min_reward_risk, min_target_distance_pct=args.min_target_distance_pct, max_absorption_zone_width_pct=args.max_absorption_zone_width_pct, target_mode=args.target_mode, lower_poc_mode=args.lower_poc_mode, lower_poc_hvn_min_prominence=args.lower_poc_hvn_min_prominence, rr_filter_target=args.rr_filter_target, write_full_audit=args.write_full_audit)
    for k, v in PRESETS[args.preset].items():
        setattr(cfg, k, v)
    cfg.out_dir = Path(args.out_dir) if args.out_dir else Path(f"data/reports/mf/eth_range_anchored_vp_absorption_v7/{args.preset}_r{int(cfg.range_pct*10000):04d}_step{cfg.price_step:g}_{cfg.target_mode}_rr{cfg.rr_filter_target}_{cfg.lower_poc_mode}_struct")
