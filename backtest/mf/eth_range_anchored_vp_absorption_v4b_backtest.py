#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Range Anchored Volume Profile Absorption Breakout V4B.
...
No dynamic stop in V4B. Dynamic structural trailing can be added only after this basic
...
STRATEGY_NAME = "ETH_Range_AnchoredVP_AbsorptionBreakout_V4BB"
...
class Config:
    ...
    max_holding_bars: int = 260  # kept for compatibility; V4B uses max_holding_hours.
    max_holding_hours: float = 48.0
    cooldown_bars: int = 15

    out_dir: Path = Path("data/reports/mf/eth_range_anchored_vp_absorption_v4b")
...
def generate_signals(bars: pd.DataFrame, fp_store: FootprintStore, cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _prepare_order_or_retest(i: int, setup: dict[str, Any], cluster: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None, bool]:
        ...
        V4B fix: if the absorption-zone trigger has already been crossed at the
        ...
        # Key V4B realism guard. A buy stop must be above the market when it is
    ...
    for i in range(loop_start_i, n):
        ...
        if active is not None:
            if i - active["created_i"] > cfg.max_active_setup_bars:
                ...
                # V4B retest mode: old trigger was already crossed. Wait for price
...
def write_outputs(signals: pd.DataFrame, events: pd.DataFrame, trades: list[dict[str, Any]], equity: pd.DataFrame, summary: dict[str, Any], cfg: Config) -> None:
    ...
    pd.DataFrame(trades).to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_trades.csv", index=False)
    equity.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_equity.csv")
    events.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_setup_events.csv", index=False)
    events["event"].value_counts().rename_axis("event").reset_index(name="count").to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_event_counts.csv", index=False)
    ...
    sigs[[c for c in cols if c in sigs.columns]].to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_signal_audit.csv", index=False)
    if cfg.write_full_audit:
        signals.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_full_audit.csv", index=False)
    with (cfg.out_dir / "eth_range_anchored_vp_absorption_v4b_summary.json").open("w", encoding="utf-8") as f:
...
def main() -> None:
    args = parse_args()
    cfg = Config(symbol=args.symbol, start_date=args.start_date, end_date=args.end_date, warmup_start_date=args.warmup_start_date, initial_capital=args.initial_capital, fee_rate=args.fee_rate, slippage_pct=args.slippage_pct, range_pct=args.range_pct, price_step=args.price_step, data_dir=args.data_dir, max_holding_hours=args.max_holding_hours, target_mode=args.target_mode, rr_filter_target=args.rr_filter_target, write_full_audit=args.write_full_audit)
    for k, v in PRESETS[args.preset].items():
        setattr(cfg, k, v)
    cfg.out_dir = Path(args.out_dir) if args.out_dir else Path(f"data/reports/mf/eth_range_anchored_vp_absorption_v4b/{args.preset}_r{int(cfg.range_pct*10000):04d}_step{cfg.price_step:g}_{cfg.target_mode}_rr{cfg.rr_filter_target}")
