#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Range Anchored Volume Profile Absorption Breakout V5.
...
No dynamic stop in V5. Dynamic structural trailing can be added only after this basic
...
STRATEGY_NAME = "ETH_Range_AnchoredVP_AbsorptionBreakout_V5B"
...
class Config:
    ...
    max_absorption_zone_width_pct: float = 0.020  # hard safety cap for any zone.
    absorption_cluster_width_pct: float = 0.0035  # V5: choose one dense price cluster, not all bubbles.
    ...
    max_holding_bars: int = 260  # kept for compatibility; V5 uses max_holding_hours.
    max_holding_hours: float = 48.0
    cooldown_bars: int = 15

    out_dir: Path = Path("data/reports/mf/eth_range_anchored_vp_absorption_v5")
...
def _lower_poc(prof: pd.DataFrame, val: float, cfg: Config) -> dict[str, float] | None:
    ...
    j = int(np.argmin(np.abs(prices - lp)))
    lo = hi = j
...
def _cluster_absorption(bars_slice: pd.DataFrame, profile: dict[str, float], cfg: Config) -> dict[str, Any] | None:
    """Find a dense repeated sell-bubble absorption cluster.

    V5 change: do not merge all qualifying bubbles across the whole VAL-to-lower-POC
    region. Fabio-style absorption should be a compact price zone that is tested
    repeatedly. We first find all qualifying sell bubbles, then choose the best
    compact price cluster by total sell notional / width.
    """
    if bars_slice.empty:
        return None
    ...
    bubbles = bars_slice.loc[mask].copy()
    raw_bubble_count = int(len(bubbles))
    if raw_bubble_count < cfg.min_absorption_bubbles:
        return None

    bubbles["_bucket"] = bubbles["max_sell_bucket"].astype(float)
    bubbles["_zone_low"] = np.maximum(profile["lower_poc_zone_low"], np.minimum(bubbles["low"].astype(float), bubbles["_bucket"]))
    bubbles["_zone_high"] = np.maximum.reduce([
        bubbles["_bucket"].to_numpy(dtype=float, copy=False) + cfg.price_step,
        bubbles["close"].to_numpy(dtype=float, copy=False),
        bubbles["open"].to_numpy(dtype=float, copy=False),
    ])
    bubbles = bubbles.sort_values(["_bucket", "end_ts"]).reset_index(drop=True)

    zone_low_a = bubbles["_zone_low"].to_numpy(dtype=float, copy=False)
    zone_high_a = bubbles["_zone_high"].to_numpy(dtype=float, copy=False)
    sell_a = bubbles["max_bucket_sell_notional"].to_numpy(dtype=float, copy=False)
    ...
    for left in range(m):
        ...
        for right in range(left, m):
            z_low = min(z_low, float(zone_low_a[right]))
            z_high = max(z_high, float(zone_high_a[right]))
            total_sell += float(sell_a[right])
            if z_low <= 0:
                continue
            width_pct = (z_high - z_low) / z_low
            if width_pct > cfg.max_absorption_zone_width_pct:
                break
            # V5 compact-cluster requirement. Once sorted by price, additional
            # higher buckets will not make this cluster narrower.
            if width_pct > cfg.absorption_cluster_width_pct:
                break
            count = right - left + 1
            if count < cfg.min_absorption_bubbles:
                continue
            # Prefer dense repeated absorption, not a wide collection of unrelated bubbles.
            score = total_sell * (count ** 0.5) / max(width_pct, 0.0005)
    ...
    chosen = bubbles.iloc[int(best["left"]): int(best["right"]) + 1]
    return {
        "bubble_count": int(best["count"]),
        "raw_bubble_count": raw_bubble_count,
        "bubble_bar_ids": [int(x) for x in chosen["bar_id"].to_list()],
        "bubble_times": [pd.Timestamp(x) for x in chosen["end_ts"].to_list()],
        "absorption_zone_low": float(best["zone_low"]),
        "absorption_zone_high": float(best["zone_high"]),
        "absorption_cluster_width_pct": float(best["width_pct"]),
        "bubble_total_sell_notional": float(best["total_sell"]),
        "cluster_score": float(best["cluster_score"]),
    }
...
def generate_signals(bars: pd.DataFrame, fp_store: FootprintStore, cfg: Config) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _prepare_order_or_retest(i: int, setup: dict[str, Any], cluster: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None, bool]:
        ...
        V5 fix: if the absorption-zone trigger has already been crossed at the
        ...
        # Key V5 realism guard. A buy stop must be above the market when it is
    ...
    for i in range(loop_start_i, n):
        ...
        if active is not None:
            if i - active["created_i"] > cfg.max_active_setup_bars:
                ...
                # V5 retest mode: old trigger was already crossed. Wait for price
...
def write_outputs(signals: pd.DataFrame, events: pd.DataFrame, trades: list[dict[str, Any]], equity: pd.DataFrame, summary: dict[str, Any], cfg: Config) -> None:
    ...
    pd.DataFrame(trades).to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v5_trades.csv", index=False)
    equity.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v5_equity.csv")
    events.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v5_setup_events.csv", index=False)
    events["event"].value_counts().rename_axis("event").reset_index(name="count").to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v5_event_counts.csv", index=False)
    sigs = signals[(signals["signal"] == 1) & (signals["end_ts"] >= pd.Timestamp(cfg.start_date))].copy()
    cols = ["end_ts", "bar_id", "open", "high", "low", "close", "setup_id", "entry_trigger", "initial_stop", "target_price", "rr_filter_target_price", "planned_reward_risk", "profile_vah", "profile_val", "profile_poc", "lower_poc", "absorption_low", "absorption_high", "bubble_count", "raw_bubble_count", "absorption_cluster_width_pct", "signal_reason"]
    sigs[[c for c in cols if c in sigs.columns]].to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v5_signal_audit.csv", index=False)
    if cfg.write_full_audit:
        signals.to_csv(cfg.out_dir / "eth_range_anchored_vp_absorption_v5_full_audit.csv", index=False)
    with (cfg.out_dir / "eth_range_anchored_vp_absorption_v5_summary.json").open("w", encoding="utf-8") as f:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--absorption-cluster-width-pct", type=float, default=0.0035)
...
def main() -> None:
    args = parse_args()
    cfg = Config(symbol=args.symbol, start_date=args.start_date, end_date=args.end_date, warmup_start_date=args.warmup_start_date, initial_capital=args.initial_capital, fee_rate=args.fee_rate, slippage_pct=args.slippage_pct, range_pct=args.range_pct, price_step=args.price_step, data_dir=args.data_dir, max_holding_hours=args.max_holding_hours, target_mode=args.target_mode, rr_filter_target=args.rr_filter_target, absorption_cluster_width_pct=args.absorption_cluster_width_pct, write_full_audit=args.write_full_audit)
    for k, v in PRESETS[args.preset].items():
        setattr(cfg, k, v)
    cfg.out_dir = Path(args.out_dir) if args.out_dir else Path(f"data/reports/mf/eth_range_anchored_vp_absorption_v5/{args.preset}_r{int(cfg.range_pct*10000):04d}_step{cfg.price_step:g}_{cfg.target_mode}_rr{cfg.rr_filter_target}")
