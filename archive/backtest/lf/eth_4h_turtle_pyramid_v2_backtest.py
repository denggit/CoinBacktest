...
# -*- coding: utf-8 -*-
"""
ETH 4H Turtle Pyramid V2 Backtest
=================================

放置位置建议：backtest/lf/eth_4h_turtle_pyramid_v2_backtest.py
运行示例：
    python backtest/lf/eth_4h_turtle_pyramid_v2_backtest.py --start-date 2023-01-01 --end-date 2026-06-17
...
核心改动：
    - 用 1D regime 过滤 4H 突破方向：多头正常，空头更严格。
    - 加仓按 R 推进，而不是按 ATR 推进。
    - 到 1R 推保本，到 2R 锁 0.5R，到 3R 锁 1R，减少浮盈变亏损。
    - 默认单层风险提高到 0.40%，最多 3 层；仍然受 max_total_notional_mult 约束。

反偷看：
    - Donchian entry/exit 通道全部 shift(1)，不包含当前 K。
    - 1D regime 使用日线收盘后再 shift(1) 映射到 4H。
...
STRATEGY_NAME = "eth_4h_turtle_pyramid_v2"
REPORT_STRATEGY_NAME = "ETH_4H_TurtlePyramid_V2_LF"
...
class StrategyConfig:
    ...
    # 4H breakout / exit
    entry_lookback: int = 96             # 16 days on 4H bars
    exit_lookback: int = 48              # 8 days on 4H bars
    atr_period: int = 20
    adx_period: int = 14
    min_adx_long: float = 16.0
    min_adx_short: float = 22.0
    min_atr_pct: float = 0.0035
    max_atr_pct: float = 0.0800
    ema_filter_4h: int = 200

    # 1D regime filter
    d1_ema_fast: int = 50
    d1_ema_slow: int = 100
    d1_slope_lookback: int = 10
    short_slope_max: float = -0.0030     # stricter short regime

    # Risk / stop / pyramid
    initial_atr_mult: float = 2.2
    trailing_atr_mult: float = 3.4
    unit_risk_per_trade: float = 0.0040  # each unit risks 0.40% before protection
    max_units: int = 3
    add_every_r: float = 1.0
    max_total_notional_mult: float = 4.0
    ...
    # Anti-chop
    no_progress_bars: int = 18           # 3 days
    no_progress_min_r: float = 0.50
    max_hold_bars: int = 180             # 30 days
    cooldown_bars: int = 4
...
def build_features(df: pd.DataFrame, cfg: StrategyConfig) -> pd.DataFrame:
    ...
    out["ema_filter_4h"] = ema(out["close"], cfg.ema_filter_4h)
    ...
    long_filter = (
        ...
        & (out["close"] > out["ema_filter_4h"])
        & (out["adx"] >= cfg.min_adx_long)
        & out["atr_ok"]
    )
    short_filter = (
        out["d1_bear"].astype("boolean").fillna(False).astype(bool)
        & (out["close"] < out["ema_filter_4h"])
        & (out["adx"] >= cfg.min_adx_short)
        & out["atr_ok"]
        & cfg.enable_short
    )

    out["long_signal"] = (out["close"] > out["entry_high"]) & long_filter
    out["short_signal"] = (out["close"] < out["entry_low"]) & short_filter
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        "open", "high", "low", "close", "volume", "atr", "atr_pct", "atr_ok", "adx", "ema_filter_4h",
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH 4H Turtle Pyramid V2 Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 4H Turtle Pyramid V2 with 1D regime and R-lock protection.")
    p.add_argument("--symbol", default="ETH-USDT-SWAP")
    p.add_argument("--start-date", default="2023-01-01")
    p.add_argument("--end-date", default=today)
    p.add_argument("--initial-capital", type=float, default=1000.0)
    p.add_argument("--unit-risk-per-trade", type=float, default=0.004)
    p.add_argument("--max-total-notional-mult", type=float, default=4.0)
    p.add_argument("--fee-rate", type=float, default=0.0005)
    p.add_argument("--slippage-pct", type=float, default=0.0002)
    p.add_argument("--entry-lookback", type=int, default=96)
    p.add_argument("--exit-lookback", type=int, default=48)
    p.add_argument("--max-units", type=int, default=3)
    p.add_argument("--disable-short", action="store_true")
    p.add_argument("--out-dir", default="data/reports/lf/eth_4h_turtle_pyramid_v2")
    return p.parse_args()
