...
# -*- coding: utf-8 -*-
"""
ETH 1D + 4H Trend Rider V8 Position Lock Backtest
====================================

放置位置建议：backtest/lf/eth_1d_4h_trend_rider_v8_position_lock_backtest.py
运行示例：
    python backtest/lf/eth_1d_4h_trend_rider_v8_position_lock_backtest.py --start-date 2023-01-01 --end-date 2026-06-15

策略定位：
    低频趋势骑乘 V8。
    在 V7 SAFE Profit Lock 基础上增加持仓均价保护：
        1) 1D 快速趋势 regime 判断大方向；
        2) 4H 顺势突破/趋势延续入场；
        3) 多空都允许，但空头门槛更高；
        4) 趋势强、ADX 高、EMA 扩张时自动提高单层风险；
        5) 震荡、高波动、日线斜率弱时自动降低单层风险；
        6) 保留 R 保护、顺势加仓、趋势止损。
...
STRATEGY_NAME = "eth_1d_4h_trend_rider_v8_position_lock"
REPORT_STRATEGY_NAME = "ETH_1D_4H_TrendRider_V8_PositionLock_LF"

PRESETS: dict[str, dict[str, float | int]] = {
    # 低风险研究：每年正收益，回撤约 10%。
    "stable": {"unit_risk_per_trade": 0.008, "max_total_notional_mult": 6.0, "max_units": 4, "max_risk_mult": 2.0},
    # 默认版：收益/回撤比较均衡。
    "high": {"unit_risk_per_trade": 0.015, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3},
    # 高收益研究版：每年正收益，回撤进入 25%-30% 区间。
    "turbo": {"unit_risk_per_trade": 0.022, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3},
    # 极限压力测试：仍然每年正收益，但回撤较高。
    "ultra": {"unit_risk_per_trade": 0.030, "max_total_notional_mult": 11.0, "max_units": 5, "max_risk_mult": 2.3},
}
...
class StrategyConfig:
    ...
    # Dynamic risk multiplier. Effective unit risk = unit_risk_per_trade * risk_mult.
...
def build_features(df: pd.DataFrame, cfg: StrategyConfig) -> pd.DataFrame:
    ...
    # Dynamic risk is intentionally NOT "higher ADX = always higher risk".
    # In ETH 4H data, extremely stretched ADX / EMA distance often appears near late trend exhaustion.
    # The best zone is moderate trend strength with enough direction but not too much extension.
    out["risk_mult"] = 0.85
    moderate_adx = out["adx"].between(14.0, 30.0)
    moderate_slope = d1_slope_abs.between(0.004, 0.060)
    moderate_distance = price_distance.between(0.010, 0.085)
    moderate_spread = ema_spread.between(0.004, 0.025)
    out.loc[moderate_adx, "risk_mult"] += 0.25
    out.loc[moderate_slope, "risk_mult"] += 0.20
    out.loc[moderate_distance, "risk_mult"] += 0.20
    out.loc[moderate_spread, "risk_mult"] += 0.15

    out.loc[out["adx"] < cfg.weak_adx, "risk_mult"] -= 0.25
    ...
    # Historical bucket risk allocation: do not remove many trades; resize them.
...
def unit_qty(capital: float, entry_price: float, stop_dist: float, current_qty: float, cfg: StrategyConfig, risk_mult: float = 1.0) -> float:
    if stop_dist <= 0:
        return 0.0
...
def protected_stop(first_entry: float, avg_entry: float, side: int, risk_per_coin: float, max_fav: float, cfg: StrategyConfig) -> float | None:
    if risk_per_coin <= 0:
        return None
    ...
    for i in range(len(rows) - 1):
        ...
        if in_pos:
            ...
            if side == 1:
                ...
                # Safe no-lookahead execution: current bar can only touch the stop that existed
                # before this bar. Stop updates based on this bar close become active next bar.
...
def summarize(trades: list[dict[str, Any]], equity: pd.DataFrame, initial_capital: float) -> dict[str, Any]:
    ...
    return {
        ...
        "mfe_ge_1r_ended_loss": int(((tdf["mfe_r"] >= 1.0) & (tdf["pnl"] <= 0)).sum()),
        "mfe_ge_2r_ended_loss": int(((tdf["mfe_r"] >= 2.0) & (tdf["pnl"] <= 0)).sum()),
        "donchian_exit_trades": int((tdf["note"] == "DONCHIAN_EXIT").sum()) if "note" in tdf.columns else 0,
        "donchian_exit_pnl": round(float(tdf.loc[tdf["note"] == "DONCHIAN_EXIT", "pnl"].sum()), 4) if "note" in tdf.columns else 0.0,
    }
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "entry_high", "entry_low", "exit_high", "exit_low", "risk_mult", "quality_mult", "long_breakout_setup", "short_breakout_setup", "long_signal", "short_signal", "signal",
    ]
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH 1D+4H Trend Rider V8 Position Lock Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 1D+4H Trend Rider V8 position-lock version with quality risk sizing and safe no-lookahead execution.")
    p.add_argument("--symbol", default="ETH-USDT-SWAP")
    p.add_argument("--start-date", default="2023-01-01")
    p.add_argument("--end-date", default=today)
    p.add_argument("--initial-capital", type=float, default=1000.0)
    p.add_argument("--preset", choices=sorted(PRESETS), default="high", help="stable=低风险, high=默认高收益, turbo=高风险高收益, ultra=极限压力测试")
    # 下面三个参数默认 None，表示使用 preset；显式传入时覆盖 preset。
    ...
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_trend_rider_v8_position_lock")
    return p.parse_args()
...
def main() -> int:
    ...
    cfg = StrategyConfig(
        ...
        max_total_notional_mult=max_total_notional_mult,
        min_risk_mult=args.min_risk_mult,
        max_risk_mult=max_risk_mult,
    ...
    print(f"Preset: {args.preset} | unit_risk={cfg.unit_risk_per_trade} | max_notional={cfg.max_total_notional_mult}x | max_units={cfg.max_units} | max_risk_mult={cfg.max_risk_mult}")
    ...
    summary["unit_risk_per_trade"] = cfg.unit_risk_per_trade
    summary["max_total_notional_mult"] = cfg.max_total_notional_mult
    summary["max_units_cfg"] = cfg.max_units
    summary["max_risk_mult_cfg"] = cfg.max_risk_mult
