...
# -*- coding: utf-8 -*-
"""
ETH 1D+4H Bull Range Reclaim V2
================================
...
STRATEGY_NAME = "eth_1d_4h_bull_range_reclaim_v2"
REPORT_STRATEGY_NAME = "ETH_1D_4H_BullRangeReclaim_V2"
...
def build_features(base_4h: pd.DataFrame, cfg: BullRangeConfig) -> pd.DataFrame:
    ...
    # V2: add a broad bull-cycle filter. This is not a year filter; it avoids weak bear-market rallies
    # where the daily EMA50 is still materially below EMA100.
    out["macro_bull_ok"] = out["daily_ok"] & ((out["d1_ema_mid"] / out["d1_ema_slow"]) > cfg.d1_mid_vs_slow_min)

    # A bucket: V1 high-quality range reclaim.
    out["quality_bucket_a"] = out["macro_bull_ok"] & out["recent_pullback"] & out["reclaim"] & out["range_ok"] & out["volume_ok"] & out["not_extended"]

    # B bucket: secondary reclaim. Adds more trades but receives lower quality_mult.
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "ema20", "ema50", "ema100", "d1_ema_fast", "d1_ema_mid", "d1_ema_slow", "d1_mid_slope", "d1_dist_mid", "d1_not_bear", "macro_bull_ok",
        "pb_min_dist50", "pb_min_dist100", "recent_pullback", "reclaim", "secondary_reclaim", "range_ok", "volume_ok", "not_extended", "quality_bucket_a", "quality_bucket_b",
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 94)
    print("ETH 1D+4H Bull Range Reclaim V2 Backtest Summary")
    print("=" * 94)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 1D+4H Bull Range Reclaim V2: two-bucket long-only bull range reclaim supplement engine.")
