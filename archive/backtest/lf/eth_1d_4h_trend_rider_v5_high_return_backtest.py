...
# -*- coding: utf-8 -*-
"""
ETH 1D + 4H Trend Rider V5 High Return Backtest
====================================

放置位置建议：backtest/lf/eth_1d_4h_trend_rider_v5_high_return_backtest.py
运行示例：
    python backtest/lf/eth_1d_4h_trend_rider_v5_high_return_backtest.py --start-date 2023-01-01 --end-date 2026-06-15

策略定位：
    低频趋势骑乘 V5。
    在 V4 动态风险基础上新增高收益 preset 和交易质量诊断：
...
STRATEGY_NAME = "eth_1d_4h_trend_rider_v5_high_return"
REPORT_STRATEGY_NAME = "ETH_1D_4H_TrendRider_V5_HighReturn_LF"

PRESETS: dict[str, dict[str, float | int]] = {
    # 保守版：接近 V4 默认，回撤更低。
    "stable": {"unit_risk_per_trade": 0.006, "max_total_notional_mult": 5.0, "max_units": 3},
    # 默认版：当前收益/回撤比较均衡。
    "balanced": {"unit_risk_per_trade": 0.008, "max_total_notional_mult": 6.0, "max_units": 4},
    # 激进版：收益更高，回撤也明显更高。
    "aggressive": {"unit_risk_per_trade": 0.010, "max_total_notional_mult": 6.0, "max_units": 4},
    # 压力测试版：只建议研究，不建议直接实盘。
    "max": {"unit_risk_per_trade": 0.012, "max_total_notional_mult": 7.0, "max_units": 4},
}
    ...
    for i in range(len(rows) - 1):
        ...
        if in_pos:
            ...
            if exit_now:
                ...
                if add_triggered:
                    ...
                    add_q = unit_qty(capital, add_price, add_stop_dist, qty, cfg, float(getattr(row, "risk_mult", entry_risk_mult)))
        ...
        if not in_pos and i - last_exit_i >= cfg.cooldown_bars:
            ...
            if signal != 0:
                ...
                entry_risk_mult = float(getattr(row, "risk_mult", 1.0))
...
def write_outputs(trades: list[dict[str, Any]], equity: pd.DataFrame, features: pd.DataFrame, summary: dict[str, Any], out_dir: Path) -> None:
    ...
    audit_cols = [
        ...
        "entry_high", "entry_low", "exit_high", "exit_low", "risk_mult", "long_signal", "short_signal", "signal",
    ]
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH 1D+4H Trend Rider V5 High Return Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 1D+4H Trend Rider V5 high-return preset version with dynamic risk and diagnostics.")
    p.add_argument("--symbol", default="ETH-USDT-SWAP")
    p.add_argument("--start-date", default="2023-01-01")
    p.add_argument("--end-date", default=today)
    p.add_argument("--initial-capital", type=float, default=1000.0)
    p.add_argument("--preset", choices=sorted(PRESETS), default="balanced", help="stable=低回撤, balanced=默认高收益, aggressive=更高收益, max=压力测试")
    ...
    p.add_argument("--max-risk-mult", type=float, default=1.50)
    ...
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_trend_rider_v5_high_return")
    return p.parse_args()
...
def main() -> int:
    ...
    cfg = StrategyConfig(
        ...
        max_risk_mult=args.max_risk_mult,
    ...
    print(f"Preset: {args.preset} | unit_risk={cfg.unit_risk_per_trade} | max_notional={cfg.max_total_notional_mult}x | max_units={cfg.max_units}")
