...
# -*- coding: utf-8 -*-
"""
ETH 1D + 4H Trend Rider V6 Quality Risk Backtest
====================================

放置位置建议：backtest/lf/eth_1d_4h_trend_rider_v6_quality_risk_safe_backtest.py
运行示例：
    python backtest/lf/eth_1d_4h_trend_rider_v6_quality_risk_safe_backtest.py --start-date 2023-01-01 --end-date 2026-06-15

策略定位：
    低频趋势骑乘 V6。
    在 V5 高收益版本基础上新增交易质量 risk model：
...
STRATEGY_NAME = "eth_1d_4h_trend_rider_v6_quality_risk_safe"
REPORT_STRATEGY_NAME = "ETH_1D_4H_TrendRider_V6_QualityRisk_Safe_LF"

PRESETS: dict[str, dict[str, float | int]] = {
    # 低风险研究：比 V5 balanced 更好的 PF/回撤结构。
    "stable": {"unit_risk_per_trade": 0.008, "max_total_notional_mult": 6.0, "max_units": 4, "max_risk_mult": 2.0},
    # 默认版：高收益但还没有进入极端回撤区。
    "high": {"unit_risk_per_trade": 0.015, "max_total_notional_mult": 10.0, "max_units": 5, "max_risk_mult": 2.2},
    # 高风险研究版：收益明显更高，回撤也明显更高。
    "turbo": {"unit_risk_per_trade": 0.025, "max_total_notional_mult": 15.0, "max_units": 6, "max_risk_mult": 2.5},
    # 极限压力测试：不建议直接实盘。
    "ultra": {"unit_risk_per_trade": 0.040, "max_total_notional_mult": 22.0, "max_units": 8, "max_risk_mult": 3.0},
}
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 86)
    print("ETH 1D+4H Trend Rider V6 Quality Risk SAFE Backtest Summary")
    print("=" * 86)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH 1D+4H Trend Rider V6 quality-risk version with bucket-based risk sizing and high-return presets.")
    ...
    p.add_argument("--out-dir", default="data/reports/lf/eth_1d_4h_trend_rider_v6_quality_risk_safe")
    return p.parse_args()
