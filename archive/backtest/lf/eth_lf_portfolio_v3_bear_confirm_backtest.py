...
# -*- coding: utf-8 -*-
"""
ETH LF Portfolio V3 Bear Confirm Backtest
=========================================

放置位置建议：backtest/lf/eth_lf_portfolio_v3_bear_confirm_backtest.py
运行示例：
    python backtest/lf/eth_lf_portfolio_v3_bear_confirm_backtest.py --start-date 2023-01-01 --end-date 2026-06-15 --preset turbo

组合定位：
    低频组合调度器 V3。V2 证明 Turtle 更适合作为确认器；V3 进一步接入 Bear Short Engine V3：
        1) 同一时刻只允许一个组合持仓；
...
STRATEGY_NAME = "eth_lf_portfolio_v3_bear_confirm"
REPORT_STRATEGY_NAME = "ETH_LF_Portfolio_V3_BearConfirm"
...
def print_summary(summary: dict[str, Any], out_dir: Path) -> None:
    print("\n" + "=" * 88)
    print("ETH LF Portfolio V3 Bear Confirm Backtest Summary")
    print("=" * 88)
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="ETH LF Portfolio V3: V8 main + Turtle confirm + Bear V3 short confirm/standalone.")
    ...
    p.add_argument("--reduce-nonbear-short-quality", type=float, default=1.0, help="防守模式：将 Bear 未确认的 V8 short 质量乘以该系数，例如 0.85。默认不削弱。")
    p.add_argument("--quality-mult-cap", type=float, default=2.20)

    p.add_argument("--out-dir", default="data/reports/lf/eth_lf_portfolio_v3_bear_confirm")
    return p.parse_args()
