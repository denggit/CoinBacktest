...
V3 replaces same-bar condition intersections with causal ordered processes that
have stage timeouts, expiry and sample-supported historical probabilities.  It
...
DEFAULT_OUT_DIR = Path("data/reports/research/market_state/03_market_state_process_probability_v3")
...
def process_profile_config(name: str, minimum_probability_samples: int) -> ProcessMapConfig:
    base = ProcessMapConfig(semantic_version="v3", minimum_probability_samples=int(minimum_probability_samples))
    if name == "base":
        return base
    if name == "fast":
        return replace(
            ...
            breakout_compression_to_impulse_bars=60,
            breakout_impulse_to_accept_bars=20,
            breakout_completed_ttl_bars=20,
        )
    if name == "slow":
        return replace(
            base,
            ...
            breakout_compression_to_impulse_bars=120,
            breakout_impulse_to_accept_bars=45,
            breakout_completed_ttl_bars=45,
        )
    raise ValueError(f"unknown profile: {name}")
...
def write_executive_summary(
    ...
    lines = [
        ...
        f"Reason: {reason}",
        "",
        "V3 is a market-state process audit, not a standalone strategy backtest.",
...
def run(args: argparse.Namespace) -> Path:
    ...
    if stage_summary.empty:
        raise RuntimeError("No eligible V3 process stage evidence was produced.")
    ...
    registry = build_process_registry(
        ...
        progression,
        config=evaluation_config,
        process_config=ProcessMapConfig(minimum_probability_samples=args.minimum_probability_samples),
    )
    _finish(started)
    ...
    metadata = {
        "title": "Market State Process & Conditional Probability Map V3",
        "experiment_id": "MARKET_STATE_PROCESS_PROBABILITY_V3",
        ...
        "warnings": [
            "V3 is a market-state process audit, not a standalone strategy.",
    ...
    if (
        ...
        if trades:
            ...
            print_full_report(
                ...
                1000.0,
                final_capital,
                "MARKET_STATE_V3_NAIVE_DIAGNOSTIC_NOT_STRATEGY",
                total_days,
                False,
                symbol=args.symbol,
