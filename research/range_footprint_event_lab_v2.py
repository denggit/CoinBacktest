#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Range-bar + footprint event feature lab for ETH OKX research.
...
SCRIPT_NAME = "RangeFootprintEventLabV2"
...
class Config:
    ...
    out_dir: Path = Path("data/reports/research/range_footprint_event_lab_v2")
...
def build_feature_binning(events: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    if events.empty:
        return pd.DataFrame()
    rows: list[dict[str, Any]] = []
    for (event_type, side), group in events.groupby(["event_type", "side"], dropna=False):
        for feature in FEATURE_COLUMNS:
...
def build_feature_correlations(events: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    for (event_type, side), group in data.groupby(["event_type", "side"], dropna=False):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default="data/reports/research/range_footprint_event_lab_v2")
...
def main() -> None:
    ...
    events_path = cfg.out_dir / "range_footprint_event_lab_v2_events.csv"
    summary_path = cfg.out_dir / "range_footprint_event_lab_v2_summary.csv"
    yearly_path = cfg.out_dir / "range_footprint_event_lab_v2_yearly.csv"
    binning_path = cfg.out_dir / "range_footprint_event_lab_v2_feature_binning.csv"
    corr_path = cfg.out_dir / "range_footprint_event_lab_v2_feature_correlations.csv"
    config_path = cfg.out_dir / "range_footprint_event_lab_v2_config.json"
    ...
    if not feature_binning.empty:
        ...
        cols2 = ["event_type", "side", "feature", "bin", "events", "hit_2r_before_stop_pct", "net_exp_2r_stop_r", "avg_mfe_r", "avg_mae_r"]
        print(feature_binning[cols2].head(30).to_string(index=False))
    if not feature_corr.empty:
        print("\n=== Top Feature Correlations ===")
        cols3 = ["event_type", "side", "feature", "target", "spearman_corr", "n"]
