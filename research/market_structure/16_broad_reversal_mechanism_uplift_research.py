...
def _fit_expert_scores(
    *,
    ...
    # Pandas/NumPy quantile cannot subtract native boolean arrays.  Real G1
    # data contains ``is_macro_first_sweep`` as bool, while the synthetic
    # model test previously generated it as float and therefore missed this
    # path.  Normalize every requested model feature to float consistently
    # across fit/policy/test before feature conditioning and model scoring.
...
def run_research(args: argparse.Namespace) -> Path:
    print(f"[version] {SCRIPT_NAME} v{SCRIPT_VERSION}", flush=True)
