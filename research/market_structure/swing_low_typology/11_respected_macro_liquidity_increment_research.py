#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Respected macro-liquidity increment research 11.

Research 11 corrects the liquidity definition tested in 10.  It excludes 1m
and generic rolling lows.  A level becomes eligible only after at least two
causally confirmed 15m/1H/4H pivot lows are respected near the same price,
with at least one respect coming from 1H or 4H under the default configuration.
Cross-timeframe detections of the same low count as one respect, not two.
The second confirmation makes the level available; no feature is backfilled.

Q0 : causal 1m trade-bar snapshot + train-fitted soft mechanisms
Q1 : Q0 + active respected macro-level context
Q2 : Q1 + first-sweep / short-window reclaim / acceptance state
Q3 : Q2 + observable aggressive-sell and post-sweep order-flow process

Liquidity remains optional.  The identical broad candidate universe is used for
all groups.  No strategy, sizing, stop, scale-in or backtest is included.
"""
...
from research.market_structure.swing_low_typology.common.respected_macro_liquidity import (  # noqa: E402
    CONTEXT_GROUP,
    EVENT_GROUP,
    ORDERFLOW_GROUP,
    build_respected_macro_liquidity_features,
)
...
SCRIPT_NAME = "11_respected_macro_liquidity_increment_research"
SCRIPT_VERSION = "1.0.0"
EXPERIMENT_ID = "ETH_1M_RESPECTED_MACRO_LIQUIDITY_INCREMENT_11"
EDGE_ID = "RESEARCH_ONLY_ETH_RESPECTED_MACRO_LIQUIDITY"
TITLE = "ETH Respected Macro Liquidity Increment Research 11"
DEFAULT_OUT_DIR = "data/reports/research/market_structure/swing_low_typology/11_respected_macro_liquidity_increment"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Walk-forward respected macro-liquidity increment research.",
    ...
    p.add_argument("--liquidity-approach-tolerance-bp", type=float, default=35.0)
...
def _increment_comparison(frontier: pd.DataFrame) -> pd.DataFrame:
    ...
    baseline = frontier[frontier["feature_group"].eq("Q0_tradebar")].copy()
    baseline = baseline.set_index(keys)
    rows: list[dict[str, object]] = []
    for row in frontier[~frontier["feature_group"].eq("Q0_tradebar")].itertuples(index=False):
...
def _respected_state_outcomes(frame: pd.DataFrame, folds: Sequence[FoldSpec]) -> pd.DataFrame:
    """Describe liquidity states without selecting or optimizing a policy."""

    definitions = {
        "no_active_level": lambda x: pd.to_numeric(x["rml_has_active_level"], errors="coerce").fillna(0).eq(0),
        "active_respected_level": lambda x: pd.to_numeric(x["rml_has_active_level"], errors="coerce").fillna(0).gt(0),
        "approach_respected_level": lambda x: pd.to_numeric(x["rml_approach_active_level"], errors="coerce").fillna(0).gt(0),
        "first_sweep": lambda x: pd.to_numeric(x["rml_first_sweep"], errors="coerce").fillna(0).gt(0),
        "sweep_reclaim": lambda x: pd.to_numeric(x["rml_reclaim"], errors="coerce").fillna(0).gt(0),
        "sweep_accept_below": lambda x: pd.to_numeric(x["rml_accept_below"], errors="coerce").fillna(0).gt(0),
    }
    rows: list[dict[str, object]] = []
    for fold in folds:
        test, _ = _subset_period(frame, fold.test_start, fold.test_end)
        for state, builder in definitions.items():
            subset = test.loc[builder(test)].copy()
            rows.append(
                {
                    "fold": fold.fold,
                    "state": state,
                    "candidate_rows": int(len(subset)),
                    "causal_regions": int(subset["causal_region_id"].nunique()) if not subset.empty else 0,
                    "tp_rate": float(subset["tp_hit_1pct"].mean()) if not subset.empty else np.nan,
                    "clean_0p50_rate": float(subset["tp_before_adverse_0p5pct"].mean()) if not subset.empty else np.nan,
                    "fast30_rate": float(subset["tp_within_30"].mean()) if not subset.empty else np.nan,
                    "median_horizon_mae_pct": float(pd.to_numeric(subset["mae_horizon_pct"], errors="coerce").median()) if not subset.empty else np.nan,
                }
            )
    return pd.DataFrame(rows)
...
def _raw_future_perturbation_audit(
    ...
    for row in sample.itertuples(index=False):
        ...
        def build_current(source_bars: pd.DataFrame) -> pd.DataFrame:
            ...
            liquidity = build_respected_macro_liquidity_features(
                source_bars,
                region,
...
def _summary(frontier: pd.DataFrame, increments: pd.DataFrame, diagnostics: pd.DataFrame, audit: pd.DataFrame) -> str:
    lines = [
        ...
        "## Purpose",
        "",
        "Research 11 tests a corrected liquidity definition: only repeated, causally confirmed 15m/1H/4H pivot lows can become active macro liquidity.",
        "A level is available only after the second respect is confirmed. Generic 1m lows, prior-day lows and persistent below-level states are excluded.",
        "Liquidity remains optional; Q0/Q1/Q2/Q3 use the identical broad candidate universe and frozen policies.",
        "",
        "## Level diagnostics",
        "",
    ]
    if diagnostics.empty:
        lines.append("No respected-liquidity diagnostics were produced.")
    else:
        aggregate = diagnostics[diagnostics.get("scope", pd.Series(index=diagnostics.index, dtype=object)).eq("aggregate")]
        if not aggregate.empty:
            row = aggregate.iloc[0]
            lines.append(
                f"- active-level candidate coverage={float(row.get('value', np.nan)):.2%}, "
                f"first-sweep candidates={int(row.get('first_sweep_candidates', 0)):,}, "
                f"reclaims={int(row.get('reclaim_candidates', 0)):,}, accepted below={int(row.get('accept_below_candidates', 0)):,}, "
                f"availability violations={int(row.get('availability_violations', 0))}."
            )
        construction = diagnostics[diagnostics.get("scope", pd.Series(index=diagnostics.index, dtype=object)).eq("level_construction")]
        for row in construction.itertuples(index=False):
            lines.append(f"- {row.metric}: {int(row.value):,}")
        lifecycle = diagnostics[diagnostics.get("scope", pd.Series(index=diagnostics.index, dtype=object)).eq("level_lifecycle")]
        if not lifecycle.empty and "status" in lifecycle.columns:
            counts = lifecycle["status"].value_counts()
            lines.append("- lifecycle: " + ", ".join(f"{name}={int(value):,}" for name, value in counts.items()))
    ...
    if increments.empty:
        lines.append("No respected-liquidity increment comparison rows.")
    else:
        ...
        for row in stable.itertuples(index=False):
            lines.append(
                ...
                f"Δfast30={row.mean_delta_fast30:+.2%}, Δmedian MAE={row.mean_delta_mae:+.4f}% across {int(row.folds)} folds."
            )
        lines.extend(
            [
                "",
                "Predeclared adoption rule: do not retain a liquidity group unless it improves clean50 or fast30 in at least two of three folds, "
                "does not materially degrade the remaining fold, survives deletion of strong days, and retains hundreds of events at non-extreme policies.",
            ]
        )
...
def run_research(args: argparse.Namespace) -> Path:
    ...
    print("[stage] causally respected macro liquidity levels and first-sweep process", flush=True)
    liquidity = build_respected_macro_liquidity_features(
        bars,
        frame,
        pivot_minutes=tuple(int(x) for x in args.liquidity_pivot_minutes),
    ...
    liquidity_group_members = {
        ...
        for group in (CONTEXT_GROUP, EVENT_GROUP, ORDERFLOW_GROUP)
    }
    liquidity_feature_groups: dict[str, tuple[str, ...]] = {
        "Q1_respected_macro_context": liquidity_group_members[CONTEXT_GROUP],
        "Q2_first_sweep_reclaim": (
            *liquidity_group_members[CONTEXT_GROUP],
            *liquidity_group_members[EVENT_GROUP],
        ),
        "Q3_sweep_orderflow": (
            *liquidity_group_members[CONTEXT_GROUP],
            *liquidity_group_members[EVENT_GROUP],
            *liquidity_group_members[ORDERFLOW_GROUP],
        ),
    }
    _write_csv(liquidity.dictionary, out_dir / "06_respected_liquidity_feature_dictionary.csv")
    _write_csv(liquidity.diagnostics, out_dir / "07_respected_liquidity_diagnostics.csv")
    _write_csv(liquidity.levels, out_dir / "07b_respected_level_table.csv")
    aggregate_diagnostics = liquidity.diagnostics[liquidity.diagnostics["scope"].eq("aggregate")]
    if aggregate_diagnostics.empty:
        raise RuntimeError("respected liquidity diagnostics missing aggregate row")
    aggregate_row = aggregate_diagnostics.iloc[0]
    if int(aggregate_row.get("availability_violations", 0)) != 0:
        raise RuntimeError("respected liquidity availability audit failed")
    overlapping_levels = int(aggregate_row.get("levels_overlapping_candidate_time", 0))
    valid_bar_windows = int(aggregate_row.get("levels_with_valid_bar_window", 0))
    candidate_overlap_levels = int(aggregate_row.get("levels_with_candidate_overlap", 0))
    active_coverage = float(aggregate_row.get("value", 0.0))
    if overlapping_levels > 0 and valid_bar_windows == 0:
        raise RuntimeError(
            "respected liquidity time alignment failed: levels overlap the candidate period "
            "but none map to a valid 1m bar window"
        )
    if candidate_overlap_levels > 0 and active_coverage <= 0.0:
        raise RuntimeError(
            "respected liquidity feature alignment failed: active level lifecycles overlap "
            "candidate positions but candidate coverage is zero"
        )
    liquidity_varying = [
        column
    ...
    if candidate_overlap_levels > 0 and not liquidity_varying:
        raise RuntimeError(
            "respected liquidity increment is invalid: all rml_* features are constant "
            "despite active level/candidate overlap"
        )

    folds = _folds(args.end_date)
    _write_csv(_respected_state_outcomes(frame, folds), out_dir / "07c_respected_state_outcomes.csv")
    ...
    for fold_index, fold in enumerate(folds, start=1):
        ...
        group_requested: dict[str, tuple[str, ...]] = {"Q0_tradebar": base_requested}
    ...
    _write_csv(increments, out_dir / "20_respected_liquidity_increment_comparison.csv")
    ...
    htf_available_violations = int(aggregate_diagnostics.iloc[0].get("availability_violations", 0))
    audit = pd.DataFrame(
        [
            ...
            {"check": "respected_levels_preexist_current_bar", "passed": True, "detail": "level becomes active only after the second confirmed HTF pivot is available"},
            {"check": "respected_level_available_before_feature", "passed": htf_available_violations == 0, "detail": f"violations={htf_available_violations}"},
            ...
            {"check": "liquidity_not_mandatory_candidate_gate", "passed": True, "detail": "Q0/Q1/Q2/Q3 use the identical broad candidate universe"},
            ...
            {"check": "liquidity_groups_evaluated_independently", "passed": True, "detail": "respected context, first-sweep state and order-flow process are separately reported against Q0"},
    ...
    if not audit["passed"].all():
        raise RuntimeError(f"11 audit failed: {audit.loc[~audit['passed'], 'check'].tolist()}")
    ...
    manifest = {
        ...
        "feature_groups": ["Q0_tradebar", *liquidity_feature_groups],
        "pivot_minutes": [int(x) for x in args.liquidity_pivot_minutes],
        "pivot_weights": [float(x) for x in args.liquidity_pivot_weights],
        "pivot_left_bars": int(args.liquidity_pivot_left_bars),
        "pivot_right_bars": int(args.liquidity_pivot_right_bars),
        "cluster_tolerance_bp": float(args.liquidity_cluster_tolerance_bp),
        "minimum_respects": int(args.liquidity_minimum_respects),
        "minimum_macro_timeframe_min": int(args.liquidity_minimum_macro_timeframe_min),
        "minimum_respect_separation_minutes": int(args.liquidity_minimum_respect_separation_minutes),
        ...
        "liquidity_alignment": "level available only after repeated confirmed HTF pivots; first sweep is a transition event",
