#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Deployable raw-score ranking and range-data increment research 09.

Research 09 fixes the calibration/ranking confusion exposed by 08.  Continuous
raw model scores are converted into frozen empirical percentiles for selection;
calibrated probabilities are retained only for interpretation and future risk
sizing.  Under the same expanding walk-forward folds, training sample and
predeclared policy family, the script compares:

R0  : causal 1m trade-bar snapshot + train-fitted soft mechanisms
R1x : R0 + one closed range-bar scale
R2x : R1x + footprint aggregates for the same range scale

No strategy, position sizing, stop, scale-in or backtest is implemented here.
"""
...
SCRIPT_NAME = "09_deployable_score_range_increment_research"
SCRIPT_VERSION = "1.0.1"
EXPERIMENT_ID = "ETH_1M_DEPLOYABLE_SCORE_RANGE_INCREMENT_09"
EDGE_ID = "RESEARCH_ONLY_ETH_RANGE_INCREMENT_REVERSAL"
TITLE = "ETH Deployable Score and Range Increment Research 09"
DEFAULT_OUT_DIR = "data/reports/research/market_structure/swing_low_typology/09_deployable_score_range_increment"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Walk-forward deployable score and range-data increment research.",
    ...
    p.add_argument("--range-footprints-db-name", default="okx_range_footprints.db")
    p.add_argument("--range-pcts", nargs="+", type=float, default=[0.0015, 0.0020, 0.0025])
    p.add_argument("--range-price-step", type=float, default=1.0)
    p.add_argument("--minimum-range-coverage", type=float, default=0.98)
    p.add_argument("--skip-range-footprint", action="store_true")
...
def _validate_range_coverage(coverage: pd.DataFrame, *, minimum: float, include_footprint: bool) -> None:
    if not 0.0 < float(minimum) <= 1.0:
        raise ValueError("--minimum-range-coverage must be in (0, 1]")
    required = {"range_code", "aligned_coverage"}
    if include_footprint:
        required.add("footprint_to_range_bar_coverage")
    missing = sorted(required.difference(coverage.columns))
    if missing:
        raise RuntimeError(f"range coverage diagnostics missing columns: {missing}")
    aligned = pd.to_numeric(coverage["aligned_coverage"], errors="coerce")
    if aligned.isna().any() or (aligned < float(minimum)).any():
        bad = coverage.loc[aligned.isna() | (aligned < float(minimum)), ["range_code", "aligned_coverage"]]
        raise RuntimeError(f"range candidate coverage below {minimum:.1%}: {bad.to_dict(orient='records')}")
    if include_footprint:
        footprint = pd.to_numeric(coverage["footprint_to_range_bar_coverage"], errors="coerce")
        if footprint.isna().any() or (footprint < float(minimum)).any():
            bad = coverage.loc[footprint.isna() | (footprint < float(minimum)), ["range_code", "footprint_to_range_bar_coverage"]]
            raise RuntimeError(f"range footprint coverage below {minimum:.1%}: {bad.to_dict(orient='records')}")
...
def _increment_comparison(frontier: pd.DataFrame) -> pd.DataFrame:
    ...
    baseline = frontier[frontier["feature_group"].eq("R0_tradebar")].copy()
    baseline = baseline.set_index(keys)
    rows: list[dict[str, object]] = []
    for row in frontier[~frontier["feature_group"].eq("R0_tradebar")].itertuples(index=False):
    ...
    for row in sample.itertuples(index=False):
        ...
        def build_current(source_bars: pd.DataFrame) -> pd.DataFrame:
            ...
            region = build_broad_candidate_regions(
                source_bars, feature, max_gap_bars=int(args.region_max_gap_bars),
                max_region_bars=int(args.region_max_bars), retest_tolerance_bp=float(args.region_retest_tolerance_bp),
                show_progress=False,
            ).frame
...
def _summary(frontier: pd.DataFrame, increments: pd.DataFrame, coverage: pd.DataFrame, audit: pd.DataFrame) -> str:
    lines = [
        "# Research 09 Summary",
        "",
        "## Purpose",
        "",
        "Continuous raw scores are used for frozen empirical-percentile ranking. Calibrated probabilities are interpretation outputs only.",
        "Range scales are evaluated independently under identical folds, samples, targets and predeclared policies.",
        "",
        "## Data coverage",
        "",
    ]
    if coverage.empty:
        lines.append("No range coverage rows were produced.")
    else:
        for row in coverage.itertuples(index=False):
            lines.append(f"- {row.range_code}: range bars={int(row.range_bar_rows):,}, footprint bars={int(row.footprint_bar_rows):,}, candidate coverage={row.aligned_coverage:.2%}, availability violations={int(row.available_time_violations)}")
    ...
    if frontier.empty:
        ...
        for row in representative.sort_values(["fold", "policy_id", "feature_group"]).itertuples(index=False):
            lines.append(f"- {row.fold} {row.feature_group} {row.policy_id}: events={int(row.event_count):,}, TP={row.tp_rate:.2%}, clean50={row.clean_0p50_rate:.2%}, fast30={row.fast30_rate:.2%}, median horizon MAE={row.median_horizon_mae_pct:.3f}%")
    lines.extend(["", "## Increment interpretation", ""])
    if increments.empty:
        lines.append("No increment comparison rows.")
    else:
        ...
        for row in stable.itertuples(index=False):
            lines.append(f"- {row.feature_group}: mean ΔTP={row.mean_delta_tp:+.2%}, Δclean50={row.mean_delta_clean50:+.2%}, Δfast30={row.mean_delta_fast30:+.2%}, Δmedian MAE={row.mean_delta_mae:+.4f}% across {int(row.folds)} folds/policy rows.")
    ...
    lines.extend(
        [
            "",
            "This remains model research. No policy is automatically selected from frozen test results, and no strategy/backtest/position-sizing rule is claimed.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def run_research(args: argparse.Namespace) -> Path:
    ...
    range_feature_groups: dict[str, tuple[str, ...]] = {}
    range_dictionary_parts: list[pd.DataFrame] = []
    range_coverage_parts: list[pd.DataFrame] = []
    range_end_columns: list[str] = []
    range_attach_parts: list[pd.DataFrame] = []
    print("[stage] local closed range-bar / footprint features", flush=True)
    for range_pct in tuple(float(x) for x in args.range_pcts):
        aligned, rb_columns, fp_columns, dictionary, diagnostics = load_and_align_range_features(
            frame,
            symbol=args.symbol,
            range_pct=range_pct,
            price_step=float(args.range_price_step),
            data_dir=args.data_dir,
            range_bar_db_name=args.range_bars_db_name,
            footprint_db_name=args.range_footprints_db_name,
            start_date=pd.Timestamp(args.warmup_start_date),
            end_date=_end_exclusive(args.end_date),
            include_footprint=not bool(args.skip_range_footprint),
            show_progress=True,
        )
        code = str(diagnostics.iloc[0]["range_code"])
        if not aligned["event_id"].equals(frame["event_id"]):
            raise RuntimeError(f"{code} range alignment changed candidate order")
        attach_data: dict[str, object] = {
            column: pd.to_numeric(aligned[column], errors="coerce").to_numpy(dtype=np.float32)
            for column in (*rb_columns, *fp_columns)
        }
        end_column = f"{code}_used_range_end_ts"
        attach_data[end_column] = pd.to_datetime(aligned["range_end_ts"]).to_numpy()
        range_attach_parts.append(pd.DataFrame(attach_data))
        range_end_columns.append(end_column)
        range_feature_groups[f"R1_{code}_rangebar"] = tuple(rb_columns)
        if fp_columns:
            range_feature_groups[f"R2_{code}_rangefootprint"] = (*rb_columns, *fp_columns)
        range_dictionary_parts.append(dictionary)
        range_coverage_parts.append(diagnostics)
        del aligned
        gc.collect()
    if range_attach_parts:
        frame = pd.concat([frame.reset_index(drop=True), *range_attach_parts], axis=1)
        del range_attach_parts
        gc.collect()
    range_dictionary = pd.concat(range_dictionary_parts, ignore_index=True) if range_dictionary_parts else pd.DataFrame()
    range_coverage = pd.concat(range_coverage_parts, ignore_index=True) if range_coverage_parts else pd.DataFrame()
    _write_csv(range_dictionary, out_dir / "06_range_feature_dictionary.csv")
    _write_csv(range_coverage, out_dir / "07_range_source_coverage.csv")
    minimum_coverage = float(args.minimum_range_coverage)
    ...
    for fold_index, fold in enumerate(folds, start=1):
        ...
        group_requested: dict[str, tuple[str, ...]] = {"R0_tradebar": base_requested}
        for group, extra in range_feature_groups.items():
    ...
    _write_csv(increments, out_dir / "20_range_increment_comparison.csv")
    ...
    for column in range_end_columns:
        availability_violations += int((pd.to_datetime(frame[column]) > feature_available).fillna(False).sum())
    ...
    raw_future_audit = _raw_future_perturbation_audit(bars, frame, m0_features, args)
    ...
    audit = pd.DataFrame(
        [
            ...
            {"check": "range_end_available_before_feature", "passed": availability_violations == 0, "detail": f"violations={availability_violations}"},
            ...
            {"check": "range_cache_read_only", "passed": True, "detail": "09 uses local load_local_data and SQLite aggregation; it never builds missing range caches"},
            ...
            {"check": "range_scales_evaluated_independently", "passed": True, "detail": "r0015/r0020/r0025 are separate R1/R2 feature groups; no multiscale stacking"},
    ...
    if not audit["passed"].all():
        raise RuntimeError(f"09 audit failed: {audit.loc[~audit['passed'], 'check'].tolist()}")
    ...
    manifest = {
        ...
        "feature_groups": ["R0_tradebar", *range_feature_groups],
        "range_pcts": [float(x) for x in args.range_pcts],
        "range_price_step": float(args.range_price_step),
        "minimum_range_coverage": float(args.minimum_range_coverage),
        "range_footprint_included": not bool(args.skip_range_footprint),
        "range_alignment": "range_bar.end_ts <= current_1m_feature_available_time",
    ...
    (out_dir / "27_RESEARCH_SUMMARY.md").write_text(_summary(test_frontier, increments, range_coverage, audit), encoding="utf-8")
