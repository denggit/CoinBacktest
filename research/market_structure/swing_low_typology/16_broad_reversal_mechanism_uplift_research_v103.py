...
SCRIPT_NAME = "16_broad_reversal_mechanism_uplift_research_v103"
...
def _condition_feature_columns(
    ...
    *,
    ...
    """Condition model features on a homogeneous float64 matrix.

    This implementation intentionally lives inside Research 16 instead of
    delegating to Research 12.  That makes Research 16 safe even when an older
    Research 12 file remains installed.  Native bool and pandas nullable
    boolean columns are converted before median, quantile, standard deviation,
    and correlation operations.
    """

    usable = tuple(_R12.select_usable_features(fit, requested, max_missing_ratio=0.40))
    ...
    for column in candidates:
        ...
        converted[column] = series.to_numpy(dtype=np.float64, na_value=np.nan)
    numeric = pd.DataFrame(converted, index=sample.index, dtype=np.float64)
    numeric = numeric.replace([np.inf, -np.inf], np.nan)
    numeric = numeric.fillna(numeric.median().fillna(0.0))

    # NumPy nanquantile runs on one guaranteed float64 ndarray.  This avoids
    # pandas block-wise quantile dispatch retaining a boolean extension block.
    matrix = numeric.to_numpy(dtype=np.float64, copy=False)
    q10 = np.nanquantile(matrix, 0.10, axis=0)
    q90 = np.nanquantile(matrix, 0.90, axis=0)
    robust_span = np.abs(q90 - q10)
    scale_mask = np.isfinite(robust_span) & (robust_span > 1e-10)
    scale_keep = [column for column, keep in zip(candidates, scale_mask, strict=True) if keep]
    removed_low_scale = int((~scale_mask).sum())
    if not scale_keep:
    ...
    if not scale_keep:
        raise RuntimeError("all model features are degenerate after standard-deviation screening")
    ...
    return selected, {
        ...
        "conditioning_numeric_dtype": str(numeric.to_numpy(copy=False).dtype),
        "conditioning_implementation": "research16_local_float64_numpy_quantile",
    }


def _startup_feature_conditioning_self_check() -> dict[str, object]:
    rows = 64
    probe = pd.DataFrame({
        "probe_float_a": np.linspace(-1.0, 1.0, rows, dtype=np.float64),
        "probe_float_b": np.sin(np.linspace(0.0, 3.0, rows, dtype=np.float64)),
        "probe_int": np.arange(rows, dtype=np.int64) % 7,
        "probe_native_bool": (np.arange(rows, dtype=np.int64) % 2) == 0,
        "probe_nullable_bool": pd.Series(
            (np.arange(rows, dtype=np.int64) % 3) == 0,
            dtype="boolean",
        ),
    })
    probe.loc[5, "probe_nullable_bool"] = pd.NA
    selected, diagnostic = _condition_feature_columns(
        probe,
        tuple(probe.columns),
        max_features=8,
        sample_rows=rows,
    )
    expected = {"probe_native_bool", "probe_nullable_bool"}
    if not expected.issubset(selected):
        raise RuntimeError(
            "Research 16 startup feature-conditioning self-check dropped boolean probes: "
            f"selected={selected}"
        )
    if diagnostic.get("conditioning_numeric_dtype") != "float64":
        raise RuntimeError(
            "Research 16 startup feature-conditioning self-check did not produce float64"
        )
    if _condition_feature_columns.__module__ != __name__:
        raise RuntimeError(
            "Research 16 startup feature-conditioning self-check detected external conditioner delegation"
        )
    return diagnostic
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s v{SCRIPT_VERSION} | feature_conditioner=research16_local_float64_numpy_quantile",
    )
...
def _fit_expert_scores(
    *,
    ...
    # Keep model fit and score matrices homogeneous as well.  Conditioning is
    # already safe locally; this second conversion protects downstream model
    # preprocessing from pandas extension dtypes.
    for frame_part in (train, policy_part, test_part):
        for column in selected:
            frame_part[column] = pd.to_numeric(frame_part[column], errors="coerce").astype(np.float64)
    if len(selected) < 4:
...
def run_research(args: argparse.Namespace) -> Path:
    print(f"[version] {SCRIPT_NAME} v{SCRIPT_VERSION} | source={Path(__file__).resolve()}", flush=True)
    self_check = _startup_feature_conditioning_self_check()
    print(
        "[safety] feature_conditioner=research16_local_float64_numpy_quantile "
        f"dtype={self_check['conditioning_numeric_dtype']} status=PASS",
        flush=True,
    )
