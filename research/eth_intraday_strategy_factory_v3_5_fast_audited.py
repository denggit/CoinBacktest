#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Intraday Strategy Factory V3.5 FAST AUDITED - single-file CoinBacktest research script.
...
---------------------------------------
python research/eth_intraday_strategy_factory_v3_5_fast_audited.py --mode smoke --max-specs 30 --out-dir data/reports/research/eth_intraday_factory_v3_5_smoke
python research/eth_intraday_strategy_factory_v3_5_fast_audited.py --mode core --max-specs 400 --out-dir data/reports/research/eth_intraday_factory_v3_5_core
python research/eth_intraday_strategy_factory_v3_5_fast_audited.py --mode wide --max-specs 1200 --robustness-top-n 80 --write-trades --out-dir data/reports/research/eth_intraday_factory_v3_5_wide
...
SCRIPT_NAME = "eth_intraday_strategy_factory_v3_5_fast_audited"
...
class RunConfig:
    ...
    out_dir: str = "data/reports/research/eth_intraday_strategy_factory_v3_5_fast_audited"
...
def signal_for_entry(df: pd.DataFrame, entry_model: str, spec: StrategySpec) -> pd.Series:
    ...
    if entry_model == "ema20_cross":
        ...
        r = _np_num(df, "ret_1")
        th = np.maximum(_np_num(df, "ret_abs_q70_past", 0.0), 0.00025)
        c = _np_num(df, "close")
        ema20 = _np_num(df, "ema_20")
        long_m = (r > th) & (c >= ema20)
        ...
        c = _np_num(df, "close")
        ph = _np_num(df, "prior_20_high", np.nan)
        pl = _np_num(df, "prior_20_low", np.nan)
        ...
        rth = np.maximum(_num(df, "ret_abs_q90_past", 0.0), 0.0008)
...
def write_raw_signal_diagnostics(raw_count_cache: dict[tuple[str, int, str], dict[str, int]], out_dir: Path) -> pd.DataFrame:
    ...
    out.to_csv(out_dir / "09_raw_signal_diagnostics.csv / 10_signal_condition_breakdown.csv", index=False)
    return out
    ...
    *,
    ...
    if cfg.fail_on_empty_audit and total_verify_signals <= 0:
        pd.DataFrame([{
            ...
            "hint": "Check 00_data_diagnostics.csv and 09_raw_signal_diagnostics.csv / 10_signal_condition_breakdown.csv and 10_signal_condition_breakdown.csv; data coverage or signal conditions are wrong.",
...
def run_factory(cfg: RunConfig) -> None:
    ...
    if cfg.fail_on_empty_signals and total_full_signals <= 0:
        ...
        raise RuntimeError(
            ...
            f"Check {out_dir / '00_data_diagnostics.csv'} and {out_dir / '09_raw_signal_diagnostics.csv / 10_signal_condition_breakdown.csv'}."
        )
...
- `09_raw_signal_diagnostics.csv / 10_signal_condition_breakdown.csv`: raw entry signal counts before regime filtering
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ETH intraday strategy factory V3.5 FAST, single-file CoinBacktest research script.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--out-dir", default="data/reports/research/eth_intraday_strategy_factory_v3_5_fast_audited")
    return p.parse_args()
