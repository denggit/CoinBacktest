#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Lead Flywheel Focused Research - five-system copy-trading upgrade research script.

Focused build: keeps the validated rangefix engine, then adds five targeted upgrade systems: trend-pullback, momentum-expansion, failed-breakout comfort, liquidity reclaim, and range-confirmation modules.
...
SCRIPT_NAME = "eth_lead_flywheel_focused_research_causal"
...
STRUCTURE_PRESETS: dict[str, dict[str, Any]] = {
    ...
    # Focused upgrade presets: smaller initial exposure and tighter post-TP handling.
    "flywheel_conservative": dict(initial_size_mult=0.40, tp_r=1.45, partial_tp_r=0.65, partial_fraction=0.60, max_hold_bars=70, stop_atr_mult=0.95, fail_fast_bars=7, fail_fast_adverse_r=0.28, trail_atr_mult=0.95),
    "flywheel_delay_safe": dict(initial_size_mult=0.50, tp_r=1.80, partial_tp_r=0.85, partial_fraction=0.50, max_hold_bars=110, stop_atr_mult=1.15, fail_fast_bars=11, fail_fast_adverse_r=0.38, trail_atr_mult=1.05),
    "trend_quality_runner": dict(initial_size_mult=0.55, tp_r=2.80, partial_tp_r=0.95, partial_fraction=0.45, add_trigger_r1=1.20, add_trigger_r2=2.40, add_size_1=0.30, add_size_2=0.20, max_hold_bars=210, stop_atr_mult=1.10, trail_atr_mult=1.05, fail_fast_bars=12, fail_fast_adverse_r=0.45),
}
...
LEAD_ENTRY_MODELS = [
    ...
    # Focused upgrade entries.  These are deliberately more selective than the broad V3.x candidates.
    "lead_trend_pullback_htf_range",
    "lead_trend_pullback_retest_delay",
    "lead_momentum_breakout_retest",
    "lead_failed_breakout_session_vwap",
    "lead_sweep_cvd_absorption",
    "lead_range_confirmed_pullback",
    "lead_range_confirmed_reclaim",
]
...
def _bool(df: pd.DataFrame, col: str, default: bool = False) -> pd.Series:
    """Return a robust boolean Series for mixed bool/int/float/string columns.

    Causal context alignment shifts higher-timeframe feature frames to their
    availability time and then fills missing rows.  Boolean feature columns may
    therefore arrive as bool, 0/1 integers, 0.0/1.0 floats, or occasional text
    values depending on pandas/local cache versions.  Pandas nullable boolean
    casting is intentionally strict and raises on non bool-like floats.  For
    research masks we need deterministic coercion instead of a dtype crash.
    """
    if col not in df.columns:
        return pd.Series(bool(default), index=df.index, dtype=bool)
    s = df[col]
    if pd.api.types.is_bool_dtype(s):
        return s.fillna(bool(default)).astype(bool)
    if pd.api.types.is_numeric_dtype(s):
        return pd.to_numeric(s, errors="coerce").fillna(1.0 if default else 0.0).ne(0.0).astype(bool)
    mapped = (
        s.astype("string")
        .str.strip()
        .str.lower()
    ...
    return mapped.fillna(bool(default)).astype(bool)
...
def _timeframe_to_timedelta(timeframe: str) -> pd.Timedelta:
    """Return the close/availability delay for a left-labeled timeframe string.

    OKXTradeBarLoader trade bars are treated as left-labeled aggregate bars:
    a row stamped 15:10 for 5m represents 15:10:00-15:14:59 and is first
    safely usable at 15:15.  All higher-timeframe context must therefore be
    aligned on bar_start + timeframe_delta, not on bar_start.
    """
    text = str(timeframe).strip()
    m = re.fullmatch(r"(\d+)\s*([mMhHdD])", text)
    if not m:
        # pandas handles common strings like "30min" or "1hour"; keep this as a
        # fallback for future local loader names.
        ...
        else:  # uppercase M is not used by current project, but treat as minute defensively.
            delta = pd.Timedelta(minutes=n)
    if delta <= pd.Timedelta(0):
        raise ValueError(f"Invalid non-positive timeframe delta for {timeframe!r}: {delta}")
    return delta
...
def load_timeframe_contexts(cfg: RunConfig, base_index: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    for tf in frames:
        ...
        # OKXTradeBarLoader aggregate bars are treated as left-labeled.  A 5m bar
        # stamped 15:10 is not available until 15:15, so aligning it directly on
        # 15:10 leaks the future final OHLCV into 1m signals.  Shift context
        # features to bar_available_time before ffill to the primary execution
        # frame.  This intentionally makes entries one primary bar more
        # conservative than using simultaneous 1m/5m close processing.
        availability_delta = _timeframe_to_timedelta(tf)
        feat = feat.sort_index().copy()
        feat.index = pd.DatetimeIndex(feat.index) + availability_delta
        feat = feat[~feat.index.duplicated(keep="last")].sort_index()
        aligned = feat.reindex(base_index, method="ffill").fillna(0.0)
        # Keep row order, but avoid duplicate-index joins later.
        parts.append(aligned.reset_index(drop=True))
        print(
            f"      {tf} context rows={len(ctx):,}; features={aligned.shape[1]} "
            f"causal_shift={availability_delta}",
            flush=True,
        )
...
def signal_for_entry(df: pd.DataFrame, entry_model: str, spec: StrategySpec) -> pd.Series:
    ...
    elif entry_model == "lead_trend_pullback_htf_range":
        # Direction 1 upgrade: tf5m trend-pullback with higher-timeframe and range/footprint confirmation.
        # The HTF columns are optional. If 15m/1H context was not loaded, the condition falls back to local trend.
        ema20 = _num(df, ccol("ema_20"))
        ema60 = _num(df, ccol("ema_60"))
        vwap = _num(df, ccol("session_vwap"))
        htf15_long = _bool(df, "tf15m_trend_up") | _num(df, "tf15m_close").eq(0)
        htf15_short = _bool(df, "tf15m_trend_down") | _num(df, "tf15m_close").eq(0)
        htf1h_long = _bool(df, "tf1H_trend_up") | _num(df, "tf1H_close").eq(0)
        htf1h_short = _bool(df, "tf1H_trend_down") | _num(df, "tf1H_close").eq(0)
        rf_seen = _num(df, "rf_bar_count").gt(0)
        rf_long_ok = (~rf_seen) | _num(df, "rf_direction_sum").ge(0) | _num(df, "rf_imbalance").gt(-0.03)
        rf_short_ok = (~rf_seen) | _num(df, "rf_direction_sum").le(0) | _num(df, "rf_imbalance").lt(0.03)
        fp_long_ok = _num(df, "fp_absorption_hint", 0.0).gt(-0.40)
        fp_short_ok = _num(df, "fp_absorption_hint", 0.0).lt(0.40)
        up_ctx = close.gt(ema60) & ema20.ge(ema60) & htf15_long & htf1h_long
        dn_ctx = close.lt(ema60) & ema20.le(ema60) & htf15_short & htf1h_short
        long_m = up_ctx & low.le(np.minimum(ema20, vwap)) & close.gt(ema20) & close.gt(open_) & delta.gt(-0.25) & rf_long_ok & fp_long_ok
        short_m = dn_ctx & high.ge(np.maximum(ema20, vwap)) & close.lt(ema20) & close.lt(open_) & delta.lt(0.25) & rf_short_ok & fp_short_ok
    elif entry_model == "lead_trend_pullback_retest_delay":
        # Direction 1 upgrade: delay-friendly retest entry. Instead of buying the first impulse,
        # enter only after a pullback/retest closes back in trend direction.
        ema20 = _num(df, ccol("ema_20"))
        ema60 = _num(df, ccol("ema_60"))
        ph60 = _num(df, ccol("prior_high_60"), np.nan)
        pl60 = _num(df, ccol("prior_low_60"), np.nan)
        up_ctx = close.gt(ema60) & ema20.ge(ema60)
        dn_ctx = close.lt(ema60) & ema20.le(ema60)
        long_m = up_ctx & low.le(ema20 * 1.0015) & close.gt(high.shift(1)) & close.lt(ph60 * 1.003) & delta.gt(-0.20)
        short_m = dn_ctx & high.ge(ema20 * 0.9985) & close.lt(low.shift(1)) & close.gt(pl60 * 0.997) & delta.lt(0.20)
    elif entry_model == "lead_momentum_breakout_retest":
        # Direction 2 upgrade: breakout-retest, not naked breakout chasing.
        ema20 = _num(df, ccol("ema_20"))
        ema60 = _num(df, ccol("ema_60"))
        ph = _num(df, ccol(f"prior_high_{spec.swing_window}"), np.nan)
        pl = _num(df, ccol(f"prior_low_{spec.swing_window}"), np.nan)
        hv = vol.gt(_num(df, ccol("vol_q75_past"), np.inf)) | _num(df, ccol("bar_range_pct")).gt(_num(df, ccol("range_q70_past"), np.inf))
        long_m = close.gt(ph) & low.le(ph * 1.0015) & close.gt(open_) & close.gt(ema60) & ema20.ge(ema60) & hv & delta.gt(-0.15)
        short_m = close.lt(pl) & high.ge(pl * 0.9985) & close.lt(open_) & close.lt(ema60) & ema20.le(ema60) & hv & delta.lt(0.15)
    elif entry_model == "lead_failed_breakout_session_vwap":
        # Direction 3 upgrade: failed breakout around opening range/session structure with VWAP target.
        orh = _num(df, ccol("opening_range_high"), np.nan)
        orl = _num(df, ccol("opening_range_low"), np.nan)
        vwap = _num(df, ccol("session_vwap"))
        active = _bool(df, ccol("after_opening_range")) & _num(df, ccol("session_minute")).between(60, 600)
        long_m = active & low.lt(orl) & close.gt(orl) & close.lt(vwap * 1.003) & close_pos.ge(0.55) & delta.gt(-0.45)
        short_m = active & high.gt(orh) & close.lt(orh) & close.gt(vwap * 0.997) & close_pos.le(0.45) & delta.lt(0.45)
    elif entry_model == "lead_sweep_cvd_absorption":
        # Direction 4 upgrade: liquidity sweep + CVD/footprint/range absorption proxy.
        cvd30 = _num(df, ccol("cvd_slope_30"))
        cvd120 = _num(df, ccol("cvd_slope_120"))
        fp = _num(df, "fp_absorption_hint", 0.0)
        rfimb = _num(df, "rf_imbalance", 0.0)
        absorb_long = (cvd30.gt(cvd30.shift(1)) | fp.gt(-0.15) | rfimb.gt(-0.02))
        absorb_short = (cvd30.lt(cvd30.shift(1)) | fp.lt(0.15) | rfimb.lt(0.02))
        long_m = low.lt(prior_low) & close.gt(prior_low) & close_pos.ge(0.58) & absorb_long & cvd120.gt(cvd120.rolling(20, min_periods=5).min())
        short_m = high.gt(prior_high) & close.lt(prior_high) & close_pos.le(0.42) & absorb_short & cvd120.lt(cvd120.rolling(20, min_periods=5).max())
    elif entry_model == "lead_range_confirmed_pullback":
        # Direction 5 upgrade: keep time/5m pullback as reason, use range bars as confirmation.
        ema20 = _num(df, ccol("ema_20"))
        ema60 = _num(df, ccol("ema_60"))
        rf_seen = _num(df, "rf_bar_count").gt(0)
        native_seen = _num(df, "direction").abs().rolling(20, min_periods=1).sum().gt(0)
        rf_long = (_num(df, "rf_direction_sum").gt(0) | _num(df, "range_dir_sum_3").gt(0))
        rf_short = (_num(df, "rf_direction_sum").lt(0) | _num(df, "range_dir_sum_3").lt(0))
        confirm_long = (~(rf_seen | native_seen)) | rf_long
        confirm_short = (~(rf_seen | native_seen)) | rf_short
        long_m = close.gt(ema60) & low.le(ema20) & close.gt(ema20) & close.gt(open_) & confirm_long & delta.gt(-0.25)
        short_m = close.lt(ema60) & high.ge(ema20) & close.lt(ema20) & close.lt(open_) & confirm_short & delta.lt(0.25)
    elif entry_model == "lead_range_confirmed_reclaim":
        # Direction 5 upgrade: sweep/reclaim only when range context does not fight the reversal.
        rf_seen = _num(df, "rf_bar_count").gt(0)
        native_seen = _num(df, "direction").abs().rolling(20, min_periods=1).sum().gt(0)
        rf_long_ok = (~(rf_seen | native_seen)) | _num(df, "rf_direction_sum").ge(0) | _num(df, "range_dir_sum_3").ge(0)
        rf_short_ok = (~(rf_seen | native_seen)) | _num(df, "rf_direction_sum").le(0) | _num(df, "range_dir_sum_3").le(0)
        long_m = low.lt(prior_low) & close.gt(prior_low) & close_pos.ge(0.55) & rf_long_ok & delta.gt(-0.45)
        short_m = high.gt(prior_high) & close.lt(prior_high) & close_pos.le(0.45) & rf_short_ok & delta.lt(0.45)
...
def apply_regime(df: pd.DataFrame, raw_signal: pd.Series, regime: str, signal_frame: str = "primary") -> pd.Series:
    ...
    if regime == "trend_aligned":
        ...
        ok_long = ok_short = m
    elif regime == "session_high_vol":
        hour = pd.Series(df.index.hour, index=df.index)
        m = hour.between(12, 23)
        hv = _num(df, ccol("bar_range_pct")) > _num(df, ccol("range_q70_past"), np.inf)
        ok_long = ok_short = m & hv
    elif regime == "range_quality":
        lowv = _num(df, ccol("bar_range_pct")) < _num(df, ccol("range_q70_past"), np.inf)
        ok_long = ok_short = lowv | _bool(df, ccol("range_regime"))
    elif regime == "lead_trend_quality":
        local_long = _bool(df, ccol("trend_up"))
        local_short = _bool(df, ccol("trend_down"))
        htf15_long = _bool(df, "tf15m_trend_up") | _num(df, "tf15m_close").eq(0)
        htf15_short = _bool(df, "tf15m_trend_down") | _num(df, "tf15m_close").eq(0)
        ok_long = local_long & htf15_long
        ok_short = local_short & htf15_short
    elif regime == "range_confirmed":
        rf_seen = _num(df, "rf_bar_count").gt(0)
        native_seen = _num(df, "direction").abs().rolling(20, min_periods=1).sum().gt(0)
        ok_long = (~(rf_seen | native_seen)) | _num(df, "rf_direction_sum").ge(0) | _num(df, "range_dir_sum_3").ge(0)
        ok_short = (~(rf_seen | native_seen)) | _num(df, "rf_direction_sum").le(0) | _num(df, "range_dir_sum_3").le(0)
    else:
...
def generate_specs(mode: str, max_specs: int | None = None, signal_frames: list[str] | None = None, include_sanity_in_core: bool = False) -> list[StrategySpec]:
    """Generate five focused upgrade systems.

    This script is intentionally not a broad V3.x signal sweep. It tries the five
    directions discovered in prior reports with more systematic confirmations:
    trend pullback, momentum expansion, failed breakout comfort, liquidity
    reclaim, and range-bar confirmation.
    """
    ...
    if mode == "smoke":
        systems: list[dict[str, Any]] = [
            dict(system="sanity_engine_check", layer="sanity", entries=["ema20_cross", "vwap_cross"], frames=["primary"], regimes=["any"], structures=["fixed_fast"], swings=[60], confirmation="engine_only"),
            dict(system="focused_trend_pullback", layer="expansion", entries=["lead_trend_pullback_htf_range", "lead_trend_pullback_retest_delay"], frames=["tf5m", "primary"], regimes=["lead_trend_quality", "session_high_vol"], structures=["flywheel_conservative", "trend_quality_runner"], swings=[120, 240], confirmation="htf_range"),
            dict(system="focused_failed_breakout", layer="comfort", entries=["lead_failed_breakout_session_vwap"], frames=["tf5m", "primary"], regimes=["range_quality", "eu_us_session"], structures=["flywheel_dopamine", "comfort_failfast"], swings=[120, 240], confirmation="session_vwap"),
        ]
    else:
        systems = [
            dict(system="focused_trend_pullback", layer="expansion", entries=["lead_trend_pullback_htf_range", "lead_trend_pullback_retest_delay", "lead_trend_pullback_flywheel"], frames=["tf5m", "primary"], regimes=["lead_trend_quality", "session_high_vol", "trend_aligned"], structures=["flywheel_conservative", "flywheel_delay_safe", "trend_quality_runner", "partial_runner"], swings=[120, 240, 480], confirmation="htf_range"),
            dict(system="focused_momentum_expansion", layer="expansion", entries=["lead_momentum_breakout_retest", "lead_range_confirmed_pullback"], frames=["tf5m", "primary"], regimes=["session_high_vol", "lead_trend_quality", "range_confirmed"], structures=["trend_quality_runner", "flywheel_delay_safe", "breakout_add_runner"], swings=[60, 120, 240], confirmation="breakout_retest"),
            dict(system="focused_failed_breakout_comfort", layer="comfort", entries=["lead_failed_breakout_session_vwap", "lead_failed_breakout_vwap", "failed_breakout_reclaim"], frames=["tf5m", "primary"], regimes=["range_quality", "low_vol", "eu_us_session"], structures=["flywheel_dopamine", "flywheel_conservative", "comfort_failfast"], swings=[120, 240, 480], confirmation="session_vwap"),
            dict(system="focused_liquidity_reclaim", layer="comfort", entries=["lead_sweep_cvd_absorption", "lead_range_confirmed_reclaim", "lead_sweep_absorption_proxy"], frames=["tf5m", "primary"], regimes=["range_quality", "eu_us_session", "range_confirmed"], structures=["flywheel_dopamine", "flywheel_probe", "flywheel_conservative", "partial_runner"], swings=[120, 240, 480], confirmation="cvd_absorption"),
            dict(system="focused_range_confirmation", layer="hybrid", entries=["lead_range_confirmed_pullback", "lead_range_confirmed_reclaim", "lead_range_burst_pullback"], frames=["tf5m", "primary"], regimes=["range_confirmed", "trend_aligned", "session_high_vol"], structures=["flywheel_delay_safe", "trend_quality_runner", "flywheel_conservative"], swings=[60, 120, 240], confirmation="range_confirm"),
        ]
        ...
        if mode == "wide":
            systems.append(dict(system="focused_baseline_compare", layer="baseline", entries=["lead_trend_pullback_flywheel", "lead_failed_breakout_vwap", "lead_range_burst_pullback", "lead_vwap_rotation_transition"], frames=["tf5m", "primary"], regimes=["high_vol", "low_vol", "eu_us_session", "trend_aligned"], structures=["flywheel_runner", "partial_runner", "expansion_pyramid"], swings=[120, 240, 480], confirmation="baseline"))
                ...
                for regime in sysdef["regimes"]:
                    for structure in sysdef["structures"]:
                        for swing in sysdef["swings"]:
                            ...
                            sid += 1
                            specs.append(replace(spec, spec_id=f"F{sid:05d}_{sysdef['system']}_{frame}_{entry}_{regime}_{structure}_sw{swing}"))
...
def _parse_mixed_datetimes(values: Any) -> pd.Series:
    """Parse trade timestamps that may mix second and microsecond precision.

    Range bars often have millisecond/microsecond timestamps, while time bars may
    stringify without fractional seconds. Pandas 2.x can infer a single strict
    format from the first value and then fail on mixed precision. Reporting must
    not crash after a valid backtest, so use format="mixed" when available and
    a safe fallback for older pandas.
    """
    try:
...
def _max_days_without_trade(entry_times: pd.Series) -> int:
    if entry_times.empty:
        return 999
    days = _parse_mixed_datetimes(entry_times).dt.normalize().dropna().drop_duplicates().sort_values()
    if len(days) <= 1:
        return 999
...
def summarize_trades(spec: StrategySpec, trades: list[TradeResult], cfg: RunConfig) -> dict[str, Any]:
    ...
    active_days = max(1, _parse_mixed_datetimes(t["entry_time"]).dt.normalize().nunique())
...
def yearly_stats(trades_df: pd.DataFrame, cfg: RunConfig) -> pd.DataFrame:
    if trades_df.empty:
        return pd.DataFrame()
    t = trades_df.copy()
    t["entry_time"] = _parse_mixed_datetimes(t["entry_time"])
...
def write_data_diagnostics(bars: pd.DataFrame, df: pd.DataFrame, cfg: RunConfig, arrays: BacktestArrays, out_dir: Path) -> pd.DataFrame:
    ...
    rows.append({"metric": "timeframe_context_alignment", "value": "causal_available_time"})
