#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""MF Low Sweep dynamic MFE/MAE path-exit research pass 03.
...
Research goal:
    Keep time48 only as the maximum fallback hold, not as the only exit. Test
    dynamic post-entry exits based on running MFE/MAE path quality: if a trade
    quickly develops adverse excursion without enough favorable excursion, exit
    on the next open. Hard stops remain only as emergency fuses, and cooldown
    remains optional after a fuse fires.
...
SCRIPT_NAME = "03_low_sweep_mfe_mae_dynamic_exit_research"
SCRIPT_VERSION = "v1.0.0"
EXPERIMENT_ID = "ETH_MF_LOW_SWEEP_MFE_MAE_DYNAMIC_EXIT_R03"
TITLE = "ETH MF Low Sweep MFE/MAE Dynamic Exit Research 03"
DEFAULT_OUT_DIR = "data/reports/research/mf_low_sweep/03_low_sweep_mfe_mae_dynamic_exit_research"
...
class RiskVariant:
    ...
    giveback_min_bars: int | None = None
    mfe_giveback_frac: float | None = None
    dynamic_min_bars: int | None = None
    dynamic_mae_floor: float | None = None
    dynamic_mfe_cap: float | None = None
    dynamic_ratio_max: float | None = None
    dynamic_require_close_below_entry: bool = True
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Research MF low-sweep dynamic MFE/MAE path exits plus emergency hard-stop/cooldown controls")
    ...
    p.add_argument(
        "--hard-stop-pcts",
        default="0.005,0.008,0.010,0.012,0.015,0.020,0.025,0.030,0.040",
        help="Actual hard-stop variants to simulate. Percent is price move against entry, e.g. 0.005 = 0.5%%.",
    )
    p.add_argument(
        "--threshold-grid-pcts",
        default="0.005,0.008,0.010,0.012,0.015,0.020,0.025,0.030,0.035,0.040,0.045,0.050,0.055,0.060,0.065,0.070,0.080",
        help="MAE/MFE diagnostic grid for stop threshold trade-off analysis. Does not by itself add variants.",
    )
    p.add_argument("--dynamic-check-bars", default="5,10,15,20,30", help="Closed-bar observation windows for dynamic MFE/MAE exits.")
    p.add_argument("--dynamic-mae-floors", default="0.005,0.008,0.010,0.012,0.015,0.020", help="Running MAE floors required before a dynamic fail exit can trigger.")
    p.add_argument("--dynamic-mfe-caps", default="0.001,0.002,0.003,0.005", help="Running MFE caps used by no-rebound fail-stop variants.")
    p.add_argument("--dynamic-ratio-maxes", default="0.25,0.35,0.50,0.75,1.00", help="Maximum running MFE/abs(MAE) ratios for ratio-fail exits.")
    p.add_argument("--giveback-mfe-triggers", default="0.008,0.012,0.016,0.020", help="MFE levels that arm giveback exits.")
    p.add_argument("--giveback-fracs", default="0.50,0.65,0.80", help="Fractions of running MFE that may be given back before exiting.")
    p.add_argument("--combo-hard-stop-pcts", default="0.025,0.030,0.040", help="Emergency hard stops used in dynamic-exit combo variants.")
...
def pct_tag(value: float) -> str:
    return f"{int(round(float(value) * 10000)):04d}"
...
def risk_variants(args: argparse.Namespace | None = None) -> list[RiskVariant]:
    ...
    R03 treats time48 as a maximum fallback horizon. The main variants exit
    earlier only when the *running* path is poor: MAE has expanded while MFE is
    still small, or running MFE/abs(MAE) is weak. This avoids the lookahead trap
    of using the completed 48-bar MFE/MAE ratio to decide an entry/exit.
    """
    ...
    variants: list[RiskVariant] = [
        RiskVariant(
            ...
            family="baseline",
            description="Current MF anchor: time48 close exit, no stop, no dynamic exit.",
        )
    ]

    # Tight hard stops are kept as diagnostics because the user explicitly asked
    # about 0.5%. They are not assumed to be good; the report should show if they
    # kill too many normal rebound winners.
    hard_stops = parse_float_list(getattr(args, "hard_stop_pcts", "0.005,0.008,0.010,0.012,0.015,0.020,0.025,0.030,0.040")) if args is not None else [0.005, 0.008, 0.010, 0.012, 0.015, 0.020, 0.025, 0.030, 0.040]
    for stop in hard_stops:
        tag = pct_tag(stop)
        family = "tight_hard_stop_diagnostic" if stop <= 0.015 else "emergency_hard_stop"
        variants.append(
            RiskVariant(
                variant_name=f"hard_stop_{tag}_time48",
                family=family,
                description=f"Diagnostic fixed hard stop at -{stop:.2%}; time48 fallback if not hit.",
                stop_pct=stop,
            )
        )

    check_bars = parse_int_list(getattr(args, "dynamic_check_bars", "5,10,15,20,30")) if args is not None else [5, 10, 15, 20, 30]
    mae_floors = parse_float_list(getattr(args, "dynamic_mae_floors", "0.005,0.008,0.010,0.012,0.015,0.020")) if args is not None else [0.005, 0.008, 0.010, 0.012, 0.015, 0.020]
    mfe_caps = parse_float_list(getattr(args, "dynamic_mfe_caps", "0.001,0.002,0.003,0.005")) if args is not None else [0.001, 0.002, 0.003, 0.005]
    ratio_maxes = parse_float_list(getattr(args, "dynamic_ratio_maxes", "0.25,0.35,0.50,0.75,1.00")) if args is not None else [0.25, 0.35, 0.50, 0.75, 1.0]

    # No-rebound fail stops: after N closed bars, if adverse excursion is already
    # meaningful but favorable excursion remains tiny, the trade has not shown
    # rebound quality. Exit next open.
    for bars_n in check_bars:
        if bars_n not in {5, 10, 15, 20, 30}:
            continue
        for mae in mae_floors:
            if mae > 0.020:
                continue
            for mfe_cap in mfe_caps:
                ...
                variants.append(
                    RiskVariant(
                        variant_name=f"dyn_norebound_b{bars_n}_mae{pct_tag(mae)}_mfe{pct_tag(mfe_cap)}_time48",
                        family="dynamic_no_rebound_exit",
                        description=f"After {bars_n} closed bars: if running MAE >= {mae:.2%} and running MFE <= {mfe_cap:.2%}, exit next open; time48 fallback.",
                        dynamic_min_bars=int(bars_n),
                        dynamic_mae_floor=float(mae),
                        dynamic_mfe_cap=float(mfe_cap),
                        dynamic_mode="no_rebound",
                    )
                )

    # Ratio-fail exits: after N closed bars, if MFE/abs(MAE) remains weak, the
    # path is more bad than good. Exit next open.
    for bars_n in check_bars:
        for mae in mae_floors:
            if mae > 0.020:
                continue
            for ratio in ratio_maxes:
                variants.append(
                    RiskVariant(
                        variant_name=f"dyn_ratio_b{bars_n}_mae{pct_tag(mae)}_r{ratio_tag(ratio)}_time48",
                        family="dynamic_mfe_mae_ratio_exit",
                        description=f"After {bars_n} closed bars: if running MAE >= {mae:.2%} and MFE/abs(MAE) <= {ratio:g}, exit next open; time48 fallback.",
                        dynamic_min_bars=int(bars_n),
                        dynamic_mae_floor=float(mae),
                        dynamic_ratio_max=float(ratio),
                        dynamic_mode="mfe_mae_ratio",
                    )
                )

    # MFE giveback exits: if a trade has already bounced but gives back most of
    # that bounce, don't wait blindly for time48.
    triggers = parse_float_list(getattr(args, "giveback_mfe_triggers", "0.008,0.012,0.016,0.020")) if args is not None else [0.008, 0.012, 0.016, 0.020]
    fracs = parse_float_list(getattr(args, "giveback_fracs", "0.50,0.65,0.80")) if args is not None else [0.50, 0.65, 0.80]
    for bars_n in [10, 15, 20, 30]:
        for trigger in triggers:
            for frac in fracs:
                variants.append(
                    RiskVariant(
                        variant_name=f"dyn_giveback_b{bars_n}_mfe{pct_tag(trigger)}_gb{pct_tag(frac)}_time48",
                        family="dynamic_mfe_giveback_exit",
                        description=f"After {bars_n} closed bars and MFE >= {trigger:.2%}: if close gives back >= {frac:.0%} of running MFE, exit next open; time48 fallback.",
                        giveback_trigger=float(trigger),
                        giveback_min_bars=int(bars_n),
                        mfe_giveback_frac=float(frac),
                        giveback_floor=0.0,
                    )
                )

    # Main combo candidates: dynamic fail-stop + wider emergency hard stop + stop-hit cooldown.
    combo_stops = parse_float_list(getattr(args, "combo_hard_stop_pcts", "0.025,0.030,0.040")) if args is not None else [0.025, 0.030, 0.040]
    core_dynamic = [
        (5, 0.005, 0.002, None, "norebound"),
        (10, 0.005, 0.002, None, "norebound"),
        (10, 0.008, 0.003, None, "norebound"),
        (15, 0.008, 0.003, None, "norebound"),
        (10, 0.008, None, 0.50, "ratio"),
        (15, 0.010, None, 0.50, "ratio"),
        (20, 0.010, None, 0.75, "ratio"),
    ]
    for bars_n, mae, mfe_cap, ratio, mode in core_dynamic:
        for stop in combo_stops:
            variants.append(
                RiskVariant(
                    variant_name=f"combo_dyn_{mode}_b{bars_n}_mae{pct_tag(mae)}_stop{pct_tag(stop)}_cooldown{int(cooldown)}h_time48",
                    family="dynamic_exit_hard_stop_cooldown_combo",
                    description=f"Dynamic {mode} exit; emergency stop -{stop:.2%}; if stop hit, cooldown {cooldown:g}h; time48 fallback.",
                    dynamic_min_bars=int(bars_n),
                    dynamic_mae_floor=float(mae),
                    dynamic_mfe_cap=float(mfe_cap) if mfe_cap is not None else None,
                    dynamic_ratio_max=float(ratio) if ratio is not None else None,
                    dynamic_mode=f"combo_{mode}",
                    stop_pct=float(stop),
                    cooldown_hours_after_stop=float(cooldown),
                )
            )
...
def simulate_trade(
    *,
    ...
    dynamic_trigger_mae_abs = np.nan
    dynamic_trigger_mfe = np.nan
    dynamic_trigger_ratio = np.nan
    dynamic_trigger_close_ret = np.nan
    dynamic_trigger_bar = np.nan
    ...
    for pos in range(entry_pos, planned_exit_pos + 1):
        ...
        running_mae = float(np.nanmin(mtm_low)) if mtm_low else np.nan
        running_mfe = float(np.nanmax(mtm_high)) if mtm_high else np.nan
        running_mae_abs = max(0.0, -running_mae) if math.isfinite(running_mae) else np.nan
        running_mfe_pos = max(0.0, running_mfe) if math.isfinite(running_mfe) else np.nan
        running_ratio = running_mfe_pos / running_mae_abs if math.isfinite(running_mae_abs) and running_mae_abs > 1e-12 else np.inf

        if variant.dynamic_min_bars is not None:
            if pos >= entry_pos + int(variant.dynamic_min_bars):
                mae_ok = math.isfinite(running_mae_abs) and running_mae_abs >= float(variant.dynamic_mae_floor or 0.0)
                cap_ok = variant.dynamic_mfe_cap is not None and math.isfinite(running_mfe_pos) and running_mfe_pos <= float(variant.dynamic_mfe_cap)
                ratio_ok = variant.dynamic_ratio_max is not None and math.isfinite(running_ratio) and running_ratio <= float(variant.dynamic_ratio_max)
                close_ok = (close_ret <= 0.0) if bool(variant.dynamic_require_close_below_entry) else True
                if mae_ok and (cap_ok or ratio_ok) and close_ok:
                    ...
                    exit_reason = f"dynamic_{variant.dynamic_mode or 'mfe_mae'}_b{int(variant.dynamic_min_bars)}_next_open"
                    dynamic_trigger_mae_abs = running_mae_abs
                    dynamic_trigger_mfe = running_mfe_pos
                    dynamic_trigger_ratio = running_ratio
                    dynamic_trigger_close_ret = close_ret
                    dynamic_trigger_bar = int(pos - entry_pos)
                    break
        ...
        if variant.giveback_trigger is not None:
            min_bar_ok = True if variant.giveback_min_bars is None else pos >= entry_pos + int(variant.giveback_min_bars)
            armed = min_bar_ok and mfe_now >= float(variant.giveback_trigger)
            if variant.mfe_giveback_frac is not None and math.isfinite(mfe_now):
                # Example: frac=0.65 exits when the close has given back at least
                # 65% of the running MFE from the entry anchor.
                giveback_floor_ret = mfe_now * (1.0 - float(variant.mfe_giveback_frac))
                weak_close = close_ret <= giveback_floor_ret
                if variant.giveback_floor is not None:
                    weak_close = weak_close and close_ret <= float(variant.giveback_floor)
            else:
            ...
            if armed and weak_close and weak_buy and weak_delta:
                ...
                exit_pos = int(next_pos)
                exit_price = float(opens[next_pos]) if next_pos < n else float(closes[pos])
                exit_reason = "dynamic_mfe_giveback_next_open"
                early_trigger_buy_share = buy_share
                early_trigger_delta_pressure = delta_pressure
                dynamic_trigger_mae_abs = running_mae_abs
                dynamic_trigger_mfe = running_mfe_pos
    ...
    mae_abs_final = max(0.0, -mae) if math.isfinite(mae) else np.nan
    mfe_pos_final = max(0.0, mfe) if math.isfinite(mfe) else np.nan
    mfe_to_mae_abs_ratio_final = mfe_pos_final / mae_abs_final if math.isfinite(mae_abs_final) and mae_abs_final > 1e-12 else np.inf
    ...
    return {
        ...
        "mae_on_equity": mae,
        "mfe_on_equity": mfe,
        "mae_abs_on_equity": mae_abs_final,
        "mfe_pos_on_equity": mfe_pos_final,
        "mfe_to_mae_abs_ratio": mfe_to_mae_abs_ratio_final,
        "mfe_gt_mae_abs_flag": bool(math.isfinite(mfe_to_mae_abs_ratio_final) and mfe_to_mae_abs_ratio_final > 1.0),
        "dynamic_trigger_mae_abs": dynamic_trigger_mae_abs,
        "dynamic_trigger_mfe": dynamic_trigger_mfe,
        "dynamic_trigger_ratio": dynamic_trigger_ratio,
        "dynamic_trigger_close_ret": dynamic_trigger_close_ret,
        "dynamic_trigger_bar": dynamic_trigger_bar,
...
def summarize_trades_for_variant(
    ...
    out: dict[str, object] = {
        ...
        "dynamic_min_bars": variant.dynamic_min_bars,
        "dynamic_mae_floor": variant.dynamic_mae_floor,
        "dynamic_mfe_cap": variant.dynamic_mfe_cap,
        "dynamic_ratio_max": variant.dynamic_ratio_max,
        "dynamic_mode": variant.dynamic_mode,
        "giveback_trigger": variant.giveback_trigger,
        "giveback_min_bars": variant.giveback_min_bars,
        "mfe_giveback_frac": variant.mfe_giveback_frac,
    ...
    out.update(
        {
            ...
            "dynamic_exit_rate": float(trades["exit_reason"].astype(str).str.startswith("dynamic_").mean()) if "exit_reason" in trades.columns else 0.0,
            "mfe_gt_mae_abs_rate": float(pd.to_numeric(trades.get("mfe_gt_mae_abs_flag", False), errors="coerce").fillna(0).mean()),
            "median_mfe_to_mae_abs_ratio": float(pd.to_numeric(trades.get("mfe_to_mae_abs_ratio", np.nan), errors="coerce").replace([np.inf, -np.inf], np.nan).median()),
...
def build_mfe_mae_quality_buckets(trades: pd.DataFrame) -> pd.DataFrame:
    """Future-known baseline path quality buckets for diagnostics only.

    This table validates the user's idea that trades with MFE > abs(MAE) are
    qualitatively different from trades whose adverse excursion dominates. It is
    *not* an entry filter because completed-path MFE/MAE is only known after the
    fact. R03 turns this into causal running-path exits elsewhere.
    """
    base = _baseline_only(trades)
    if base.empty:
        return pd.DataFrame()
    ratio = pd.to_numeric(base["mfe_to_mae_abs_ratio"], errors="coerce").replace([np.inf, -np.inf], np.nan)
    groups: list[tuple[str, pd.Series, str]] = [
        ("all_baseline", pd.Series(True, index=base.index), "All baseline time48 trades."),
        ("mfe_gt_mae", base["mfe_pos"] > base["mae_abs"], "Completed path had MFE greater than abs(MAE); diagnostic only."),
        ("mfe_le_mae", base["mfe_pos"] <= base["mae_abs"], "Completed path had MAE greater than/equal to MFE; diagnostic only."),
        ("mfe_gt_2x_mae", ratio > 2.0, "Strong favorable path quality; diagnostic only."),
        ("mfe_le_0p5x_mae", ratio <= 0.5, "Very weak path quality; diagnostic only."),
        ("positive_net", base["net_return_on_equity"] > 0, "Baseline winners."),
        ("negative_net", base["net_return_on_equity"] <= 0, "Baseline losers."),
    ]
    rows: list[dict[str, object]] = []
    for name, mask, desc in groups:
        grp = base.loc[mask.fillna(False)].copy()
        if grp.empty:
            rows.append({"path_quality_bucket": name, "description": desc, "trades": 0})
            continue
        x = pd.to_numeric(grp["net_return_on_equity"], errors="coerce").fillna(0.0)
        mae = pd.to_numeric(grp["mae_abs"], errors="coerce")
        mfe = pd.to_numeric(grp["mfe_pos"], errors="coerce")
        rows.append(
            {
                "path_quality_bucket": name,
                "description": desc,
                "trades": int(len(grp)),
                "share_of_baseline": float(len(grp) / max(len(base), 1)),
                "return_sum": float(x.sum()),
                "mean_return": float(x.mean()),
                "median_return": float(x.median()),
                "win_rate": float((x > 0).mean()),
                "profit_factor": profit_factor(x),
                "mae_abs_median": float(mae.median()),
                "mae_abs_p90": float(mae.quantile(0.90)),
                "mae_abs_p95": float(mae.quantile(0.95)),
                "mfe_median": float(mfe.median()),
                "mfe_p90": float(mfe.quantile(0.90)),
                "mfe_to_mae_abs_ratio_median": float(pd.to_numeric(grp["mfe_to_mae_abs_ratio"], errors="coerce").replace([np.inf, -np.inf], np.nan).median()),
                "liquidation_proxy_hits": int(pd.to_numeric(grp.get("liquidation_proxy_hit", False), errors="coerce").fillna(0).sum()),
            }
        )
    return pd.DataFrame(rows)
...
def build_exit_reason_breakdown(trades: pd.DataFrame) -> pd.DataFrame:
    if trades.empty:
        return pd.DataFrame()
    rows: list[dict[str, object]] = []
    df = trades.copy()
    df["net_return_on_equity"] = pd.to_numeric(df["net_return_on_equity"], errors="coerce").fillna(0.0)
    for (variant, reason), grp in df.groupby(["variant_name", "exit_reason"], dropna=False):
        x = grp["net_return_on_equity"]
        rows.append(
            {
                ...
                "exit_reason": reason,
                "trades": int(len(grp)),
                "share_of_variant_trades": float(len(grp) / max(int((df["variant_name"] == variant).sum()), 1)),
                "return_sum": float(x.sum()),
                "mean_return": float(x.mean()),
                "win_rate": float((x > 0).mean()),
                "profit_factor": profit_factor(x),
                "mae_min": float(pd.to_numeric(grp.get("mae_on_equity", np.nan), errors="coerce").min()),
                "mfe_mean": float(pd.to_numeric(grp.get("mfe_on_equity", np.nan), errors="coerce").mean()),
                "median_bars_held": float(pd.to_numeric(grp.get("bars_held", np.nan), errors="coerce").median()),
                "liquidation_proxy_hits": int(pd.to_numeric(grp.get("liquidation_proxy_hit", False), errors="coerce").fillna(0).sum()),
            }
        )
...
def build_dynamic_trigger_diagnostics(trades: pd.DataFrame) -> pd.DataFrame:
    if trades.empty:
        return pd.DataFrame()
    df = trades.loc[trades["exit_reason"].astype(str).str.startswith("dynamic_")].copy()
    if df.empty:
        return pd.DataFrame()
    df["net_return_on_equity"] = pd.to_numeric(df["net_return_on_equity"], errors="coerce").fillna(0.0)
    rows: list[dict[str, object]] = []
    for name, grp in df.groupby("variant_name", dropna=False):
        x = grp["net_return_on_equity"]
        rows.append(
            {
                "variant_name": name,
                "variant_family": grp["variant_family"].iloc[0] if "variant_family" in grp else "",
                "dynamic_exit_trades": int(len(grp)),
                "dynamic_exit_return_sum": float(x.sum()),
                "dynamic_exit_mean_return": float(x.mean()),
                "dynamic_exit_win_rate": float((x > 0).mean()),
                "trigger_mae_abs_median": float(pd.to_numeric(grp.get("dynamic_trigger_mae_abs", np.nan), errors="coerce").median()),
                "trigger_mfe_median": float(pd.to_numeric(grp.get("dynamic_trigger_mfe", np.nan), errors="coerce").median()),
                "trigger_ratio_median": float(pd.to_numeric(grp.get("dynamic_trigger_ratio", np.nan), errors="coerce").replace([np.inf, -np.inf], np.nan).median()),
                "trigger_bar_median": float(pd.to_numeric(grp.get("dynamic_trigger_bar", np.nan), errors="coerce").median()),
                "post_exit_liq_proxy_hits": int(pd.to_numeric(grp.get("liquidation_proxy_hit", False), errors="coerce").fillna(0).sum()),
            }
        )
    return pd.DataFrame(rows).sort_values(["dynamic_exit_mean_return", "dynamic_exit_trades"]).reset_index(drop=True)
...
def build_focus_day_report(trades: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    cols = [
        ...
        "mfe_to_mae_abs_ratio",
        "mfe_gt_mae_abs_flag",
        "dynamic_trigger_mae_abs",
        "dynamic_trigger_mfe",
        "dynamic_trigger_ratio",
        "dynamic_trigger_bar",
        "account_mae_proxy",
        "stop_pct",
        "stop_to_liq_ratio",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("[class] MF / Exit research only; no portfolio/live strategy changes", flush=True)
    ...
    stress_variants = [
        ...
        if v.family in {
            "baseline",
            "tight_hard_stop_diagnostic",
            "emergency_hard_stop",
            "dynamic_no_rebound_exit",
            "dynamic_mfe_mae_ratio_exit",
            "dynamic_mfe_giveback_exit",
            "dynamic_exit_hard_stop_cooldown_combo",
        }
    ]
    ...
    mfe_mae_quality_buckets = build_mfe_mae_quality_buckets(trades)
    exit_reason_breakdown = build_exit_reason_breakdown(trades)
    dynamic_trigger_diagnostics = build_dynamic_trigger_diagnostics(trades)
    ...
    manifest = {
        ...
        "combo_hard_stop_pcts": parse_float_list(args.combo_hard_stop_pcts),
        "threshold_grid_pcts": parse_float_list(args.threshold_grid_pcts),
        "dynamic_check_bars": parse_int_list(args.dynamic_check_bars),
        "dynamic_mae_floors": parse_float_list(args.dynamic_mae_floors),
        "dynamic_mfe_caps": parse_float_list(args.dynamic_mfe_caps),
        "dynamic_ratio_maxes": parse_float_list(args.dynamic_ratio_maxes),
        "giveback_mfe_triggers": parse_float_list(args.giveback_mfe_triggers),
        "giveback_fracs": parse_float_list(args.giveback_fracs),
        "stop_selection_policy": "Dynamic running MFE/MAE path quality first; hard stop is an emergency fuse; liquidation proxy is tail/execution audit only",
    ...
    write_csv(mfe_mae_quality_buckets, out_dir / "16_mfe_mae_quality_buckets.csv", "mfe_mae_quality_buckets")
    write_csv(exit_reason_breakdown, out_dir / "17_exit_reason_breakdown.csv", "exit_reason_breakdown")
    write_csv(dynamic_trigger_diagnostics, out_dir / "18_dynamic_trigger_diagnostics.csv", "dynamic_trigger_diagnostics")
    write_csv(extreme_trade_paths, out_dir / "19_extreme_trade_paths.csv", "extreme_trade_paths")
