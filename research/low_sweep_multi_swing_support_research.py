#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Multi-swing support research for Low Sweep MF upgrade.
...
SCRIPT_NAME = "low_sweep_multi_swing_support_research"
DEFAULT_OUT_DIR = "data/reports/research/low_sweep_multi_swing_support_research_tradebar_1m"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    own.add_argument(
        "--multi-support-modes",
        default="rank1_latest,first_swept_top2,first_swept_top3,first_swept_top5,older_only_top3,older_only_top5,rank2_prev,rank3_prev",
        help=(
            "Comma list. rankN_prev tests the Nth most recent confirmed low. "
            "first_swept_topN picks the most recent swept support among top N, "
            "so it includes baseline rank1 plus older supports only when rank1 was not swept. "
            "older_only_topN keeps only incremental older-support signals."
        ),
    )
    own.add_argument("--max-support-rank", type=int, default=5, help="Number of most recent confirmed low pivots to inspect per signal bar.")
...
def prepare_multi_swing_events_and_context(bars: pd.DataFrame, args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    print("[events] building multi-swing support events", flush=True)
    events = build_multi_swing_events(features, pivots, args)
    if events.empty:
    ...
    if bool(bad.any()):
        sample = events.loc[bad, ["signal_time", "support_available_time", "support_rank"]].head(5).to_dict("records")
        raise RuntimeError(f"multi-swing support availability violation: {sample}")
    ...
    return events.sort_values(["signal_time", "support_rank"]).reset_index(drop=True), features, pivots
...
def build_multi_support_mask(events: pd.DataFrame, mode: str) -> pd.Series:
    ...
    if m:
        return rank.eq(int(m.group(1)))
    m = re.fullmatch(r"first_swept_top(\d+)", mode)
    if m:
        n = int(m.group(1))
        return rank.le(n) & swept_order.eq(1)
    m = re.fullmatch(r"older_only_top(\d+)", mode)
    if m:
        n = int(m.group(1))
        return rank.ge(2) & rank.le(n) & swept_order.eq(1)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    meta = {
        ...
        "performance_guards": [
            ...
            "multi-support events are generated only from broad down-spike bars and scan at most max_support_rank pivots",
