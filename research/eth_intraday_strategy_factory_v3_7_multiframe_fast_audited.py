#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Intraday Strategy Factory V3.7 MULTIFRAME FAST AUDITED - single-file CoinBacktest research script.
...
---------------------------------------
python research/eth_intraday_strategy_factory_v3_7_fast_audited.py --mode smoke --max-specs 30 --out-dir data/reports/research/eth_intraday_factory_v3_7_smoke
python research/eth_intraday_strategy_factory_v3_7_fast_audited.py --mode core --max-specs 400 --out-dir data/reports/research/eth_intraday_factory_v3_7_core
python research/eth_intraday_strategy_factory_v3_7_fast_audited.py --mode wide --max-specs 1200 --robustness-top-n 80 --write-trades --out-dir data/reports/research/eth_intraday_factory_v3_7_wide
...
SCRIPT_NAME = "eth_intraday_strategy_factory_v3_7_multiframe_fast_audited"
...
class RunConfig:
    ...
    out_dir: str = "data/reports/research/eth_intraday_strategy_factory_v3_7_fast_audited"
...
ENTRY_MODELS = SANITY_ENTRY_MODELS + EDGE_ENTRY_MODELS
...
def load_timeframe_contexts(cfg: RunConfig, base_index: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    for tf in frames:
        ...
        aligned = feat.sort_index().reindex(base_index, method="ffill")
...
def build_features(bars: pd.DataFrame, range_ctx: pd.DataFrame, footprint_ctx: pd.DataFrame, time_ctx: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    df = df.join(range_ctx, how="left").join(footprint_ctx, how="left").join(time_ctx, how="left")
...
def generate_specs(mode: str, max_specs: int | None = None, signal_frames: list[str] | None = None, include_sanity_in_core: bool = False) -> list[StrategySpec]:
    signal_frames = signal_frames or ["primary"]
    if mode == "smoke":
        entries = SANITY_ENTRY_MODELS + [
            "sweep_reclaim",
            "trend_pullback_reclaim",
            "opening_range_breakout",
            "microtrend_continuation",
        ...
        entries = EDGE_ENTRY_MODELS + (SANITY_ENTRY_MODELS if include_sanity_in_core else [])
    ...
    sid = 0
    # Interleave frames inside each entry.  This matters when --max-specs is
    # small: the sample should contain primary + auxiliary-frame ideas instead
    # of filling the entire cap with primary-only specs.
    for entry in entries:
        for frame in signal_frames:
            # Range-context/native-range entries do not use higher-timeframe OHLC prefixes.
            ...
            for regime in regimes:
                for structure in structures:
                    for swing in swing_windows:
                        ...
                        sid += 1
                        specs.append(replace(spec, spec_id=f"S{sid:05d}_{frame}_{entry}_{regime}_{structure}_sw{swing}"))
...
def run_factory(cfg: RunConfig) -> None:
    ...
    readme = f"""# ETH Intraday Strategy Factory V3.7 MULTIFRAME FAST Report
...
- V3.7 supports time primary, range primary, multi-timeframe context, range context and footprint context;
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ETH intraday strategy factory V3.7 MULTIFRAME FAST, single-file CoinBacktest research script.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--out-dir", default="data/reports/research/eth_intraday_strategy_factory_v3_7_multiframe_fast_audited")
    return p.parse_args()
