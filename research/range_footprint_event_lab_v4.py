#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Range-bar + footprint event path-management lab for ETH OKX research.

This is not a trading strategy.  It creates objective event samples and measures
what happens after each event using only future labels for research reporting.
All event features are computed with shifted rolling windows, so event detection
itself does not use future data.
"""
from __future__ import annotations

import argparse
import json
import os
import sqlite3
import sys
from dataclasses import dataclass
from pathlib import Path
from typing import Any

import numpy as np
import pandas as pd

CURRENT_FILE = os.path.abspath(__file__)
PROJECT_ROOT = os.path.dirname(os.path.dirname(CURRENT_FILE))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from src.data_feed.okx_range_bar_loader import OKXRangeBarLoader
from src.data_feed.okx_range_footprint_loader import OKXRangeFootprintLoader

SCRIPT_NAME = "RangeFootprintEventLabV4"


@dataclass
class Config:
    symbol: str = "ETH-USDT-SWAP"
    start_date: str = "2023-01-01"
    end_date: str = "2026-06-15"
    warmup_start_date: str = "2022-01-01"
    range_pct: float = 0.0020
    price_step: float = 1.0
    data_dir: str | None = None
    out_dir: Path = Path("data/reports/research/range_footprint_event_lab_v4")

    swing_window: int = 80
    quantile_window: int = 300
    pressure_quantile: float = 0.88
    horizon_bars: int = 80
    stop_buffer_pct: float = 0.0003
    sweep_buffer_pct: float = 0.0
    breakout_buffer_pct: float = 0.0
    fee_rate_per_side: float = 0.00055
    slippage_pct: float = 0.00015
    max_events_per_type: int = 0  # 0 = no cap
    include_footprint: bool = True
    bin_quantiles: int = 5
    min_bin_events: int = 80
    entry_mode: str = "event_break_trigger"  # next_open | event_break_trigger
    entry_wait_bars: int = 20
    entry_buffer_pct: float = 0.00015
    partial_fraction: float = 0.5


def _ts_text(value: Any) -> str:
    return pd.Timestamp(value).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]


def _progress_marks(start: str, end: str) -> list[pd.Timestamp]:
    cur = pd.Timestamp(start) + pd.DateOffset(months=1)
    end_ts = pd.Timestamp(end)
    marks: list[pd.Timestamp] = []
    while cur <= end_ts:
        marks.append(cur)
        cur += pd.DateOffset(months=1)
    if not marks or marks[-1] < end_ts:
        marks.append(end_ts)
    return marks


def _rolling_quantile_shifted(s: pd.Series, window: int, q: float) -> pd.Series:
    return s.rolling(int(window), min_periods=max(20, int(window // 4))).quantile(float(q)).shift(1)


def _safe_div(num: pd.Series, den: pd.Series, default: float = np.nan) -> pd.Series:
    out = num.astype(float) / den.replace(0, np.nan).astype(float)
    return out.replace([np.inf, -np.inf], default)


def _rolling_zscore_shifted(s: pd.Series, window: int) -> pd.Series:
    mean = s.rolling(int(window), min_periods=max(20, int(window // 4))).mean().shift(1)
    std = s.rolling(int(window), min_periods=max(20, int(window // 4))).std(ddof=0).shift(1)
    return ((s - mean) / std.replace(0, np.nan)).replace([np.inf, -np.inf], np.nan)


def _ema(s: pd.Series, span: int) -> pd.Series:
    return s.ewm(span=int(span), adjust=False, min_periods=max(5, int(span // 4))).mean()


def _run_length(sign: pd.Series) -> pd.Series:
    vals = sign.fillna(0).astype(int).to_numpy()
    out = np.zeros(len(vals), dtype=int)
    run = 0
    last = 0
    for i, v in enumerate(vals):
        if v == 0:
            run = 0
            last = 0
        elif v == last:
            run += 1
        else:
            run = 1
            last = v
        out[i] = run if v > 0 else -run if v < 0 else 0
    return pd.Series(out, index=sign.index)


FEATURE_COLUMNS = [
    "range_pct_bar",
    "body_pct_of_range",
    "close_pos",
    "bar_return_pct",
    "delta_ratio",
    "buy_sell_imbalance",
    "taker_buy_ratio",
    "rolling_delta_sum_20",
    "rolling_delta_sum_80",
    "rolling_delta_sum_240",
    "cvd_slope_20_pct",
    "cvd_slope_80_pct",
    "ema_20_80_diff_pct",
    "ema_80_240_diff_pct",
    "ema_20_slope_pct",
    "ema_80_slope_pct",
    "range_zscore_300",
    "volume_zscore_300",
    "notional_zscore_300",
    "duration_zscore_300",
    "distance_to_prior_high_pct",
    "distance_to_prior_low_pct",
    "sweep_depth_pct",
    "breakout_distance_pct",
    "direction_run",
    "abs_direction_run",
    "max_sell_bucket_pos",
    "max_buy_bucket_pos",
    "max_sell_bucket_share",
    "max_buy_bucket_share",
    "approx_roundtrip_cost_r",
]


class FootprintFeatureStore:
    """SQLite footprint access for one-row-per-bar max buy/sell bucket features."""

    def __init__(self, cfg: Config):
        self.loader = OKXRangeFootprintLoader(
            symbol=cfg.symbol,
            range_pct=cfg.range_pct,
            price_step=cfg.price_step,
            data_dir=cfg.data_dir,
        )
        self.db_path = Path(self.loader.db_path)
        self.table_name = self.loader.table_name
        self._ensure_indexes()

    def _connect(self) -> sqlite3.Connection:
        return sqlite3.connect(str(self.db_path))

    def _ensure_indexes(self) -> None:
        with self._connect() as conn:
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.table_name}_bar_id ON {self.table_name}(bar_id)")
            conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{self.table_name}_end_ts ON {self.table_name}(end_ts)")

    def load_max_bucket_features(self, start_date: str, end_date: str) -> pd.DataFrame:
        sell_sql = f"""
            SELECT bar_id, price_bucket AS max_sell_bucket,
                   sell_notional AS max_bucket_sell_notional,
                   delta_notional AS max_sell_bucket_delta_notional,
                   large_sell_notional AS max_bucket_large_sell_notional
            FROM (
                SELECT bar_id, price_bucket, sell_notional, delta_notional, large_sell_notional,
                       ROW_NUMBER() OVER (
                           PARTITION BY bar_id
                           ORDER BY sell_notional DESC, price_bucket ASC
                       ) AS rn
                FROM {self.table_name}
                WHERE end_ts >= ? AND end_ts <= ? AND sell_notional > 0
            ) ranked
            WHERE rn = 1
        """
        buy_sql = f"""
            SELECT bar_id, price_bucket AS max_buy_bucket,
                   buy_notional AS max_bucket_buy_notional,
                   delta_notional AS max_buy_bucket_delta_notional,
                   large_buy_notional AS max_bucket_large_buy_notional
            FROM (
                SELECT bar_id, price_bucket, buy_notional, delta_notional, large_buy_notional,
                       ROW_NUMBER() OVER (
                           PARTITION BY bar_id
                           ORDER BY buy_notional DESC, price_bucket DESC
                       ) AS rn
                FROM {self.table_name}
                WHERE end_ts >= ? AND end_ts <= ? AND buy_notional > 0
            ) ranked
            WHERE rn = 1
        """
        params = (_ts_text(start_date), _ts_text(end_date))
        with self._connect() as conn:
            sell = pd.read_sql_query(sell_sql, conn, params=params)
            buy = pd.read_sql_query(buy_sql, conn, params=params)
        out = sell.merge(buy, on="bar_id", how="outer")
        return out


def load_bars(cfg: Config) -> pd.DataFrame:
    loader = OKXRangeBarLoader(symbol=cfg.symbol, range_pct=cfg.range_pct, data_dir=cfg.data_dir)
    bars = loader.load_local_data(start_date=cfg.warmup_start_date, end_date=cfg.end_date)
    if bars.empty:
        raise RuntimeError("No local range bars found. Build/preload range bars first.")
    bars = bars.reset_index(drop=True).copy()
    bars["end_ts"] = pd.to_datetime(bars["end_ts"])
    bars["start_ts"] = pd.to_datetime(bars["start_ts"])
    bars = bars.sort_values(["end_ts", "bar_id"]).reset_index(drop=True)
    print(
        f"Loaded range bars: {len(bars):,} | {bars['end_ts'].iloc[0]} -> {bars['end_ts'].iloc[-1]}",
        flush=True,
    )
    if cfg.include_footprint:
        try:
            fp = FootprintFeatureStore(cfg).load_max_bucket_features(cfg.warmup_start_date, cfg.end_date)
            bars = bars.merge(fp, on="bar_id", how="left")
            print(f"Loaded footprint max-bucket features: {len(fp):,}", flush=True)
        except Exception as exc:
            print(f"[WARN] footprint max-bucket features unavailable: {exc}", flush=True)
    return bars


def add_features(b: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    out = b.copy()
    for col in [
        "open",
        "high",
        "low",
        "close",
        "volume",
        "notional",
        "buy_notional",
        "sell_notional",
        "delta_notional",
        "taker_buy_ratio",
        "duration_seconds",
        "trades_count",
    ]:
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors="coerce").fillna(0.0)

    price_span = (out["high"] - out["low"]).replace(0, np.nan)
    out["range_pct_bar"] = _safe_div(out["high"] - out["low"], out["open"], 0.0)
    out["body_pct_of_range"] = ((out["close"] - out["open"]).abs() / price_span).fillna(0.0)
    out["close_pos"] = ((out["close"] - out["low"]) / price_span).clip(0, 1).fillna(0.5)
    out["bar_return_pct"] = _safe_div(out["close"] - out["open"], out["open"], 0.0)
    out["delta_ratio"] = _safe_div(out["delta_notional"], out["notional"].abs(), 0.0)
    out["buy_sell_imbalance"] = _safe_div(out["buy_notional"] - out["sell_notional"], out["buy_notional"] + out["sell_notional"], 0.0)
    out["cvd_notional"] = out["delta_notional"].cumsum()

    for w in (20, 80, 240):
        out[f"rolling_delta_sum_{w}"] = out["delta_notional"].rolling(w, min_periods=max(5, w // 4)).sum().shift(1)
        out[f"rolling_return_sum_{w}"] = out["bar_return_pct"].rolling(w, min_periods=max(5, w // 4)).sum().shift(1)
    out["rolling_delta_sum"] = out["rolling_delta_sum_80"]  # V1 compatibility.

    cvd_abs_base = out["notional"].rolling(240, min_periods=60).mean().shift(1).replace(0, np.nan)
    out["cvd_slope_20_pct"] = ((out["cvd_notional"] - out["cvd_notional"].shift(20)) / cvd_abs_base).replace([np.inf, -np.inf], np.nan)
    out["cvd_slope_80_pct"] = ((out["cvd_notional"] - out["cvd_notional"].shift(80)) / cvd_abs_base).replace([np.inf, -np.inf], np.nan)

    ema20 = _ema(out["close"], 20)
    ema80 = _ema(out["close"], 80)
    ema240 = _ema(out["close"], 240)
    out["ema_20_80_diff_pct"] = _safe_div(ema20 - ema80, out["close"], 0.0)
    out["ema_80_240_diff_pct"] = _safe_div(ema80 - ema240, out["close"], 0.0)
    out["ema_20_slope_pct"] = _safe_div(ema20 - ema20.shift(20), ema20.shift(20), 0.0)
    out["ema_80_slope_pct"] = _safe_div(ema80 - ema80.shift(80), ema80.shift(80), 0.0)

    out["range_zscore_300"] = _rolling_zscore_shifted(out["range_pct_bar"], 300)
    out["volume_zscore_300"] = _rolling_zscore_shifted(out["volume"], 300)
    out["notional_zscore_300"] = _rolling_zscore_shifted(out["notional"], 300)
    out["duration_zscore_300"] = _rolling_zscore_shifted(out["duration_seconds"], 300)

    out["prior_swing_high"] = out["high"].shift(1).rolling(cfg.swing_window, min_periods=max(10, cfg.swing_window // 4)).max()
    out["prior_swing_low"] = out["low"].shift(1).rolling(cfg.swing_window, min_periods=max(10, cfg.swing_window // 4)).min()
    out["prior_cvd_high"] = out["cvd_notional"].shift(1).rolling(cfg.swing_window, min_periods=max(10, cfg.swing_window // 4)).max()
    out["prior_cvd_low"] = out["cvd_notional"].shift(1).rolling(cfg.swing_window, min_periods=max(10, cfg.swing_window // 4)).min()
    out["distance_to_prior_high_pct"] = _safe_div(out["prior_swing_high"] - out["close"], out["close"], np.nan)
    out["distance_to_prior_low_pct"] = _safe_div(out["close"] - out["prior_swing_low"], out["close"], np.nan)
    out["sweep_depth_pct"] = np.where(
        out["low"] < out["prior_swing_low"],
        _safe_div(out["prior_swing_low"] - out["low"], out["prior_swing_low"], np.nan),
        np.where(out["high"] > out["prior_swing_high"], _safe_div(out["high"] - out["prior_swing_high"], out["prior_swing_high"], np.nan), np.nan),
    )
    out["breakout_distance_pct"] = np.where(
        out["close"] > out["prior_swing_high"],
        _safe_div(out["close"] - out["prior_swing_high"], out["prior_swing_high"], np.nan),
        np.where(out["close"] < out["prior_swing_low"], _safe_div(out["prior_swing_low"] - out["close"], out["prior_swing_low"], np.nan), np.nan),
    )

    direction = np.sign(out["bar_return_pct"])
    out["direction_run"] = _run_length(direction)
    out["abs_direction_run"] = out["direction_run"].abs()

    out["buy_notional_q"] = _rolling_quantile_shifted(out["buy_notional"], cfg.quantile_window, cfg.pressure_quantile)
    out["sell_notional_q"] = _rolling_quantile_shifted(out["sell_notional"], cfg.quantile_window, cfg.pressure_quantile)
    out["abs_delta_q"] = _rolling_quantile_shifted(out["delta_notional"].abs(), cfg.quantile_window, cfg.pressure_quantile)
    out["volume_q"] = _rolling_quantile_shifted(out["volume"], cfg.quantile_window, cfg.pressure_quantile)

    # Footprint bucket positions inside the bar, when available.
    for col in ["max_sell_bucket", "max_buy_bucket", "max_bucket_sell_notional", "max_bucket_buy_notional"]:
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors="coerce")
    if "max_sell_bucket" in out.columns:
        out["max_sell_bucket_pos"] = ((out["max_sell_bucket"] - out["low"]) / price_span).replace([np.inf, -np.inf], np.nan)
        out["max_sell_bucket_share"] = _safe_div(out["max_bucket_sell_notional"], out["sell_notional"], np.nan)
    else:
        out["max_sell_bucket_pos"] = np.nan
        out["max_sell_bucket_share"] = np.nan
    if "max_buy_bucket" in out.columns:
        out["max_buy_bucket_pos"] = ((out["max_buy_bucket"] - out["low"]) / price_span).replace([np.inf, -np.inf], np.nan)
        out["max_buy_bucket_share"] = _safe_div(out["max_bucket_buy_notional"], out["buy_notional"], np.nan)
    else:
        out["max_buy_bucket_pos"] = np.nan
        out["max_buy_bucket_share"] = np.nan
    return out


def _first_touch_outcomes(
    *,
    side: str,
    future_high: np.ndarray,
    future_low: np.ndarray,
    entry: float,
    stop: float,
    risk: float,
    targets_r: tuple[float, ...] = (1.0, 2.0, 3.0),
) -> dict[str, Any]:
    results: dict[str, Any] = {}
    for r in targets_r:
        target = entry + r * risk if side == "long" else entry - r * risk
        hit = False
        hit_bar = np.nan
        for j, (hi, lo) in enumerate(zip(future_high, future_low), start=1):
            # Conservative same-bar ordering: stop first if both are touched.
            if side == "long":
                if lo <= stop:
                    hit = False
                    hit_bar = j
                    break
                if hi >= target:
                    hit = True
                    hit_bar = j
                    break
            else:
                if hi >= stop:
                    hit = False
                    hit_bar = j
                    break
                if lo <= target:
                    hit = True
                    hit_bar = j
                    break
        results[f"hit_{int(r)}r_before_stop"] = bool(hit)
        results[f"touch_{int(r)}r_or_stop_bars"] = float(hit_bar) if np.isfinite(hit_bar) else np.nan
    return results



def _fixed_target_outcome(
    *,
    side: str,
    future_high: np.ndarray,
    future_low: np.ndarray,
    horizon_r: float,
    entry: float,
    stop: float,
    risk: float,
    target_r: float,
) -> tuple[float, int, str]:
    """Return gross R outcome, bars to exit, and reason for a fixed target/stop path."""
    target = entry + target_r * risk if side == "long" else entry - target_r * risk
    for j, (hi, lo) in enumerate(zip(future_high, future_low), start=1):
        # Conservative same-bar ordering: stop first if both are touched.
        if side == "long":
            if lo <= stop:
                return -1.0, j, "stop"
            if hi >= target:
                return float(target_r), j, f"target_{target_r:g}r"
        else:
            if hi >= stop:
                return -1.0, j, "stop"
            if lo <= target:
                return float(target_r), j, f"target_{target_r:g}r"
    return float(np.clip(horizon_r, -1.0, target_r)), len(future_high), "horizon"


def _be_after_1r_outcome(
    *,
    side: str,
    future_high: np.ndarray,
    future_low: np.ndarray,
    horizon_r: float,
    entry: float,
    stop: float,
    risk: float,
    runner_target_r: float,
) -> tuple[float, int, str]:
    """Move stop to breakeven only after +1R is touched; then aim for runner_target_r."""
    first_target = entry + risk if side == "long" else entry - risk
    runner_target = entry + runner_target_r * risk if side == "long" else entry - runner_target_r * risk
    be_active = False
    for j, (hi, lo) in enumerate(zip(future_high, future_low), start=1):
        if not be_active:
            if side == "long":
                if lo <= stop:
                    return -1.0, j, "initial_stop_before_1r"
                if hi >= runner_target:
                    return float(runner_target_r), j, f"runner_{runner_target_r:g}r_same_bar"
                if hi >= first_target:
                    be_active = True
                    continue
            else:
                if hi >= stop:
                    return -1.0, j, "initial_stop_before_1r"
                if lo <= runner_target:
                    return float(runner_target_r), j, f"runner_{runner_target_r:g}r_same_bar"
                if lo <= first_target:
                    be_active = True
                    continue
        else:
            if side == "long":
                if lo <= entry:
                    return 0.0, j, "breakeven_after_1r"
                if hi >= runner_target:
                    return float(runner_target_r), j, f"runner_{runner_target_r:g}r"
            else:
                if hi >= entry:
                    return 0.0, j, "breakeven_after_1r"
                if lo <= runner_target:
                    return float(runner_target_r), j, f"runner_{runner_target_r:g}r"
    if be_active:
        return float(np.clip(horizon_r, 0.0, runner_target_r)), len(future_high), "horizon_after_1r"
    return float(np.clip(horizon_r, -1.0, 1.0)), len(future_high), "horizon_before_1r"


def _partial_be_after_1r_outcome(
    *,
    side: str,
    future_high: np.ndarray,
    future_low: np.ndarray,
    horizon_r: float,
    entry: float,
    stop: float,
    risk: float,
    runner_target_r: float,
    partial_fraction: float,
) -> tuple[float, int, str]:
    """Take partial_fraction at +1R, move remainder to BE, aim for runner_target_r."""
    first_target = entry + risk if side == "long" else entry - risk
    runner_target = entry + runner_target_r * risk if side == "long" else entry - runner_target_r * risk
    pf = float(np.clip(partial_fraction, 0.0, 1.0))
    runner_weight = 1.0 - pf
    partial_taken = False
    for j, (hi, lo) in enumerate(zip(future_high, future_low), start=1):
        if not partial_taken:
            if side == "long":
                if lo <= stop:
                    return -1.0, j, "initial_stop_before_partial"
                if hi >= runner_target:
                    return pf * 1.0 + runner_weight * runner_target_r, j, f"partial_and_runner_{runner_target_r:g}r_same_bar"
                if hi >= first_target:
                    partial_taken = True
                    continue
            else:
                if hi >= stop:
                    return -1.0, j, "initial_stop_before_partial"
                if lo <= runner_target:
                    return pf * 1.0 + runner_weight * runner_target_r, j, f"partial_and_runner_{runner_target_r:g}r_same_bar"
                if lo <= first_target:
                    partial_taken = True
                    continue
        else:
            if side == "long":
                if lo <= entry:
                    return pf * 1.0, j, "partial_then_breakeven"
                if hi >= runner_target:
                    return pf * 1.0 + runner_weight * runner_target_r, j, f"partial_then_runner_{runner_target_r:g}r"
            else:
                if hi >= entry:
                    return pf * 1.0, j, "partial_then_breakeven"
                if lo <= runner_target:
                    return pf * 1.0 + runner_weight * runner_target_r, j, f"partial_then_runner_{runner_target_r:g}r"
    if partial_taken:
        return pf * 1.0 + runner_weight * float(np.clip(horizon_r, 0.0, runner_target_r)), len(future_high), "horizon_after_partial"
    return float(np.clip(horizon_r, -1.0, 1.0)), len(future_high), "horizon_before_partial"


def _path_management_outcomes(
    *,
    side: str,
    future_high: np.ndarray,
    future_low: np.ndarray,
    horizon_r: float,
    entry: float,
    stop: float,
    risk: float,
    partial_fraction: float,
) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for target_r in (1.0, 2.0, 3.0):
        value, bars, reason = _fixed_target_outcome(
            side=side,
            future_high=future_high,
            future_low=future_low,
            horizon_r=horizon_r,
            entry=entry,
            stop=stop,
            risk=risk,
            target_r=target_r,
        )
        key = f"fixed_{int(target_r)}r"
        out[f"{key}_gross_r"] = float(value)
        out[f"{key}_exit_bars"] = int(bars)
        out[f"{key}_exit_reason"] = reason
    for runner_target_r in (2.0, 3.0):
        value, bars, reason = _be_after_1r_outcome(
            side=side,
            future_high=future_high,
            future_low=future_low,
            horizon_r=horizon_r,
            entry=entry,
            stop=stop,
            risk=risk,
            runner_target_r=runner_target_r,
        )
        key = f"be1_to_{int(runner_target_r)}r"
        out[f"{key}_gross_r"] = float(value)
        out[f"{key}_exit_bars"] = int(bars)
        out[f"{key}_exit_reason"] = reason
        value, bars, reason = _partial_be_after_1r_outcome(
            side=side,
            future_high=future_high,
            future_low=future_low,
            horizon_r=horizon_r,
            entry=entry,
            stop=stop,
            risk=risk,
            runner_target_r=runner_target_r,
            partial_fraction=partial_fraction,
        )
        key = f"partial50_be1_to_{int(runner_target_r)}r"
        out[f"{key}_gross_r"] = float(value)
        out[f"{key}_exit_bars"] = int(bars)
        out[f"{key}_exit_reason"] = reason
    return out


def _entry_from_event(
    b: pd.DataFrame,
    i: int,
    side: str,
    cfg: Config,
) -> tuple[int, float, float, str] | None:
    """Return (entry_i, entry_price, stop_price, entry_mode_used).

    V3 adds event_break_trigger mode:
    - long: after event bar, buy stop above event high;
    - short: after event bar, sell stop below event low;
    - if invalidation stop is touched before trigger, no entry sample is created.

    This keeps feature timing clean: event features are known at event close;
    trigger/stop evaluation starts only from future bars.
    """
    n = len(b)
    high_a = b["high"].to_numpy(float)
    low_a = b["low"].to_numpy(float)
    open_a = b["open"].to_numpy(float)

    if cfg.entry_mode == "next_open":
        entry_i = i + 1
        if entry_i >= n:
            return None
        if side == "long":
            entry = open_a[entry_i] * (1.0 + cfg.slippage_pct)
            stop = low_a[i] * (1.0 - cfg.stop_buffer_pct)
        else:
            entry = open_a[entry_i] * (1.0 - cfg.slippage_pct)
            stop = high_a[i] * (1.0 + cfg.stop_buffer_pct)
        return entry_i, float(entry), float(stop), "next_open"

    if cfg.entry_mode != "event_break_trigger":
        raise ValueError(f"unsupported entry_mode={cfg.entry_mode}")

    max_i = min(n - 1, i + int(cfg.entry_wait_bars))
    if side == "long":
        trigger = high_a[i] * (1.0 + cfg.entry_buffer_pct)
        stop = low_a[i] * (1.0 - cfg.stop_buffer_pct)
        if trigger <= stop:
            return None
        for j in range(i + 1, max_i + 1):
            # Conservative same-bar ordering: if invalidation and trigger both happen,
            # treat invalidation first.
            if low_a[j] <= stop:
                return None
            if high_a[j] >= trigger:
                entry = trigger * (1.0 + cfg.slippage_pct)
                return j, float(entry), float(stop), "event_break_trigger"
    else:
        trigger = low_a[i] * (1.0 - cfg.entry_buffer_pct)
        stop = high_a[i] * (1.0 + cfg.stop_buffer_pct)
        if trigger >= stop:
            return None
        for j in range(i + 1, max_i + 1):
            if high_a[j] >= stop:
                return None
            if low_a[j] <= trigger:
                entry = trigger * (1.0 - cfg.slippage_pct)
                return j, float(entry), float(stop), "event_break_trigger"
    return None


def _evaluate_event(b: pd.DataFrame, i: int, side: str, event_type: str, cfg: Config) -> dict[str, Any] | None:
    n = len(b)
    entry_info = _entry_from_event(b, i, side, cfg)
    if entry_info is None:
        return None
    entry_i, entry, stop, entry_mode_used = entry_info
    end_i = min(n, entry_i + cfg.horizon_bars)
    if entry_i >= n or end_i <= entry_i:
        return None

    high_a = b["high"].to_numpy(float)
    low_a = b["low"].to_numpy(float)
    close_a = b["close"].to_numpy(float)

    if side == "long":
        risk = entry - stop
        if risk <= 0:
            return None
        future_high = high_a[entry_i:end_i]
        future_low = low_a[entry_i:end_i]
        mfe_r = (float(np.max(future_high)) - entry) / risk
        mae_r = (entry - float(np.min(future_low))) / risk
        horizon_r = (close_a[end_i - 1] - entry) / risk
    else:
        risk = stop - entry
        if risk <= 0:
            return None
        future_high = high_a[entry_i:end_i]
        future_low = low_a[entry_i:end_i]
        mfe_r = (entry - float(np.min(future_low))) / risk
        mae_r = (float(np.max(future_high)) - entry) / risk
        horizon_r = (entry - close_a[end_i - 1]) / risk

    touches = _first_touch_outcomes(
        side=side,
        future_high=future_high,
        future_low=future_low,
        entry=entry,
        stop=stop,
        risk=risk,
    )
    path_outcomes = _path_management_outcomes(
        side=side,
        future_high=future_high,
        future_low=future_low,
        horizon_r=float(horizon_r),
        entry=entry,
        stop=stop,
        risk=risk,
        partial_fraction=cfg.partial_fraction,
    )
    approx_roundtrip_cost_r = entry * (2.0 * cfg.fee_rate_per_side + 2.0 * cfg.slippage_pct) / risk

    row = b.iloc[i]
    out = {
        "event_time": row["end_ts"],
        "entry_time": b.iloc[entry_i]["start_ts"],
        "event_i": int(i),
        "entry_i": int(entry_i),
        "entry_delay_bars": int(entry_i - i),
        "bar_id": int(row["bar_id"]),
        "event_type": event_type,
        "side": side,
        "entry_mode": entry_mode_used,
        "entry": float(entry),
        "stop": float(stop),
        "risk": float(risk),
        "horizon_bars": int(end_i - entry_i),
        "mfe_r": float(mfe_r),
        "mae_r": float(mae_r),
        "horizon_r": float(horizon_r),
        "approx_roundtrip_cost_r": float(approx_roundtrip_cost_r),
        "open": float(row["open"]),
        "high": float(row["high"]),
        "low": float(row["low"]),
        "close": float(row["close"]),
        "prior_swing_high": float(row.get("prior_swing_high", np.nan)),
        "prior_swing_low": float(row.get("prior_swing_low", np.nan)),
        "delta_notional": float(row.get("delta_notional", np.nan)),
        "delta_ratio": float(row.get("delta_ratio", np.nan)),
        "buy_notional": float(row.get("buy_notional", np.nan)),
        "sell_notional": float(row.get("sell_notional", np.nan)),
        "taker_buy_ratio": float(row.get("taker_buy_ratio", np.nan)),
        "volume": float(row.get("volume", np.nan)),
        "close_pos": float(row.get("close_pos", np.nan)),
        "max_sell_bucket_pos": float(row.get("max_sell_bucket_pos", np.nan)),
        "max_buy_bucket_pos": float(row.get("max_buy_bucket_pos", np.nan)),
    }
    for feature in FEATURE_COLUMNS:
        if feature in row.index:
            try:
                out[feature] = float(row.get(feature, np.nan))
            except Exception:
                out[feature] = np.nan
    out.update(touches)
    out.update(path_outcomes)
    return out


def generate_events(b: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    start_ts = pd.Timestamp(cfg.start_date)
    end_ts = pd.Timestamp(cfg.end_date)
    marks = _progress_marks(cfg.start_date, cfg.end_date)
    mark_pos = 0
    monthly_counts: dict[str, int] = {}

    required_cols = ["prior_swing_high", "prior_swing_low", "buy_notional_q", "sell_notional_q", "abs_delta_q"]
    valid = b[required_cols].notna().all(axis=1)

    high_pressure_buy = (b["buy_notional"] >= b["buy_notional_q"]) & (b["delta_notional"] >= b["abs_delta_q"])
    high_pressure_sell = (b["sell_notional"] >= b["sell_notional_q"]) & (b["delta_notional"] <= -b["abs_delta_q"])

    sweep_low = (b["low"] < b["prior_swing_low"] * (1.0 - cfg.sweep_buffer_pct)) & (b["close"] > b["prior_swing_low"])
    sweep_high = (b["high"] > b["prior_swing_high"] * (1.0 + cfg.sweep_buffer_pct)) & (b["close"] < b["prior_swing_high"])

    breakout_up = (b["close"] > b["prior_swing_high"] * (1.0 + cfg.breakout_buffer_pct)) & (b["close_pos"] >= 0.70)
    breakout_down = (b["close"] < b["prior_swing_low"] * (1.0 - cfg.breakout_buffer_pct)) & (b["close_pos"] <= 0.30)

    cvd_higher_low = b["cvd_notional"] > b["prior_cvd_low"]
    cvd_lower_high = b["cvd_notional"] < b["prior_cvd_high"]

    event_masks: list[tuple[str, str, pd.Series]] = [
        ("sweep_low_reclaim_sell_pressure", "long", valid & sweep_low & high_pressure_sell),
        ("sweep_high_reclaim_buy_pressure", "short", valid & sweep_high & high_pressure_buy),
        ("initiative_breakout_up_buy_pressure", "long", valid & breakout_up & high_pressure_buy),
        ("initiative_breakout_down_sell_pressure", "short", valid & breakout_down & high_pressure_sell),
        ("delta_divergence_low_reclaim", "long", valid & sweep_low & (b["delta_notional"] < 0) & cvd_higher_low),
        ("delta_divergence_high_reclaim", "short", valid & sweep_high & (b["delta_notional"] > 0) & cvd_lower_high),
    ]

    for event_type, side, mask in event_masks:
        idxs = np.flatnonzero(mask.to_numpy(dtype=bool))
        if cfg.max_events_per_type and len(idxs) > cfg.max_events_per_type:
            idxs = idxs[-cfg.max_events_per_type :]
        print(f"[{SCRIPT_NAME}] event_type={event_type} side={side} candidates={len(idxs):,}", flush=True)
        for i in idxs:
            ts = pd.Timestamp(b.iloc[i]["end_ts"])
            if ts < start_ts or ts > end_ts:
                continue
            while mark_pos < len(marks) and ts >= marks[mark_pos]:
                print(
                    f"[{SCRIPT_NAME}] completed_to={marks[mark_pos].strftime('%Y-%m-%d')} events={len(rows):,}",
                    flush=True,
                )
                mark_pos += 1
            ev = _evaluate_event(b, int(i), side, event_type, cfg)
            if ev is not None:
                rows.append(ev)
                monthly_counts[event_type] = monthly_counts.get(event_type, 0) + 1

    events = pd.DataFrame(rows)
    if events.empty:
        return events
    events["event_time"] = pd.to_datetime(events["event_time"])
    events["entry_time"] = pd.to_datetime(events["entry_time"])
    return events.sort_values(["event_time", "event_type"]).reset_index(drop=True)


def _summarize_group(g: pd.DataFrame) -> pd.Series:
    n = len(g)
    hit1 = float(g["hit_1r_before_stop"].mean()) if n else np.nan
    hit2 = float(g["hit_2r_before_stop"].mean()) if n else np.nan
    hit3 = float(g["hit_3r_before_stop"].mean()) if n else np.nan
    gross_exp_1r = hit1 * 1.0 - (1.0 - hit1) * 1.0
    gross_exp_2r = hit2 * 2.0 - (1.0 - hit2) * 1.0
    gross_exp_3r = hit3 * 3.0 - (1.0 - hit3) * 1.0
    avg_cost = float(g["approx_roundtrip_cost_r"].mean())
    return pd.Series(
        {
            "events": n,
            "avg_mfe_r": float(g["mfe_r"].mean()),
            "median_mfe_r": float(g["mfe_r"].median()),
            "avg_mae_r": float(g["mae_r"].mean()),
            "median_mae_r": float(g["mae_r"].median()),
            "avg_horizon_r": float(g["horizon_r"].mean()),
            "hit_1r_before_stop_pct": hit1 * 100.0,
            "hit_2r_before_stop_pct": hit2 * 100.0,
            "hit_3r_before_stop_pct": hit3 * 100.0,
            "gross_exp_1r_stop_r": gross_exp_1r,
            "gross_exp_2r_stop_r": gross_exp_2r,
            "gross_exp_3r_stop_r": gross_exp_3r,
            "avg_cost_r": avg_cost,
            "net_exp_1r_stop_r": gross_exp_1r - avg_cost,
            "net_exp_2r_stop_r": gross_exp_2r - avg_cost,
            "net_exp_3r_stop_r": gross_exp_3r - avg_cost,
        }
    )


def build_summaries(events: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    if events.empty:
        return pd.DataFrame(), pd.DataFrame()
    summary = events.groupby(["event_type", "side", "entry_mode"], dropna=False).apply(_summarize_group, include_groups=False).reset_index()
    events = events.copy()
    events["year"] = events["event_time"].dt.year
    yearly = events.groupby(["year", "event_type", "side", "entry_mode"], dropna=False).apply(_summarize_group, include_groups=False).reset_index()
    return summary.sort_values("net_exp_2r_stop_r", ascending=False), yearly.sort_values(["event_type", "year"])


def _bin_label(x: Any) -> str:
    return str(x).replace("\n", " ")


def build_feature_binning(events: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    if events.empty:
        return pd.DataFrame()
    rows: list[dict[str, Any]] = []
    for (event_type, side, entry_mode), group in events.groupby(["event_type", "side", "entry_mode"], dropna=False):
        for feature in FEATURE_COLUMNS:
            if feature not in group.columns:
                continue
            vals = pd.to_numeric(group[feature], errors="coerce")
            finite = vals.replace([np.inf, -np.inf], np.nan).notna()
            g = group.loc[finite].copy()
            if len(g) < cfg.min_bin_events * 2:
                continue
            try:
                bins = pd.qcut(g[feature], q=int(cfg.bin_quantiles), duplicates="drop")
            except Exception:
                continue
            for bin_value, bin_group in g.groupby(bins, observed=True):
                if len(bin_group) < cfg.min_bin_events:
                    continue
                metrics = _summarize_group(bin_group)
                row = {
                    "event_type": event_type,
                    "side": side,
                    "entry_mode": entry_mode,
                    "feature": feature,
                    "bin": _bin_label(bin_value),
                    "bin_left": float(getattr(bin_value, "left", np.nan)),
                    "bin_right": float(getattr(bin_value, "right", np.nan)),
                }
                row.update(metrics.to_dict())
                rows.append(row)
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame(rows).sort_values(["net_exp_2r_stop_r", "events"], ascending=[False, False]).reset_index(drop=True)


def build_feature_correlations(events: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    if events.empty:
        return pd.DataFrame()
    targets = ["mfe_r", "mae_r", "horizon_r", "hit_1r_before_stop", "hit_2r_before_stop", "hit_3r_before_stop"]
    rows: list[dict[str, Any]] = []
    data = events.copy()
    for t in targets:
        if t in data.columns and data[t].dtype == bool:
            data[t] = data[t].astype(float)
    for (event_type, side, entry_mode), group in data.groupby(["event_type", "side", "entry_mode"], dropna=False):
        if len(group) < cfg.min_bin_events:
            continue
        for feature in FEATURE_COLUMNS:
            if feature not in group.columns:
                continue
            x = pd.to_numeric(group[feature], errors="coerce").replace([np.inf, -np.inf], np.nan)
            for target in targets:
                if target not in group.columns:
                    continue
                y = pd.to_numeric(group[target], errors="coerce").replace([np.inf, -np.inf], np.nan)
                ok = x.notna() & y.notna()
                if ok.sum() < cfg.min_bin_events:
                    continue
                x_ok = x.loc[ok]
                y_ok = y.loc[ok]
                if x_ok.nunique(dropna=True) < 2 or y_ok.nunique(dropna=True) < 2:
                    continue
                corr = x_ok.corr(y_ok, method="spearman")
                rows.append({
                    "event_type": event_type,
                    "side": side,
                    "entry_mode": entry_mode,
                    "feature": feature,
                    "target": target,
                    "spearman_corr": float(corr) if np.isfinite(corr) else np.nan,
                    "n": int(ok.sum()),
                })
    if not rows:
        return pd.DataFrame()
    out = pd.DataFrame(rows)
    out["abs_corr"] = out["spearman_corr"].abs()
    return out.sort_values(["abs_corr", "n"], ascending=[False, False]).reset_index(drop=True)



MANAGEMENT_OUTCOME_COLUMNS = [
    "fixed_1r_gross_r",
    "fixed_2r_gross_r",
    "fixed_3r_gross_r",
    "be1_to_2r_gross_r",
    "be1_to_3r_gross_r",
    "partial50_be1_to_2r_gross_r",
    "partial50_be1_to_3r_gross_r",
]


def _management_label(col: str) -> str:
    return col.replace("_gross_r", "")


def _summarize_management_group(g: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    avg_cost = float(g["approx_roundtrip_cost_r"].mean())
    for col in MANAGEMENT_OUTCOME_COLUMNS:
        if col not in g.columns:
            continue
        vals = pd.to_numeric(g[col], errors="coerce").replace([np.inf, -np.inf], np.nan).dropna()
        if vals.empty:
            continue
        rows.append(
            {
                "management": _management_label(col),
                "events": int(len(vals)),
                "avg_gross_r": float(vals.mean()),
                "median_gross_r": float(vals.median()),
                "positive_pct": float((vals > 0).mean() * 100.0),
                "loss_pct": float((vals < 0).mean() * 100.0),
                "flat_pct": float((vals == 0).mean() * 100.0),
                "avg_cost_r": avg_cost,
                "avg_net_r": float(vals.mean() - avg_cost),
                "median_net_r": float(vals.median() - avg_cost),
            }
        )
    return pd.DataFrame(rows)


def build_management_summaries(events: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    if events.empty:
        return pd.DataFrame(), pd.DataFrame()
    rows: list[pd.DataFrame] = []
    for keys, group in events.groupby(["event_type", "side", "entry_mode"], dropna=False):
        event_type, side, entry_mode = keys
        mg = _summarize_management_group(group)
        if mg.empty:
            continue
        mg.insert(0, "entry_mode", entry_mode)
        mg.insert(0, "side", side)
        mg.insert(0, "event_type", event_type)
        rows.append(mg)
    management = pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()
    if not management.empty:
        management = management.sort_values(["avg_net_r", "events"], ascending=[False, False]).reset_index(drop=True)

    yr_rows: list[pd.DataFrame] = []
    ev = events.copy()
    ev["year"] = ev["event_time"].dt.year
    for keys, group in ev.groupby(["year", "event_type", "side", "entry_mode"], dropna=False):
        year, event_type, side, entry_mode = keys
        mg = _summarize_management_group(group)
        if mg.empty:
            continue
        mg.insert(0, "entry_mode", entry_mode)
        mg.insert(0, "side", side)
        mg.insert(0, "event_type", event_type)
        mg.insert(0, "year", year)
        yr_rows.append(mg)
    yearly = pd.concat(yr_rows, ignore_index=True) if yr_rows else pd.DataFrame()
    if not yearly.empty:
        yearly = yearly.sort_values(["event_type", "management", "year"]).reset_index(drop=True)
    return management, yearly


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description=SCRIPT_NAME, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    p.add_argument("--symbol", default="ETH-USDT-SWAP")
    p.add_argument("--start-date", default="2023-01-01")
    p.add_argument("--end-date", default="2026-06-15")
    p.add_argument("--warmup-start-date", default="2022-01-01")
    p.add_argument("--range-pct", type=float, default=0.0020)
    p.add_argument("--price-step", type=float, default=1.0)
    p.add_argument("--data-dir", default=None)
    p.add_argument("--out-dir", default="data/reports/research/range_footprint_event_lab_v4")
    p.add_argument("--swing-window", type=int, default=80)
    p.add_argument("--quantile-window", type=int, default=300)
    p.add_argument("--pressure-quantile", type=float, default=0.88)
    p.add_argument("--horizon-bars", type=int, default=80)
    p.add_argument("--stop-buffer-pct", type=float, default=0.0003)
    p.add_argument("--sweep-buffer-pct", type=float, default=0.0)
    p.add_argument("--breakout-buffer-pct", type=float, default=0.0)
    p.add_argument("--fee-rate-per-side", type=float, default=0.00055)
    p.add_argument("--slippage-pct", type=float, default=0.00015)
    p.add_argument("--max-events-per-type", type=int, default=0)
    p.add_argument("--no-footprint", action="store_true")
    p.add_argument("--bin-quantiles", type=int, default=5)
    p.add_argument("--min-bin-events", type=int, default=80)
    p.add_argument("--entry-mode", choices=["next_open", "event_break_trigger"], default="event_break_trigger")
    p.add_argument("--entry-wait-bars", type=int, default=20)
    p.add_argument("--entry-buffer-pct", type=float, default=0.00015)
    p.add_argument("--partial-fraction", type=float, default=0.5)
    return p.parse_args()


def main() -> None:
    args = parse_args()
    cfg = Config(
        symbol=args.symbol,
        start_date=args.start_date,
        end_date=args.end_date,
        warmup_start_date=args.warmup_start_date,
        range_pct=args.range_pct,
        price_step=args.price_step,
        data_dir=args.data_dir,
        out_dir=Path(args.out_dir),
        swing_window=args.swing_window,
        quantile_window=args.quantile_window,
        pressure_quantile=args.pressure_quantile,
        horizon_bars=args.horizon_bars,
        stop_buffer_pct=args.stop_buffer_pct,
        sweep_buffer_pct=args.sweep_buffer_pct,
        breakout_buffer_pct=args.breakout_buffer_pct,
        fee_rate_per_side=args.fee_rate_per_side,
        slippage_pct=args.slippage_pct,
        max_events_per_type=args.max_events_per_type,
        include_footprint=not args.no_footprint,
        bin_quantiles=args.bin_quantiles,
        min_bin_events=args.min_bin_events,
        entry_mode=args.entry_mode,
        entry_wait_bars=args.entry_wait_bars,
        entry_buffer_pct=args.entry_buffer_pct,
        partial_fraction=args.partial_fraction,
    )
    cfg.out_dir.mkdir(parents=True, exist_ok=True)
    print(f"[{SCRIPT_NAME}] cfg={json.dumps({**cfg.__dict__, 'out_dir': str(cfg.out_dir)}, default=str, ensure_ascii=False)}", flush=True)

    bars = load_bars(cfg)
    bars = add_features(bars, cfg)
    events = generate_events(bars, cfg)
    summary, yearly = build_summaries(events)
    management, management_yearly = build_management_summaries(events)
    feature_binning = build_feature_binning(events, cfg)
    feature_corr = build_feature_correlations(events, cfg)

    events_path = cfg.out_dir / "range_footprint_event_lab_v4_events.csv"
    summary_path = cfg.out_dir / "range_footprint_event_lab_v4_summary.csv"
    yearly_path = cfg.out_dir / "range_footprint_event_lab_v4_yearly.csv"
    management_path = cfg.out_dir / "range_footprint_event_lab_v4_management_summary.csv"
    management_yearly_path = cfg.out_dir / "range_footprint_event_lab_v4_management_yearly.csv"
    binning_path = cfg.out_dir / "range_footprint_event_lab_v4_feature_binning.csv"
    corr_path = cfg.out_dir / "range_footprint_event_lab_v4_feature_correlations.csv"
    config_path = cfg.out_dir / "range_footprint_event_lab_v4_config.json"

    events.to_csv(events_path, index=False)
    summary.to_csv(summary_path, index=False)
    yearly.to_csv(yearly_path, index=False)
    management.to_csv(management_path, index=False)
    management_yearly.to_csv(management_yearly_path, index=False)
    feature_binning.to_csv(binning_path, index=False)
    feature_corr.to_csv(corr_path, index=False)
    config_path.write_text(json.dumps({**cfg.__dict__, "out_dir": str(cfg.out_dir)}, ensure_ascii=False, indent=2, default=str), encoding="utf-8")

    print("\n=== Event Summary sorted by net_exp_2r_stop_r ===")
    if summary.empty:
        print("No events generated.")
    else:
        cols = [
            "event_type",
            "side",
            "entry_mode",
            "events",
            "avg_mfe_r",
            "avg_mae_r",
            "hit_1r_before_stop_pct",
            "hit_2r_before_stop_pct",
            "net_exp_1r_stop_r",
            "net_exp_2r_stop_r",
            "avg_cost_r",
        ]
        print(summary[cols].to_string(index=False))
    if not management.empty:
        print("\n=== Top Path Management by avg_net_r ===")
        cols_m = ["event_type", "side", "entry_mode", "management", "events", "avg_gross_r", "avg_net_r", "positive_pct", "loss_pct", "avg_cost_r"]
        print(management[cols_m].head(40).to_string(index=False))

    if not feature_binning.empty:
        print("\n=== Top Feature Bins by net_exp_2r_stop_r ===")
        cols2 = ["event_type", "side", "entry_mode", "feature", "bin", "events", "hit_2r_before_stop_pct", "net_exp_2r_stop_r", "avg_mfe_r", "avg_mae_r"]
        print(feature_binning[cols2].head(30).to_string(index=False))
    if not feature_corr.empty:
        print("\n=== Top Feature Correlations ===")
        cols3 = ["event_type", "side", "entry_mode", "feature", "target", "spearman_corr", "n"]
        print(feature_corr[cols3].head(30).to_string(index=False))
    print(f"\nOutputs: {cfg.out_dir}")


if __name__ == "__main__":
    main()
