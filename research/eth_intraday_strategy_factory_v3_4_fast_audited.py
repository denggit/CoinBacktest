#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Intraday Strategy Factory V3.4 FAST AUDITED - single-file CoinBacktest research script.
...
---------------------------------------
python research/eth_intraday_strategy_factory_v3_4_fast_audited.py --mode smoke --max-specs 30 --out-dir data/reports/research/eth_intraday_factory_v3_4_smoke
python research/eth_intraday_strategy_factory_v3_4_fast_audited.py --mode core --max-specs 400 --out-dir data/reports/research/eth_intraday_factory_v3_4_core
python research/eth_intraday_strategy_factory_v3_4_fast_audited.py --mode wide --max-specs 1200 --robustness-top-n 80 --write-trades --out-dir data/reports/research/eth_intraday_factory_v3_4_wide
...
SCRIPT_NAME = "eth_intraday_strategy_factory_v3_4_fast_audited"
...
class RunConfig:
    ...
    out_dir: str = "data/reports/research/eth_intraday_strategy_factory_v3_4_fast_audited"
...
def signal_for_entry(df: pd.DataFrame, entry_model: str, spec: StrategySpec) -> pd.Series:
    ...
    if entry_model == "ema20_cross":
        ...
        ema60 = _num(df, "ema_60")
        # Simple tradable baseline: closed bar crosses EMA20 in the direction of
        # EMA60 context.  Used in smoke/core as a sanity check and candidate seed.
        long_m = close.gt(ema20) & close.shift(1).le(ema20.shift(1)) & close.ge(ema60 * 0.998)
        short_m = close.lt(ema20) & close.shift(1).ge(ema20.shift(1)) & close.le(ema60 * 1.002)
    elif entry_model == "vwap_cross":
        vwap = _num(df, "session_vwap")
        # Another baseline that should fire frequently on healthy 1m data.
        long_m = close.gt(vwap) & close.shift(1).le(vwap.shift(1))
        short_m = close.lt(vwap) & close.shift(1).ge(vwap.shift(1))
    elif entry_model == "ret_impulse_continuation":
        # Past-only threshold with conservative floor; no orderbook required.
        rth = np.maximum(_num(df, "ret_abs_q90_past", 0.0), 0.0010)
        long_m = ret.gt(rth) & close.ge(_num(df, "ema_20"))
        short_m = ret.lt(-rth) & close.le(_num(df, "ema_20"))
    elif entry_model == "or_breakout_close":
        orh = _num(df, "opening_range_high", np.nan)
        orl = _num(df, "opening_range_low", np.nan)
        active = _bool(df, "after_opening_range") & _num(df, "session_minute").le(720)
        ...
        pressure_ok = high_vol | _num(df, "bar_range_pct").gt(_num(df, "range_q70_past", np.inf))
        ...
        ema60 = _num(df, "ema_60")
        vwap = _num(df, "session_vwap")
        up_ctx = close.gt(ema60) | _bool(df, "trend_up")
        dn_ctx = close.lt(ema60) | _bool(df, "trend_down")
        ...
        th = np.maximum(_num(df, "vwap_dev_q90_past", 0.0), 0.0012)
        long_m = dev.lt(-th) & close_pos.ge(0.55) & delta.gt(-0.60) & (~_bool(df, "trend_down"))
        short_m = dev.gt(th) & close_pos.le(0.45) & delta.lt(0.60) & (~_bool(df, "trend_up"))
        ...
        rth = np.maximum(_num(df, "ret_abs_q90_past", 0.0), 0.0015)
        panic = extreme_vol | _num(df, "bar_range_pct").gt(_num(df, "range_q90_past", np.inf))
        ...
        compressed = _num(df, "bar_range_pct").shift(1).rolling(30, min_periods=10).mean() < _num(df, "range_q30_past", np.inf)
        ...
        long_m = rf_fast & rf_imb.gt(0.015) & (~_bool(df, "trend_down"))
        short_m = rf_fast & rf_imb.lt(-0.015) & (~_bool(df, "trend_up"))
    else:
...
def generate_specs(mode: str, max_specs: int | None = None) -> list[StrategySpec]:
    if mode == "smoke":
        # Smoke is an engine/data validation pass first.  Include simple live-valid
        # baseline signals that should fire on any healthy multi-year ETH 1m cache.
        # These are not claimed to be final edge; they prevent zero-signal audits.
        entries = [
            "ema20_cross",
            "vwap_cross",
            "ret_impulse_continuation",
...
def build_backtest_arrays(df: pd.DataFrame, cfg: RunConfig) -> BacktestArrays:
    ...
    start_ns = _window_start_ts(cfg.start_date).value
    end_ns = _window_end_ts(cfg.end_date).value
...
def build_signal_caches(
    ...
    raw_count_cache: dict[tuple[str, int, str], dict[str, int]] = {}
...
def write_data_diagnostics(bars: pd.DataFrame, df: pd.DataFrame, cfg: RunConfig, arrays: BacktestArrays, out_dir: Path) -> pd.DataFrame:
    ...
    for col in ["prior_high_120", "prior_low_120", "opening_range_high", "opening_range_low", "session_vwap", "ema_20", "ema_60", "range_q70_past", "vol_q75_past"]:
...
def write_raw_signal_diagnostics(raw_count_cache: dict[tuple[str, int, str], dict[str, int]], out_dir: Path) -> pd.DataFrame:
    rows = []
    for (entry_model, swing_window, side_mode), counts in raw_count_cache.items():
        rows.append({
    ...
    out = pd.DataFrame(rows).sort_values("raw_signals", ascending=False) if rows else pd.DataFrame()
...
def verify_fast_exactness(
    ...
    *,
    ...
    signal_cache: dict[tuple[str, int, str, str], np.ndarray],
    index_cache: dict[tuple[str, int, str, str], np.ndarray],
    count_cache: dict[tuple[str, int, str, str], dict[str, int]] | None,
    out_dir: Path,
) -> pd.DataFrame:
    ...
    if cfg.fail_on_empty_audit and total_verify_signals <= 0:
        pd.DataFrame([{
            ...
            "hint": "Check 00_data_diagnostics.csv and 09_raw_signal_diagnostics.csv; data coverage or signal conditions are wrong.",
...
def run_factory(cfg: RunConfig) -> None:
    ...
    if cfg.fail_on_empty_signals and total_full_signals <= 0:
        ...
        raise RuntimeError(
            ...
            f"Check {out_dir / '00_data_diagnostics.csv'} and {out_dir / '09_raw_signal_diagnostics.csv'}."
        )
    ...
    readme = f"""# ETH Intraday Strategy Factory V3.3 FAST Report
...
- `09_raw_signal_diagnostics.csv`: raw entry signal counts before regime filtering
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ETH intraday strategy factory V3.4 FAST, single-file CoinBacktest research script.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--out-dir", default="data/reports/research/eth_intraday_strategy_factory_v3_4_fast_audited")
    return p.parse_args()
