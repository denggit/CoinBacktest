#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""SOXL ICT R07: liquidity sweep -> structural MSS -> displacement leg -> FVG.
...
DEFAULT_OUT_DIR = "data/reports/research/ict/soxl/mss/r07_semantic_gap_atlas"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SOXL ICT R07 semantic-gap atlas for Sweep -> MSS -> FVG research",
...
def _design(args: argparse.Namespace, cfg: ResearchConfig, valid_days: int) -> str:
    return f"""# SOXL ICT R07 — Semantic Gap Atlas

## Purpose
R07 does **not** tighten the entry rules. It keeps the broad causal Sweep -> MSS -> FVG universe from R06, then describes each setup with interpretable path features to study why discretionary ICT chart selection may outperform the current mechanical translation.

## Frozen base setup
1. Liquidity families stay separate: premarket extreme, major 15m swing, and every causally confirmed unconsumed 1H/4H/1D swing active at 08:30.
2. Trade window is NY 08:30-16:30. Sweep episodes track the live terminal extreme.
3. MSS is a completed close through the latest causally confirmed opposing STH/STL. STH/STL formed after the terminal extreme is valid.
4. Displacement has **no strength gate**. FVG may form before/on MSS or complete afterwards while the terminal extreme remains intact.
5. Stop and target remain unchanged from R06 so this study isolates semantic selection rather than exit optimization.

## Semantic-gap features (diagnostics only)
R07 records sweep depth, terminal extension, time spent accepted beyond liquidity, time to reclaim liquidity, terminal retests, MSS-reference timing/shape, displacement speed/efficiency/body delivery, MSS overshoot, FVG count/timing/size/location, entry progress toward the opposite target, and the existing HTF age/distance/confluence context. None of these features filters a trade in R07.

## Outcome-path atlas
After replay only, R07 labels whether a filled trade reached +0.5R/+1R/+2R/+3R, hit target, stopped quickly, or reached +1R/+2R and later failed. These outcome labels are analysis-only and never enter signal construction.

## Anti-overfit protocol
Semantic feature bin edges are learned only on data through 2024 and frozen for 2025 forward and 2026 late holdout. R07 reports the whole response curve rather than selecting the best bucket as a strategy rule.

## Causality
All entry-semantic features have `semantic_feature_available_time <= signal_time`. Outcome fields are added only after replay. Higher-timeframe data continues to use available_time.
...
def _emit_platform_reports(base_lifecycle: pd.DataFrame, bars: pd.DataFrame, args: argparse.Namespace, out_dir: Path) -> None:
    ...
    for (tf, family), group in base_lifecycle.groupby(["execution_tf", "liquidity_family"], sort=True):
        ...
        print_full_report(
            ...
            capital=float(account["capital"].iloc[-1]), strategy_name=f"SOXL_ICT_R07_{tf}_{family}",
...
def run_research(bars: pd.DataFrame, args: argparse.Namespace) -> dict[str, Path]:
    ...
    stage = ProgressReporter(label="[research] build stages", total=9, every=1, enabled=not args.no_progress)
    ...
    # R07 deliberately does not filter the candidate universe here. These
    ...
    audit = pd.concat([causal_audit, semantic_audit], ignore_index=True, sort=False)
    ...
    (out_dir / "29_findings.md").write_text("\n".join(findings) + "\n", encoding="utf-8")

    manifest = {
        "experiment_id": "SOXL_ICT_MSS_R07_SEMANTIC_GAP_ATLAS",
        "edge_id": "SOXL_ICT_SWEEP_MSS_FVG_SEMANTIC_GAP",
        ...
        "semantic_gap_protocol": "no semantic feature is an entry gate; discovery edges <=2024, frozen validation in 2025/2026",
    ...
    _write_json(manifest, out_dir / "30_manifest.json")
    ...
    if not args.skip_review_pack:
        finalize_research_report(
            ...
            title="SOXL ICT R07 Semantic Gap Atlas", print_log=True,
        )
    return {"report_dir": out_dir, "review_pack": out_dir / "gpt_review_pack.zip"}

def run_self_test(args: argparse.Namespace) -> int:
    bars = make_synthetic_ict_day()
    with tempfile.TemporaryDirectory(prefix="soxl_ict_r07_") as tmp:
