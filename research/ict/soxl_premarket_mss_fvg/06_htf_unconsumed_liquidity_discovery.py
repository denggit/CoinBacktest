#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""SOXL ICT R06: liquidity sweep -> structural MSS -> displacement leg -> FVG.
...
DEFAULT_OUT_DIR = "data/reports/research/ict/soxl/mss/r06_htf_unconsumed_liquidity"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SOXL ICT R06 unconsumed HTF liquidity + MSS/FVG causal research",
...
def _design(args: argparse.Namespace, cfg: ResearchConfig, valid_days: int) -> str:
    return f"""# SOXL ICT R06 — unconsumed higher-timeframe liquidity discovery

## Purpose
R06 keeps R05 displacement discovery ungated and expands the liquidity universe. Premarket liquidity and remote unconsumed 1H/4H/1D swings are separate families and are never pooled when judging family-level edge.

## ICT process modeled
1. Freeze NY 04:00-08:30 premarket levels and independently carry every causally confirmed 1H/4H/1D swing that remains unconsumed at 08:30. No nearest-only shortcut is used.
...
Liquidity family is the primary comparison dimension. HTF age, distance, same-family stack count, and exact-minute cross-timeframe confluence remain diagnostics. Displacement quartiles are still learned only through 2024 and frozen for 2025/2026.
...
def _emit_platform_reports(base_lifecycle: pd.DataFrame, bars: pd.DataFrame, args: argparse.Namespace, out_dir: Path) -> None:
    ...
    for (tf, family), group in base_lifecycle.groupby(["execution_tf", "liquidity_family"], sort=True):
        ...
        print_full_report(
            ...
            capital=float(account["capital"].iloc[-1]), strategy_name=f"SOXL_ICT_R06_{tf}_{family}",
...
def run_research(bars: pd.DataFrame, args: argparse.Namespace) -> dict[str, Path]:
    ...
    stage = ProgressReporter(label="[research] build stages", total=7, every=1, enabled=not args.no_progress)
    ...
    (out_dir / "24_findings.md").write_text("\n".join(findings) + "\n", encoding="utf-8")

    manifest = {
        "experiment_id": "SOXL_ICT_MSS_R06_HTF_UNCONSUMED_LIQUIDITY",
        "edge_id": "SOXL_ICT_SWEEP_MSS_FVG_MULTI_LIQUIDITY",
    ...
    _write_json(manifest, out_dir / "25_manifest.json")
    ...
    if not args.skip_review_pack:
        finalize_research_report(
            ...
            title="SOXL ICT R06 Unconsumed HTF Liquidity", print_log=True,
        )
