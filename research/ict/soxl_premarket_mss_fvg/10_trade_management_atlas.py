#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""SOXL ICT R10: multi-timeframe structural trade-management atlas for Sweep -> MSS -> FVG.
...
from src.research_common.ict.trade_management import (  # noqa: E402
    TradeManagementConfig,
    replay_management_scenarios,
    management_summary_table,
    management_period_table,
    structure_target_availability_table,
)
...
DEFAULT_OUT_DIR = "data/reports/research/ict/soxl/mss/r10_trade_management_atlas"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SOXL ICT R10 multi-timeframe structural trade-management research",
    ...
    p.add_argument("--management-structure-timeframes", default="1,2,5,15")
    p.add_argument("--management-runner-timeframes", default="2,5,15")
    p.add_argument("--management-pivot-left", type=int, default=1)
    p.add_argument("--management-pivot-right", type=int, default=1)
    p.add_argument("--management-internal-partial-fraction", type=float, default=0.50)
    p.add_argument("--management-main-target-fraction", type=float, default=0.80)
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    mtfs = _csv_numbers(args.management_structure_timeframes, cast=int)
    if any(tf not in {1, 2, 5, 15} for tf in mtfs):
        raise ValueError("management_structure_timeframes must be a subset of 1,2,5,15")
    rtfs = _csv_numbers(args.management_runner_timeframes, cast=int)
    if any(tf not in {2, 5, 15} for tf in rtfs):
        raise ValueError("management_runner_timeframes must be a subset of 2,5,15")
    if int(args.management_pivot_left) < 1 or int(args.management_pivot_right) < 1:
        raise ValueError("management pivot left/right must be >= 1")
    if not 0 < float(args.management_internal_partial_fraction) <= 1:
        raise ValueError("management_internal_partial_fraction must be in (0,1]")
    if not 0 < float(args.management_main_target_fraction) < 1:
        raise ValueError("management_main_target_fraction must be in (0,1)")
...
def _management_cfg(args: argparse.Namespace) -> TradeManagementConfig:
    return TradeManagementConfig(
        structure_timeframes=tuple(_csv_numbers(args.management_structure_timeframes, cast=int)),
        pivot_left=int(args.management_pivot_left),
        pivot_right=int(args.management_pivot_right),
        internal_partial_fraction=float(args.management_internal_partial_fraction),
        main_target_fraction=float(args.management_main_target_fraction),
        runner_timeframes=tuple(_csv_numbers(args.management_runner_timeframes, cast=int)),
    )
...
def _design(args: argparse.Namespace, cfg: ResearchConfig, valid_days: int) -> str:
    return f"""# SOXL ICT R10 — Multi-Timeframe Structural Trade Management Atlas

## Purpose
R10 freezes the broad R09 Sweep -> MSS -> FVG entry universe. It does not add any entry filter. The sole purpose is to study whether multi-timeframe structure can convert correctly-moving trades into better realized PnL, lower drawdown and smoother monthly equity without sacrificing opportunity frequency.
...
## Frozen entry
Liquidity/MSS/FVG/order fill/initial terminal-extreme stop semantics remain unchanged from R09. EQH/EQL remains only an ordinary liquidity/context family; R10 does not require it.

## Management research
R10 compares a small pre-declared set of post-fill mechanisms: 2m/5m/15m internal structural partials, a causal ITH/ITL cost-cover partial, and 2m/5m/15m structure-trailing runners after the original opposite-liquidity target. The 50% internal partial and 80% main-target partial are neutral research anchors, not optimized final parameters. `cost_cover` computes the minimum fraction that would approximately offset a later full initial-stop loss including round-trip cost at the observed structural target R. All pivots become usable only at their causal confirmation available time.
...
def run_research(bars: pd.DataFrame, args: argparse.Namespace) -> dict[str, Path]:
    ...
    management_cfg = _management_cfg(args)
    ...
    stage = ProgressReporter(label="[research] build stages", total=15, every=1, enabled=not args.no_progress)
    ...
    management_progress = ProgressReporter(
        label="[management] structural replay",
        total=max(1, int(len(base_lifecycle.loc[base_lifecycle["filled"].fillna(False).astype(bool)]) if not base_lifecycle.empty else 0) * (7 + 2 * len(_csv_numbers(args.cost_multipliers, cast=float)))),
        every=250,
        enabled=not args.no_progress,
    )
    managed = replay_management_scenarios(
        bars, base_lifecycle, round_trip_cost=cfg.round_trip_cost,
        cost_multipliers=tuple(_csv_numbers(args.cost_multipliers, cast=float)),
        config=management_cfg, progress=management_progress,
    ) if not base_lifecycle.empty else pd.DataFrame()
    management_progress.close()
    management_summary = management_summary_table(managed, initial_capital=float(args.initial_capital))
    management_periods = management_period_table(managed)
    target_availability = structure_target_availability_table(managed)
    preservation_rows = []
    if not managed.empty:
        baseline_ids = set(base_lifecycle.loc[base_lifecycle["filled"].fillna(False).astype(bool), "attempt_id"].astype(str))
        base_cost = managed.loc[managed["management_cost_multiple"] == 1.0]
        for scenario, g in base_cost.groupby("management_scenario", sort=True):
            ids = set(g["attempt_id"].astype(str))
            preservation_rows.append({
                "management_scenario": scenario, "baseline_filled_trades": len(baseline_ids),
                "scenario_trades": len(g), "unique_attempts": len(ids),
                "same_entry_universe": ids == baseline_ids,
            })
    management_preservation = pd.DataFrame(preservation_rows)
    stage.update(15)
    stage.close()
    ...
    _write_csv(managed, out_dir / "39_trade_management_lifecycle.csv")
    _write_csv(management_summary, out_dir / "40_trade_management_scenario_summary.csv")
    _write_csv(management_periods, out_dir / "41_trade_management_period_validation.csv")
    _write_csv(target_availability, out_dir / "42_structure_target_availability.csv")
    _write_csv(management_preservation, out_dir / "43_management_entry_preservation_audit.csv")

    findings = [
        "# R10 Multi-Timeframe Trade Management Findings", "",
    ...
    if not management_summary.empty:
        best = management_summary.loc[management_summary["management_cost_multiple"] == 1.0].sort_values(["profit_factor", "positive_month_rate"], ascending=False).head(10)
        findings.append("\n## R10 management research\n")
        findings.append("Entries are frozen from R09. Management scenarios are compared on identical filled attempts; any row with `same_entry_universe=False` invalidates the comparison.")
        for row in best.to_dict("records"):
            findings.append(f"- {row['execution_tf']} {row['liquidity_family']} {row['management_scenario']}: trades={int(row['trades'])}, PF={row['profit_factor']:.3f}, MDD={row['account_max_drawdown']:.3%}, positive_month_rate={row['positive_month_rate']:.1%}")
    (out_dir / "44_findings.md").write_text("\n".join(findings) + "\n", encoding="utf-8")

    manifest = {
        "experiment_id": "SOXL_ICT_MSS_R10_TRADE_MANAGEMENT_ATLAS",
        "edge_id": "SOXL_ICT_SWEEP_MSS_FVG_TRADE_MANAGEMENT",
        ...
        "management_protocol": "R09 entries frozen; compare internal 2m/5m/15m partials, causal ITH/ITL cost-cover partial, and opposite-target 80% + 2m/5m/15m structural runner",
    }
    _write_json(manifest, out_dir / "45_manifest.json")
    ...
    if not args.skip_review_pack:
        finalize_research_report(
            ...
            title="SOXL ICT R10 Multi-Timeframe Trade Management Atlas", print_log=True,
        )
    return {"report_dir": out_dir, "review_pack": out_dir / "gpt_review_pack.zip"}

def run_self_test(args: argparse.Namespace) -> int:
    bars = make_synthetic_ict_day()
    with tempfile.TemporaryDirectory(prefix="soxl_ict_r10_") as tmp:
        ...
        if not (result["report_dir"] / "43_management_entry_preservation_audit.csv").exists():
            raise AssertionError("missing management entry preservation audit")
        pres = pd.read_csv(result["report_dir"] / "43_management_entry_preservation_audit.csv")
        if pres.empty or not pres["same_entry_universe"].fillna(False).all():
            raise AssertionError("R10 management scenarios changed the frozen R09 entry universe")
