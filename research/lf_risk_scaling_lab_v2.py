...
SCRIPT_NAME = "LFRiskScalingLabV2"


@dataclass
class Config:
    trades_csv: Path
    out_dir: Path = Path("data/reports/research/lf_risk_scaling_lab_v2")
...
EQUITY_AFTER_CANDIDATES = ["equity_after", "capital_after", "balance_after", "end_capital", "capital"]
...
def load_trade_returns(cfg: Config) -> pd.DataFrame:
    ...
    eq_before_col = _find_col(raw, EQUITY_BEFORE_CANDIDATES)
    eq_after_col = _find_col(raw, EQUITY_AFTER_CANDIDATES)

    # Prefer realized account return from PnL and capital/equity columns when possible.
    # Many report files use a column named return_pct but store it as a decimal fraction
    # rather than percentage points. PnL/capital is less ambiguous.
    if pnl_col is not None and eq_before_col is not None:
        pnl = pd.to_numeric(raw[pnl_col], errors="coerce")
        eq_before = pd.to_numeric(raw[eq_before_col], errors="coerce").replace(0, np.nan)
        base_return = pnl / eq_before
        source = f"{pnl_col}/{eq_before_col}"
    elif pnl_col is not None and eq_after_col is not None:
        pnl = pd.to_numeric(raw[pnl_col], errors="coerce")
        eq_after = pd.to_numeric(raw[eq_after_col], errors="coerce")
        eq_before = (eq_after - pnl).replace(0, np.nan)
        base_return = pnl / eq_before
        source = f"{pnl_col}/({eq_after_col}-{pnl_col})"
    else:
        ret_col = _find_col(raw, RETURN_CANDIDATES)
        if ret_col is not None:
            ret = pd.to_numeric(raw[ret_col], errors="coerce")
            finite_abs_max = ret.replace([np.inf, -np.inf], np.nan).abs().max()
            if pd.notna(finite_abs_max) and finite_abs_max <= 2.0:
                base_return = ret
                source = f"{ret_col} as_fraction"
            else:
                base_return = ret / 100.0
            ...
            if r_col is None:
                raise RuntimeError(
                    ...
                    f"pnl+equity/capital columns, return columns={RETURN_CANDIDATES}, or R columns={R_CANDIDATES}."
                )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", default="data/reports/research/lf_risk_scaling_lab_v2")
...
def main() -> None:
    ...
    trades.to_csv(cfg.out_dir / "lf_risk_scaling_lab_v2_trade_returns.csv", index=False)
    summary.to_csv(cfg.out_dir / "lf_risk_scaling_lab_v2_summary.csv", index=False)
    yearly.to_csv(cfg.out_dir / "lf_risk_scaling_lab_v2_yearly.csv", index=False)
    monthly.to_csv(cfg.out_dir / "lf_risk_scaling_lab_v2_monthly.csv", index=False)
    mc.to_csv(cfg.out_dir / "lf_risk_scaling_lab_v2_monte_carlo.csv", index=False)
    (cfg.out_dir / "lf_risk_scaling_lab_v2_config.json").write_text(
