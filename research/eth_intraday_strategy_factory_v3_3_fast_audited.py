#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH Intraday Strategy Factory V3.2 FAST AUDITED - single-file CoinBacktest research script.
...
---------------------------------------
python research/eth_intraday_strategy_factory_v3_3_fast_audited.py --mode smoke --max-specs 30 --out-dir data/reports/research/eth_intraday_factory_v3_1_smoke
python research/eth_intraday_strategy_factory_v3_3_fast_audited.py --mode core --max-specs 400 --out-dir data/reports/research/eth_intraday_factory_v3_1_core
python research/eth_intraday_strategy_factory_v3_3_fast_audited.py --mode wide --max-specs 1200 --robustness-top-n 80 --write-trades --out-dir data/reports/research/eth_intraday_factory_v3_1_wide
...
SCRIPT_NAME = "eth_intraday_strategy_factory_v3_3_fast_audited"
...
class RunConfig:
    ...
    out_dir: str = "data/reports/research/eth_intraday_strategy_factory_v3_3_fast_audited"
...
def generate_specs(mode: str, max_specs: int | None = None) -> list[StrategySpec]:
    if mode == "smoke":
        # Smoke must include at least a few high-fire opportunity signals, otherwise
        # fast-vs-slow audit can falsely pass on a zero-trade sample.
        entries = [
            "sweep_reclaim",
            "trend_pullback_reclaim",
            "vwap_deviation_reversion",
...
def write_data_diagnostics(bars: pd.DataFrame, df: pd.DataFrame, cfg: RunConfig, arrays: BacktestArrays, out_dir: Path) -> pd.DataFrame:
    ...
    idx = pd.DatetimeIndex(pd.to_datetime(df.index).tz_localize(None))
    start_ts = pd.Timestamp(cfg.start_date).tz_localize(None)
    end_ts = pd.Timestamp(cfg.end_date).tz_localize(None)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ETH intraday strategy factory V3.3 FAST, single-file CoinBacktest research script.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--out-dir", default="data/reports/research/eth_intraday_strategy_factory_v3_3_fast_audited")
    return p.parse_args()
