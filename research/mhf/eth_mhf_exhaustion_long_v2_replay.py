#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH MHF Exhaustion Long V2 Replay: focused replay validation for OKX trade bars.
...
V2 replays the fixed V1.1 research-continue exhaustion/reclaim long combinations.
It does not add new event families, tune exits, or promote a tradable edge.
The goal is to validate cost/delay, matched-baseline excess, yearly stability,
and MHF frequency before any formal backtest.
...
SCRIPT_NAME = "eth_mhf_exhaustion_long_v2_replay.py"
SCRIPT_VERSION = "2.0.1"
EXPERIMENT_ID = "ETH_MHF_EXHAUSTION_LONG_V2_REPLAY"
EDGE_ID = "ETH_EDGE_MHF_EXHAUSTION_LONG_RESEARCH_V2"
DEFAULT_OUT_DIR = "data/reports/research/mhf_exhaustion_long_v2_replay"
CAUSAL_POLICY = (
...
# Fixed, pre-declared V2 replay combos. These are V1.1 research-continue lines,
# not a new parameter search. The w120 strict/deep item is kept as a control.
REPLAY_COMBOS: tuple[ReplayCombo, ...] = (
    ReplayCombo(
        "A_impulse_deep15_normal_down_h120",
        "impulse_exhaustion_reversal__deep_15m__long",
        "regime",
        "normal_down",
        120,
        "V1.1 strongest matched-baseline excess line: impulse deep 15m long under normal_down regime.",
    ),
    ReplayCombo(
        "B_impulse_deep15_asia_h240",
        "impulse_exhaustion_reversal__deep_15m__long",
        "session",
        "asia_00_08",
        240,
        "V1.1 session line with positive fee2/delay and matched excess; tests whether it is more than Asia drift.",
    ),
    ReplayCombo(
        "C_forced_w60_asia_europe_h120",
        "forced_flow_exhaustion_reclaim__w60_strict__long",
        "session",
        "asia_europe_08_16",
        120,
        "V1.1 forced-flow strict line with best fee2/delay balance in Asia-Europe overlap.",
    ),
    ReplayCombo(
        "D_forced_w60_normal_down_h15",
        "forced_flow_exhaustion_reclaim__w60_strict__long",
        "regime",
        "normal_down",
        15,
        "Shorter-horizon MHF-like forced-flow line under normal_down regime.",
    ),
    ReplayCombo(
        "E_forced_w60_asia_h60",
        "forced_flow_exhaustion_reclaim__w60_strict__long",
        "session",
        "asia_00_08",
        60,
        "Higher-count forced-flow Asia line; included to audit whether trade frequency improves enough.",
    ),
    ReplayCombo(
        "F_forced_w120_deep_normal_down_control_h120",
        "forced_flow_exhaustion_reclaim__w120_deep__long",
        "regime",
        "normal_down",
        120,
        "Pre-declared control: deeper forced-flow window to test whether the structure survives wider sweep context.",
    ),
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Research-only ETH MHF Exhaustion Long V2 replay validation over OKX trade bars.",
    ...
    p.add_argument("--primary-horizon", type=int, default=60, help="Compatibility horizon for V1-style summary tables only; V2 uses fixed pre-declared replay combos and horizons.")
...
def build_event_specs() -> list[EventSpec]:
    """Build only the pre-declared exhaustion/reclaim long specs used by V2 replay.

    V2 deliberately does not add new event families or broad parameter grids. It
    replays the V1.1 research-continue lines plus one wider-window forced-flow
    control, so the result tests robustness instead of optimizing for a prettier
    report.
    """
    ...
    return [
        EventSpec("impulse_exhaustion_reversal", "deep_15m", "LONG", impulse_variants["deep_15m"]),
        EventSpec("forced_flow_exhaustion_reclaim", "w60_strict", "LONG", forced_variants["w60_strict"]),
        EventSpec("forced_flow_exhaustion_reclaim", "w120_deep", "LONG", forced_variants["w120_deep"]),
    ]
...
def _combo_condition_label(combo: ReplayCombo) -> str:
    return f"{combo.condition_type}={combo.condition_value}"


def _select_combo_events(events: pd.DataFrame, combo: ReplayCombo) -> pd.DataFrame:
    if events.empty:
        return events.copy()
    part = events.loc[events["event_name"].astype(str).eq(combo.event_name)].copy()
    if part.empty:
        return part
    direction = str(part["direction"].iloc[0]).upper()
    mask = condition_mask(part, direction=direction, condition_type=combo.condition_type, condition_value=combo.condition_value)
    return part.loc[mask].copy()


def _combo_frequency_stats(part: pd.DataFrame, *, start_date: str, end_date: str) -> dict[str, object]:
    total = int(len(part))
    years_span = _study_year_span(start_date, end_date)
    total_months = _study_month_count(start_date, end_date)
    if part.empty:
        return {
            "events_per_year": 0.0,
            ...
            "min_year_count": 0,
            "max_year_count": 0,
            "max_days_without_event": max_days_without_event(pd.Series(dtype="datetime64[ns]"), start_date, end_date),
        }
    times = pd.to_datetime(part["signal_time"], errors="coerce")
    ym = times.dt.to_period("M")
    active_months = int(ym.dropna().nunique())
    y_counts = part.groupby("year", dropna=False).size() if "year" in part.columns else pd.Series(dtype=int)
    return {
        "events_per_year": float(total / years_span),
        "events_per_month": float(total / max(1, total_months)),
        "active_months": active_months,
        "month_coverage": float(active_months / max(1, total_months)),
        "min_year_count": int(y_counts.min()) if not y_counts.empty else 0,
        "max_year_count": int(y_counts.max()) if not y_counts.empty else 0,
        "max_days_without_event": max_days_without_event(part["signal_time"], start_date, end_date),
    }


def _prefix_stats(stats: ReturnStats, prefix: str = "") -> dict[str, object]:
    return {
        f"{prefix}count": stats.count,
        f"{prefix}mean_net": stats.mean,
        f"{prefix}median_net": stats.median,
        f"{prefix}win_rate": stats.win_rate,
        f"{prefix}profit_factor": stats.profit_factor,
        f"{prefix}top5_winner_share": stats.top5_winner_share,
        f"{prefix}p05": stats.p05,
...
def _combo_decision(
    *,
    ...
    profit_factor: float,
    fee2_mean_net: float,
    delay1_mean_net: float,
    matched_excess_mean_net: float,
    baseline_match_rate: float,
    positive_years: int,
    top5_winner_share: float,
    events_per_year: float,
    month_coverage: float,
    min_count: int,
    ...
    if not np.isfinite(mean_net) or mean_net <= 0:
        failed.append("mean_net<=0")
    if not np.isfinite(profit_factor) or profit_factor < 1.10:
        failed.append("profit_factor<1.10")
    if not np.isfinite(fee2_mean_net) or fee2_mean_net <= -0.0005:
        failed.append("fee2_mean_net<=-0.0005")
    if not np.isfinite(delay1_mean_net) or delay1_mean_net <= -0.0005:
        failed.append("delay1_mean_net<=-0.0005")
    if not np.isfinite(matched_excess_mean_net) or matched_excess_mean_net <= 0:
        failed.append("matched_excess_mean_net<=0")
    if not np.isfinite(baseline_match_rate) or baseline_match_rate < 0.80:
        failed.append("baseline_match_rate<0.80")
    if int(positive_years) < 2:
        failed.append("positive_years<2")
    if np.isfinite(top5_winner_share) and top5_winner_share > 0.45:
        failed.append("top5_winner_share>0.45")
    if not np.isfinite(events_per_year) or events_per_year < float(min_events_per_year):
        failed.append(f"events_per_year<{min_events_per_year:g}")
    if not np.isfinite(month_coverage) or month_coverage < float(min_month_coverage):
        failed.append(f"month_coverage<{min_month_coverage:g}")
    if failed:
        return "rejected_or_research_continue_only", ";".join(failed)
    return "research_continue_replay_pass", "pass_fixed_replay_research_filters"
...
def build_replay_reports(
    ...
    *,
    ...
    """Build fixed-combo replay reports.

    This function intentionally evaluates only the pre-declared ReplayCombo list. It is not a
    parameter search and does not choose a best horizon after seeing results.
    """
    summary_rows: list[dict[str, object]] = []
    trade_frames: list[pd.DataFrame] = []
    yearly_rows: list[dict[str, object]] = []
    session_regime_rows: list[dict[str, object]] = []
    cost_rows: list[dict[str, object]] = []
    delay_rows: list[dict[str, object]] = []
    baseline_rows: list[dict[str, object]] = []
    selected_signal_sets: dict[str, set[int]] = {}

    bars_c = _bars_cache(bars)
    baseline_cache: dict[tuple[str, str, str], dict[tuple[object, ...], np.ndarray]] = {}
    progress = ProgressReporter(label="[aggregate] replay combos", total=max(1, len(combos)), every=1)

    for i, combo in enumerate(combos, start=1):
        part = _select_combo_events(events, combo)
        horizon = int(combo.horizon)
        direction = str(part["direction"].iloc[0]).upper() if not part.empty else "LONG"
        family = str(part["family"].iloc[0]) if not part.empty and "family" in part.columns else ""
        ret_col = f"delay0_h{horizon}_net_c1"
        gross_col = f"delay0_h{horizon}_gross"
        valid_col = f"delay0_h{horizon}_valid"
        valid_part = part.loc[part[valid_col].astype(bool)].copy() if (not part.empty and valid_col in part.columns) else part.copy()
        stats = summarize_return_series(valid_part[ret_col]) if ret_col in valid_part.columns else ReturnStats(0, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
        positive_years, year_count = _positive_years(valid_part, ret_col)
        freq = _combo_frequency_stats(valid_part, start_date=start_date, end_date=end_date)

        # Cost stress for the fixed horizon.
        fee2_mean = np.nan
        for mult in cost_multipliers:
            if gross_col in valid_part.columns:
                vals = pd.to_numeric(valid_part[gross_col], errors="coerce") - float(round_trip_cost_pct) * float(mult)
                st = summarize_return_series(vals)
            else:
                st = ReturnStats(0, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
            if np.isclose(float(mult), 2.0):
                fee2_mean = st.mean
            cost_rows.append({
                "combo_id": combo.combo_id,
                "event_name": combo.event_name,
                "condition_type": combo.condition_type,
                "condition_value": combo.condition_value,
                "horizon": horizon,
                "cost_multiplier": float(mult),
                **_prefix_stats(st),
            })

        # Delay stress for the fixed horizon.
        delay1_mean = np.nan
        selected_idx = valid_part.index
        for delay in delay_bars_list:
            ddf = delayed_events.get(int(delay))
            dcol = f"delay{int(delay)}_h{horizon}_net_c1"
            dvalid = f"delay{int(delay)}_h{horizon}_valid"
            if ddf is not None and dcol in ddf.columns and len(selected_idx) > 0:
                dpart = ddf.loc[selected_idx]
                dvalid_mask = dpart[dvalid].astype(bool) if dvalid in dpart.columns else pd.Series(True, index=dpart.index)
                st = summarize_return_series(dpart.loc[dvalid_mask, dcol])
            else:
                st = ReturnStats(0, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan)
            if int(delay) == 1:
                delay1_mean = st.mean
        ...
        baseline_rows.append({
            ...
            "event_mean_net": stats.mean,
            **baseline,
        })

        decision, reason = _combo_decision(
            count=stats.count,
            mean_net=stats.mean,
            profit_factor=stats.profit_factor,
            fee2_mean_net=fee2_mean,
            delay1_mean_net=delay1_mean,
            matched_excess_mean_net=_finite_float(baseline.get("matched_excess_mean_net")),
            baseline_match_rate=_finite_float(baseline.get("baseline_match_rate"), 0.0),
            positive_years=positive_years,
            top5_winner_share=stats.top5_winner_share,
            events_per_year=_finite_float(freq.get("events_per_year"), 0.0),
            month_coverage=_finite_float(freq.get("month_coverage"), 0.0),
            min_count=int(min_count),
            min_events_per_year=float(min_events_per_year),
            min_month_coverage=float(min_month_coverage),
        )
        ...
        summary_rows.append({
            ...
            "condition_type": combo.condition_type,
            "condition_value": combo.condition_value,
            "condition_label": _combo_condition_label(combo),
            "horizon": horizon,
            "rationale": combo.rationale,
            **_prefix_stats(stats),
            "fee2_mean_net": fee2_mean,
            "delay1_mean_net": delay1_mean,
            "matched_excess_mean_net": baseline.get("matched_excess_mean_net", np.nan),
            "baseline_mean_net": baseline.get("baseline_mean_net", np.nan),
            "baseline_p_value": baseline.get("baseline_p_value", np.nan),
            "baseline_match_rate": baseline.get("baseline_match_rate", np.nan),
            "positive_years": positive_years,
            "year_count": year_count,
            **freq,
            "decision": decision,
        ...
        if not valid_part.empty:
            mfe_mae_cols = [c for c in valid_part.columns if c.startswith("delay0_mfe_h") or c.startswith("delay0_mae_h")]
            trade_cols = [
                "event_name", "family", "variant", "direction", "side", "signal_idx", "signal_time", "session", "year", "month", "weekday", "regime", "volatility_bucket",
                "delay0_entry_time", "delay0_entry_price", f"delay0_h{horizon}_exit_time", gross_col, ret_col, *mfe_mae_cols,
            ]
            tf = valid_part[[c for c in trade_cols if c in valid_part.columns]].copy()
            tf.insert(0, "combo_id", combo.combo_id)
            tf.insert(1, "combo_horizon", horizon)
            tf.insert(2, "condition_type", combo.condition_type)
            tf.insert(3, "condition_value", combo.condition_value)
            trade_frames.append(tf)
            selected_signal_sets[combo.combo_id] = set(pd.to_numeric(valid_part["signal_idx"], errors="coerce").dropna().astype(int).tolist())
        else:
            selected_signal_sets[combo.combo_id] = set()

        if not valid_part.empty and ret_col in valid_part.columns:
            for year, g in valid_part.groupby("year", dropna=False, sort=True):
                st = summarize_return_series(g[ret_col])
                yearly_rows.append({"combo_id": combo.combo_id, "event_name": combo.event_name, "horizon": horizon, "year": year, **_prefix_stats(st)})
            group_cols = [c for c in ["session", "regime"] if c in valid_part.columns]
            if group_cols:
                for key, g in valid_part.groupby(group_cols, dropna=False, sort=True):
                    kt = key if isinstance(key, tuple) else (key,)
                    row = {"combo_id": combo.combo_id, "event_name": combo.event_name, "horizon": horizon}
                    row.update(dict(zip(group_cols, kt)))
                    row.update(_prefix_stats(summarize_return_series(g[ret_col])))
                    session_regime_rows.append(row)
    ...
    for i, c1 in enumerate(combos):
        s1 = selected_signal_sets.get(c1.combo_id, set())
        for c2 in combos[i + 1:]:
            s2 = selected_signal_sets.get(c2.combo_id, set())
            inter = len(s1 & s2)
            union = len(s1 | s2)
            overlap_rows.append({
                "combo_id_left": c1.combo_id,
                "combo_id_right": c2.combo_id,
                "left_count": len(s1),
                "right_count": len(s2),
                "overlap_count": inter,
                "overlap_share_left": float(inter / len(s1)) if s1 else np.nan,
                "overlap_share_right": float(inter / len(s2)) if s2 else np.nan,
                "jaccard": float(inter / union) if union else np.nan,
            })

    replay_summary = pd.DataFrame(summary_rows)
    replay_trades = pd.concat(trade_frames, ignore_index=True) if trade_frames else pd.DataFrame()
    yearly = pd.DataFrame(yearly_rows)
    session_regime = pd.DataFrame(session_regime_rows)
    cost_stress = pd.DataFrame(cost_rows)
    delay_stress = pd.DataFrame(delay_rows)
    baseline_summary = pd.DataFrame(baseline_rows)
    combo_overlap = pd.DataFrame(overlap_rows)
    if not replay_summary.empty:
        replay_summary = replay_summary.sort_values(["decision", "matched_excess_mean_net", "mean_net"], ascending=[True, False, False], kind="stable")
    return replay_summary, replay_trades, yearly, session_regime, cost_stress, delay_stress, baseline_summary, combo_overlap
...
def build_causal_audit(events: pd.DataFrame, bars: pd.DataFrame, primary_timeframe: str, primary_horizon: int, context_timeframes: tuple[str, ...]) -> pd.DataFrame:
    ...
    out["expected_entry_time"] = pd.to_datetime(expected, errors="coerce")
    comparable_entry = out["entry_time"].notna() & out["expected_entry_time"].notna()
    out["entry_not_next_open_flag"] = comparable_entry & (out["entry_time"] != out["expected_entry_time"])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    required_horizons = tuple(sorted(set([*horizons, *[c.horizon for c in REPLAY_COMBOS]])))
    ...
    print("[events] building fixed replay event masks", flush=True)
    ...
    print("[forward] fixed-hold replay outcomes", flush=True)
    ...
    print("[aggregate] delay stress precompute", flush=True)
    delay_stress_all, delayed_events = build_delay_stress(events, primary_raw, horizons, delay_bars_list, float(args.round_trip_cost_pct))
    ...
    print("[aggregate] replay combo reports", flush=True)
    replay_summary, replay_trades, yearly, session_regime, cost_stress, delay_stress, baseline_summary, combo_overlap = build_replay_reports(
        events,
        baseline_universe,
        primary_raw,
        delayed_events,
        combos=REPLAY_COMBOS,
        round_trip_cost_pct=float(args.round_trip_cost_pct),
        ...
        start_date=args.start_date,
        end_date=args.end_date,
        min_events_per_year=50.0,
        min_month_coverage=0.55,
        min_count=int(args.conditional_min_count),
    )
    ...
    if lookahead_count > 0:
        print(f"[aggregate] WARNING causal audit lookahead_count={lookahead_count:,}; replay decisions must be ignored until fixed", flush=True)
        if not replay_summary.empty:
            replay_summary["decision"] = "blocked_by_causal_audit"
            replay_summary["reason"] = replay_summary.get("reason", "").astype(str) + ";causal_lookahead_count_gt_0"
    ...
    manifest = {
        ...
        "replay_combos": [c.__dict__ for c in REPLAY_COMBOS],
        "mfe_mae_horizon": int(args.mfe_mae_horizon),
        "round_trip_cost_pct": float(args.round_trip_cost_pct),
        ...
        "replay_combo_count": int(len(REPLAY_COMBOS)),
        "replay_pass_count": int(replay_summary.get("decision", pd.Series(dtype=str)).astype(str).eq("research_continue_replay_pass").sum()) if not replay_summary.empty else 0,
        "causal_lookahead_count": lookahead_count,
        "frequency_policy": "V2 requires events_per_year>=50 and month_coverage>=0.55 for research_continue_replay_pass.",
        "causal_policy": CAUSAL_POLICY,
        "created_at": datetime.utcnow().isoformat(timespec="seconds") + "Z",
    }
    write_json(out_dir / "00_manifest.json", manifest)
    write_csv(out_dir / "01_replay_combo_summary.csv", replay_summary)
    write_csv(out_dir / "02_replay_trades.csv", replay_trades)
    write_csv(out_dir / "03_yearly_breakdown.csv", yearly)
    write_csv(out_dir / "04_session_regime_breakdown.csv", session_regime)
    write_csv(out_dir / "05_cost_stress.csv", cost_stress)
    write_csv(out_dir / "06_delay_stress.csv", delay_stress)
    write_csv(out_dir / "07_matched_baseline_summary.csv", baseline_summary)
    write_csv(out_dir / "08_combo_overlap.csv", combo_overlap)
    write_csv(out_dir / "09_all_event_delay_stress.csv", delay_stress_all)
    write_csv(out_dir / "10_research_decision.csv", replay_summary)
    write_csv(out_dir / "11_causal_audit.csv", causal_audit)
    sample_n = int(args.event_sample_size)
    write_csv(out_dir / "12_event_sample.csv", events.head(sample_n).copy() if sample_n > 0 else events.head(0).copy())
    write_json(
        out_dir / "13_feature_columns.json",
        {
    ...
    finalize_research_report(out_dir, experiment_id=EXPERIMENT_ID, edge_id=EDGE_ID, title="ETH MHF Exhaustion Long V2 Replay")
