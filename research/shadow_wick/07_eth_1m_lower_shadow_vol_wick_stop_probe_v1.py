#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""ETH 1m lower-shadow vol-regime wick-stop probe V1.

Research-only diagnostic probe for the strongest currently recorded raw-shadow lead:

    lower_volume_shadow_long + vol_regime in {mid_high_vol, extreme_vol}
    diagnostic horizon = 60 bars

The goal is not to immediately produce a final strategy. This script tests whether the lower-wick outside area is a valid structural failure boundary: hard touch stop, small buffered stop, and close-below-low stop. The 60-bar horizon is used only as an observation window to evaluate MFE/MAE and post-stop recovery/misfire risk, not as the desired production exit.
...
SCRIPT_NAME = "eth_1m_lower_shadow_vol_wick_stop_probe_v1"
SCRIPT_VERSION = "1.0.0"
EXPERIMENT_ID = "ETH_MF_1M_LOWER_SHADOW_VOL_WICK_STOP_PROBE_V1"
EDGE_ID = "RESEARCH_ONLY_ETH_MF_1M_LOWER_SHADOW_VOL_WICK_STOP_PROBE_V1"
TITLE = "ETH 1m Lower Shadow Vol-Regime Wick-Stop Probe V1"
DEFAULT_OUT_DIR = "data/reports/research/eth_1m_lower_shadow_vol_wick_stop_probe_v1"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Lower-shadow mid/high/extreme-vol wick-stop diagnostic: test lower-wick outside structural stops without turning horizon60 into a desired production exit.",
    ...
    p.add_argument("--wick-stop-buffer-pct", type=float, default=0.0002, help="Minimum buffer below event_low for buffered wick stop. 0.0002 = 0.02%%.")
    p.add_argument("--wick-stop-buffer-atr-mult", type=float, default=0.10, help="ATR-pct multiplier for buffered wick stop; buffer_pct=max(wick_stop_buffer_pct, atr_pct*mult).")
...
def build_profit_effect_labels(
    ...
    *,
    ...
    wick_stop_buffer_pct: float,
    wick_stop_buffer_atr_mult: float,
) -> pd.DataFrame:
    ...
    for j, delay in enumerate(entry_delays, start=1):
        ...
        # Wick-stop diagnostics. Horizon is only an observation window.
        # These columns answer: if we place the stop outside the event lower wick,
        # does it mostly cut bad trades, or does it often cut trades that later
        # produce meaningful MFE?
        atr_pct = pd.to_numeric(part.get("atr_pct", pd.Series(np.nan, index=part.index)), errors="coerce").to_numpy(dtype=float)
        buffer_pct = np.nanmax(
            np.vstack([
                np.full(len(part), float(wick_stop_buffer_pct), dtype=float),
                np.nan_to_num(atr_pct, nan=0.0) * float(wick_stop_buffer_atr_mult),
            ]),
            axis=0,
        )
        hard_stop_price = event_low.copy()
        buffered_stop_price = event_low * (1.0 - buffer_pct)
        hard_stop_mask = win_low <= hard_stop_price[:, None]
        buffered_stop_mask = win_low <= buffered_stop_price[:, None]
        close_below_stop_mask = win_close < event_low[:, None]

        stop_payloads = {
            "hard_wick_stop": (hard_stop_mask, hard_stop_price, "stop_price"),
            "buffered_wick_stop": (buffered_stop_mask, buffered_stop_price, "stop_price"),
            "close_below_wick_stop": (close_below_stop_mask, None, "close_price"),
        }
        stop_diag: dict[str, dict[str, np.ndarray]] = {}
        rows_n = len(part)
        arange_rows = np.arange(rows_n)
        for policy_name, (stop_mask, stop_price_arr, fill_mode) in stop_payloads.items():
            first_stop = _first_true_offset(stop_mask)
            hit = np.isfinite(first_stop)
            first_idx = np.zeros(rows_n, dtype=int)
            if hit.any():
                first_idx[hit] = first_stop[hit].astype(int)
            if fill_mode == "close_price":
                stop_exit_price = np.where(hit, win_close[arange_rows, first_idx], np.nan)
            else:
                stop_exit_price = np.asarray(stop_price_arr, dtype=float)
                stop_exit_price = np.where(hit, stop_exit_price, np.nan)
            diagnostic_exit_price = np.where(hit, stop_exit_price, exit_price)
            diagnostic_net = diagnostic_exit_price / entry_price - 1.0 - float(round_trip_cost_pct)
            stop_net = stop_exit_price / entry_price - 1.0 - float(round_trip_cost_pct)
            mfe_after_stop = np.full(rows_n, np.nan, dtype=float)
            mae_after_stop = np.full(rows_n, np.nan, dtype=float)
            close_end_after_stop = np.full(rows_n, np.nan, dtype=float)
            for ii in np.where(hit)[0]:
                start = int(first_idx[ii]) + 1
                if start <= h:
                    mfe_after_stop[ii] = np.nanmax(win_high[ii, start:]) / entry_price[ii] - 1.0
                    mae_after_stop[ii] = np.nanmin(win_low[ii, start:]) / entry_price[ii] - 1.0
                    close_end_after_stop[ii] = win_close[ii, -1] / entry_price[ii] - 1.0 - float(round_trip_cost_pct)
            stop_diag[policy_name] = {
                "hit": hit,
                "first_bars": first_stop,
                "stop_price": stop_exit_price,
                "stop_net": stop_net,
                "diagnostic_net": diagnostic_net,
                "mfe_after_stop": mfe_after_stop,
                "mae_after_stop": mae_after_stop,
                "end_net_after_stop": close_end_after_stop,
            }
        ...
        part["wick_stop_buffer_pct_used"] = buffer_pct
        for policy_name, payload in stop_diag.items():
            part[f"{policy_name}_hit"] = payload["hit"]
            part[f"{policy_name}_first_hit_bars"] = payload["first_bars"]
            part[f"{policy_name}_stop_price"] = payload["stop_price"]
            part[f"{policy_name}_stop_net_return"] = payload["stop_net"]
            part[f"{policy_name}_diagnostic_net_return"] = payload["diagnostic_net"]
            part[f"{policy_name}_mfe_after_stop"] = payload["mfe_after_stop"]
            part[f"{policy_name}_mae_after_stop"] = payload["mae_after_stop"]
            part[f"{policy_name}_end_net_after_stop"] = payload["end_net_after_stop"]
            part[f"{policy_name}_stop_then_mfe_ge_0p3"] = payload["mfe_after_stop"] >= float(fast_profit_threshold)
            part[f"{policy_name}_stop_then_mfe_ge_0p5"] = payload["mfe_after_stop"] >= float(strong_profit_threshold)
            part[f"{policy_name}_stop_then_mfe_ge_1p0"] = payload["mfe_after_stop"] >= 0.010
...
STOP_POLICIES = ("hard_wick_stop", "buffered_wick_stop", "close_below_wick_stop")


def _bool_rate_from_series(s: pd.Series) -> float:
    if s.empty:
        return float("nan")
    return float(s.fillna(False).astype(bool).mean())


def _summarize_no_stop(part: pd.DataFrame, policy: str) -> dict[str, float | int]:
    hit = part[f"{policy}_hit"].fillna(False).astype(bool)
    kept = part.loc[~hit]
    out: dict[str, float | int] = {
        "no_stop_trades": int(len(kept)),
        "no_stop_retention_rate": float((~hit).mean()) if len(part) else np.nan,
    }
    if kept.empty:
        for k in ["no_stop_mean_net_60", "no_stop_win_rate_60", "no_stop_pf_60", "no_stop_mfe_median", "no_stop_mae_median", "no_stop_bad_fast_adverse_rate", "no_stop_mfe_ge_0p5_rate"]:
            out[k] = np.nan
        return out
    ret = summarize_return(kept["net_return"])
    out.update({
        "no_stop_mean_net_60": ret["mean_net"],
        "no_stop_win_rate_60": ret["win_rate"],
        "no_stop_pf_60": ret["profit_factor"],
        "no_stop_total_return_60": ret["total_return"],
        "no_stop_max_drawdown_60": ret["max_drawdown"],
        "no_stop_mfe_median": float(pd.to_numeric(kept["mfe"], errors="coerce").median()),
        "no_stop_mae_median": float(pd.to_numeric(kept["mae"], errors="coerce").median()),
        "no_stop_bad_fast_adverse_rate": _rate(kept, "bad_fast_adverse_low_mfe"),
        "no_stop_mfe_ge_0p5_rate": _rate(kept, "mfe_ge_0p5"),
    })
    return out


def _wick_stop_score(row: pd.Series) -> float:
    trades = float(row.get("trades", 0) or 0)
    no_stop_trades = float(row.get("no_stop_trades", 0) or 0)
    stop_hit_rate = float(row.get("stop_hit_rate", np.nan))
    stop_good_cut_rate = float(row.get("stop_good_cut_rate", np.nan))
    stop_misfire_0p5_rate = float(row.get("stop_misfire_0p5_rate", np.nan))
    no_stop_wr = float(row.get("no_stop_win_rate_60", np.nan))
    no_stop_pf = float(row.get("no_stop_pf_60", np.nan))
    no_stop_bad = float(row.get("no_stop_bad_fast_adverse_rate", np.nan))
    diag_mean = float(row.get("diagnostic_mean_net", np.nan))
    base_mean = float(row.get("mean_net", np.nan))
    if not np.isfinite(trades) or trades <= 0:
        return float("nan")
    retention = no_stop_trades / trades if trades else 0.0
    retention_score = _clip01((retention - 0.55) / 0.35)
    win_score = _clip01((no_stop_wr - 0.55) / 0.15) if np.isfinite(no_stop_wr) else 0.0
    pf_score = _clip01((no_stop_pf - 1.20) / 0.70) if np.isfinite(no_stop_pf) else 0.0
    good_cut_score = _clip01(stop_good_cut_rate / 0.45) if np.isfinite(stop_good_cut_rate) else 0.0
    misfire_score = _clip01(1.0 - stop_misfire_0p5_rate / 0.45) if np.isfinite(stop_misfire_0p5_rate) else 0.0
    bad_score = _clip01(1.0 - no_stop_bad / 0.18) if np.isfinite(no_stop_bad) else 0.0
    hit_score = _clip01(1.0 - max(stop_hit_rate - 0.45, 0.0) / 0.35) if np.isfinite(stop_hit_rate) else 0.0
    lift = diag_mean - base_mean if np.isfinite(diag_mean) and np.isfinite(base_mean) else np.nan
    lift_score = _clip01((lift + 0.0002) / 0.0015) if np.isfinite(lift) else 0.0
    return float(0.18 * retention_score + 0.16 * win_score + 0.13 * pf_score + 0.15 * good_cut_score + 0.14 * misfire_score + 0.10 * bad_score + 0.07 * hit_score + 0.07 * lift_score)


def build_wick_stop_summary(labels: pd.DataFrame, directions: list[DirectionSpec]) -> pd.DataFrame:
    if labels.empty:
        return pd.DataFrame()
    rows = []
    specs = [DirectionSpec("base_mid_high_or_extreme_vol", "base_core", "All base events", "base", "base", pd.Series(True, index=labels.index))]
    specs += [s for s in directions if s.direction_name != "base_mid_high_or_extreme_vol"]
    for spec in specs:
        mask_all = spec.mask.reindex(labels.index).fillna(False).to_numpy(dtype=bool)
        for keys, part_delay in labels.groupby(["entry_delay_bars", "horizon_bars"], dropna=False):
            part = part_delay.loc[mask_all[part_delay.index]].copy()
            if part.empty:
                continue
            base_ret = summarize_return(part["net_return"])
            for policy in STOP_POLICIES:
                if f"{policy}_hit" not in part.columns:
                    continue
                hit = part[f"{policy}_hit"].fillna(False).astype(bool)
                stopped = part.loc[hit].copy()
                diag_ret = summarize_return(part[f"{policy}_diagnostic_net_return"])
                row = {
                    "direction_name": spec.direction_name,
                    "family": spec.family,
                    "entry_delay_bars": int(keys[0]),
                    "horizon_bars": int(keys[1]),
                    "stop_policy": policy,
                    "description": spec.description,
                    "trades": int(len(part)),
                    "mean_net": base_ret["mean_net"],
                    "win_rate": base_ret["win_rate"],
                    "profit_factor": base_ret["profit_factor"],
                    "max_drawdown": base_ret["max_drawdown"],
                    "diagnostic_mean_net": diag_ret["mean_net"],
                    "diagnostic_win_rate": diag_ret["win_rate"],
                    "diagnostic_profit_factor": diag_ret["profit_factor"],
                    "diagnostic_max_drawdown": diag_ret["max_drawdown"],
                    "diagnostic_mean_lift_vs_60bar_hold": diag_ret["mean_net"] - base_ret["mean_net"] if np.isfinite(diag_ret["mean_net"]) and np.isfinite(base_ret["mean_net"]) else np.nan,
                    "stop_hit_rate": float(hit.mean()) if len(part) else np.nan,
                    "stop_hit_trades": int(hit.sum()),
                    "stop_net_mean": float(pd.to_numeric(stopped.get(f"{policy}_stop_net_return", pd.Series(dtype=float)), errors="coerce").mean()) if not stopped.empty else np.nan,
                    "stop_first_hit_median_bars": float(pd.to_numeric(stopped.get(f"{policy}_first_hit_bars", pd.Series(dtype=float)), errors="coerce").median()) if not stopped.empty else np.nan,
                    "stop_hit_original_mean_net_60": float(pd.to_numeric(stopped.get("net_return", pd.Series(dtype=float)), errors="coerce").mean()) if not stopped.empty else np.nan,
                    "stop_hit_original_mfe_median": float(pd.to_numeric(stopped.get("mfe", pd.Series(dtype=float)), errors="coerce").median()) if not stopped.empty else np.nan,
                    "stop_mfe_after_stop_median": float(pd.to_numeric(stopped.get(f"{policy}_mfe_after_stop", pd.Series(dtype=float)), errors="coerce").median()) if not stopped.empty else np.nan,
                    "stop_misfire_0p3_rate": _rate(stopped, f"{policy}_stop_then_mfe_ge_0p3") if not stopped.empty else np.nan,
                    "stop_misfire_0p5_rate": _rate(stopped, f"{policy}_stop_then_mfe_ge_0p5") if not stopped.empty else np.nan,
                    "stop_misfire_1p0_rate": _rate(stopped, f"{policy}_stop_then_mfe_ge_1p0") if not stopped.empty else np.nan,
                    "stop_good_cut_rate": _rate(stopped, "bad_fast_adverse_low_mfe") if not stopped.empty else np.nan,
                    "bad_capture_rate": float((hit & part["bad_fast_adverse_low_mfe"].fillna(False).astype(bool)).sum() / max(int(part["bad_fast_adverse_low_mfe"].fillna(False).astype(bool).sum()), 1)),
                }
                row.update(_summarize_no_stop(part, policy))
                row["wick_stop_score"] = _wick_stop_score(pd.Series(row))
                rows.append(row)
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    return out.sort_values(["wick_stop_score", "trades"], ascending=[False, False]).reset_index(drop=True)


def build_wick_stop_yearly(labels: pd.DataFrame, directions: list[DirectionSpec]) -> pd.DataFrame:
    if labels.empty:
        return pd.DataFrame()
    specs = [DirectionSpec("base_mid_high_or_extreme_vol", "base_core", "All base events", "base", "base", pd.Series(True, index=labels.index))]
    specs += [s for s in directions if s.direction_name != "base_mid_high_or_extreme_vol"]
    ...
    for spec in specs:
        ...
        for keys, part_delay in labels.groupby(["entry_delay_bars", "horizon_bars"], dropna=False):
            ...
            if part.empty:
                continue
            part["year"] = years.loc[part.index].to_numpy()
            for year, part_year in part.groupby("year"):
                for policy in STOP_POLICIES:
                    if f"{policy}_hit" not in part_year.columns:
                        continue
                    hit = part_year[f"{policy}_hit"].fillna(False).astype(bool)
                    base_ret = summarize_return(part_year["net_return"])
                    diag_ret = summarize_return(part_year[f"{policy}_diagnostic_net_return"])
                    no_stop = _summarize_no_stop(part_year, policy)
                    rows.append({
                        "direction_name": spec.direction_name,
                        "family": spec.family,
                        "entry_delay_bars": int(keys[0]),
                        ...
                        "diagnostic_win_rate": diag_ret["win_rate"],
                        "diagnostic_profit_factor": diag_ret["profit_factor"],
                        "stop_hit_rate": float(hit.mean()) if len(part_year) else np.nan,
                        **no_stop,
                    })
    return pd.DataFrame(rows).sort_values(["direction_name", "entry_delay_bars", "year", "stop_policy"]).reset_index(drop=True) if rows else pd.DataFrame()


def build_stop_recovery_diagnostics(labels: pd.DataFrame) -> pd.DataFrame:
    if labels.empty:
        return pd.DataFrame()
    rows = []
    for keys, part in labels.groupby(["entry_delay_bars", "horizon_bars"], dropna=False):
        for policy in STOP_POLICIES:
            ...
            if hit_col not in part.columns:
                continue
            hit = part[hit_col].fillna(False).astype(bool)
            stopped = part.loc[hit]
            kept = part.loc[~hit]
            rows.append({
                "scope": "base_mid_high_or_extreme_vol",
                "entry_delay_bars": int(keys[0]),
                "horizon_bars": int(keys[1]),
                "stop_policy": policy,
                "trades": int(len(part)),
                "stop_hit_trades": int(hit.sum()),
                "no_stop_trades": int((~hit).sum()),
                "stop_hit_rate": float(hit.mean()) if len(part) else np.nan,
                "stop_hit_bad_fast_adverse_rate": _rate(stopped, "bad_fast_adverse_low_mfe") if not stopped.empty else np.nan,
                "no_stop_bad_fast_adverse_rate": _rate(kept, "bad_fast_adverse_low_mfe") if not kept.empty else np.nan,
                "stop_hit_mfe_median": _q(stopped, "mfe", 0.50) if not stopped.empty else np.nan,
                "no_stop_mfe_median": _q(kept, "mfe", 0.50) if not kept.empty else np.nan,
                "stop_hit_mfe_after_stop_median": _q(stopped, f"{policy}_mfe_after_stop", 0.50) if not stopped.empty else np.nan,
                "stop_hit_then_mfe_ge_0p3_rate": _rate(stopped, f"{policy}_stop_then_mfe_ge_0p3") if not stopped.empty else np.nan,
                "stop_hit_then_mfe_ge_0p5_rate": _rate(stopped, f"{policy}_stop_then_mfe_ge_0p5") if not stopped.empty else np.nan,
                "stop_hit_then_mfe_ge_1p0_rate": _rate(stopped, f"{policy}_stop_then_mfe_ge_1p0") if not stopped.empty else np.nan,
            })
    return pd.DataFrame(rows)


def build_wick_stop_candidates(stop_summary: pd.DataFrame, args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    if stop_summary.empty:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    out = stop_summary.copy()
    out["candidate_reason"] = np.select(
        [
            (out["no_stop_trades"] >= int(args.min_tier_a_trades))
            & (out["no_stop_win_rate_60"] >= 0.58)
            & (out["no_stop_pf_60"] >= 1.45)
            & (out["stop_misfire_0p5_rate"] <= 0.35)
            & (out["no_stop_bad_fast_adverse_rate"] <= 0.11),
            (out["no_stop_trades"] >= int(args.min_tier_b_trades))
            & (out["no_stop_win_rate_60"] >= 0.58)
            & (out["no_stop_pf_60"] >= 1.35)
            & (out["stop_good_cut_rate"] >= 0.25),
        ],
        ["research_continue_wick_stop_core", "diagnostic_wick_stop_candidate"],
        default="reject_or_diagnostic",
    )
    leaderboard = out.sort_values(["wick_stop_score", "no_stop_trades"], ascending=[False, False]).reset_index(drop=True)
    candidates = out.loc[out["candidate_reason"] != "reject_or_diagnostic"].sort_values(["candidate_reason", "wick_stop_score", "no_stop_trades"], ascending=[True, False, False]).reset_index(drop=True)
    rejected = out.loc[out["candidate_reason"] == "reject_or_diagnostic"].sort_values(["wick_stop_score", "no_stop_trades"], ascending=[False, False]).reset_index(drop=True)
    return leaderboard, candidates, rejected
...
def write_reports(
    *,
    ...
    print("[report] building wick-stop diagnostic reports", flush=True)
    ...
    manifest = {
        ...
        "horizon_bars_observation_only": args.horizon_bars,
        "entry_delay_bars_list": args.entry_delay_bars_list,
        "base_vol_regimes": args.base_vol_regimes,
        "wick_stop_buffer_pct": args.wick_stop_buffer_pct,
        "wick_stop_buffer_atr_mult": args.wick_stop_buffer_atr_mult,
        "research_policy": "Test lower-wick outside structural stop diagnostics inside the remembered lower-shadow mid/high/extreme-vol lead. Horizon60 is observation only, not desired production exit.",
        "notes": [
            "Main remembered lead: lower_volume_shadow_long + mid_high_vol/extreme_vol.",
            "This probe tests hard touch stop, small buffered stop, and close-below-low stop.",
            "A good wick stop should cut bad_fast_adverse_low_mfe paths while not frequently cutting trades that later generate meaningful MFE.",
            "Do not interpret diagnostic_net_return as final production strategy because profitable exits are not defined here.",
        ],
    }
    ...
    stop_summary = build_wick_stop_summary(labels, direction_specs)
    stop_yearly = build_wick_stop_yearly(labels, direction_specs)
    stop_recovery = build_stop_recovery_diagnostics(labels)
    stop_leaderboard, stop_candidates, stop_rejected = build_wick_stop_candidates(stop_summary, args)
    causal = build_causal_audit_summary(labels)
    feature_coverage = build_feature_coverage(labels)
    event_counts = pd.DataFrame([{"event_name": "lower_volume_shadow_long_mid_high_or_extreme_vol", "events": int(len(events)), "base_vol_regimes": args.base_vol_regimes, "directions": int(len(direction_specs)), "stop_policies": ",".join(STOP_POLICIES), "min_event_time": str(pd.to_datetime(events["event_bar_time"]).min()) if not events.empty else "", "max_event_time": str(pd.to_datetime(events["event_bar_time"]).max()) if not events.empty else ""}])
    ...
    _write_csv(_pct_columns(stop_summary), out_dir / "04_wick_stop_summary.csv")
    _write_csv(_pct_columns(stop_yearly), out_dir / "05_wick_stop_yearly.csv")
    _write_csv(_pct_columns(stop_recovery), out_dir / "06_stop_hit_recovery_diagnostics.csv")
    _write_csv(_pct_columns(stop_leaderboard.head(2000)), out_dir / "07_wick_stop_leaderboard.csv")
    _write_csv(_pct_columns(stop_candidates), out_dir / "08_research_continue_wick_stop_candidates.csv")
    _write_csv(_pct_columns(stop_rejected.head(5000)), out_dir / "09_rejected_or_diagnostic_wick_stops.csv")
    _write_csv(_pct_columns(direction_scorecard), out_dir / "10_direction_scorecard_context.csv")
    _write_csv(_pct_columns(direction_overlap), out_dir / "11_direction_overlap_delay0.csv")
    _write_csv(causal, out_dir / "12_causal_audit_summary.csv")
    sample_cols = [
        "event_id", "event_bar_time", "entry_delay_bars", "horizon_bars", "entry_time", "entry_price", "event_low", "event_high", "event_mid",
        "net_return", "mfe", "mae", "mfe_mae_ratio", "bad_fast_adverse_low_mfe", "adverse_sweep_count", "first_adverse_sweep_bars",
        "hard_wick_stop_hit", "hard_wick_stop_first_hit_bars", "hard_wick_stop_stop_net_return", "hard_wick_stop_mfe_after_stop", "hard_wick_stop_stop_then_mfe_ge_0p5",
        "buffered_wick_stop_hit", "buffered_wick_stop_first_hit_bars", "buffered_wick_stop_stop_net_return", "buffered_wick_stop_mfe_after_stop", "buffered_wick_stop_stop_then_mfe_ge_0p5",
        "close_below_wick_stop_hit", "close_below_wick_stop_first_hit_bars", "close_below_wick_stop_stop_net_return", "close_below_wick_stop_mfe_after_stop", "close_below_wick_stop_stop_then_mfe_ge_0p5",
        "range_pct", "volume_ratio", "event_wick_share", "event_wick_atr", "favorable_close_pos", "prior_move_against_120", "prior_move_against_720", "signed_delta_against_entry", "taker_against_entry", "session", "vol_regime", "trend_regime",
    ]
    sample_cols = [c for c in sample_cols if c in labels.columns]
    _write_csv(_sample(labels[sample_cols], int(args.event_sample_size)), out_dir / "13_stop_trade_sample.csv")
    if bool(args.write_slim_labels):
        _write_csv(labels[sample_cols], out_dir / "14_slim_wick_stop_labels.csv")
    readme = f"""# {TITLE}

Scope: `lower_volume_shadow_long` events inside `{args.base_vol_regimes}` only. `horizon={int(args.horizon_bars)}` is an observation window, not the intended production exit.

Stop policies tested:
- `hard_wick_stop`: touch `event_low` after entry.
- `buffered_wick_stop`: touch `event_low - max({args.wick_stop_buffer_pct:.6f}, atr_pct * {args.wick_stop_buffer_atr_mult:.3f})`.
- `close_below_wick_stop`: 1m close below `event_low`.

Reading order:
1. `12_causal_audit_summary.csv` — confirm no lookahead/context leakage.
2. `03_base_profit_effect_summary.csv` — remembered lead baseline.
3. `04_wick_stop_summary.csv` — main table: does each wick stop cut bad trades or misfire?
4. `06_stop_hit_recovery_diagnostics.csv` — how often stopped trades later recover MFE.
5. `07_wick_stop_leaderboard.csv` and `08_research_continue_wick_stop_candidates.csv` — candidates for next research.
6. `10_direction_scorecard_context.csv` — context for panic-depth/intensity/absorption directions.

Important: `diagnostic_net_return` uses stop if hit, otherwise 60bar close. It is diagnostic only because profitable exit is not defined here.
"""
    (out_dir / "README_RESEARCH.md").write_text(readme, encoding="utf-8")
    review_prompt = f"""You are reviewing {TITLE}.

Known lead to preserve: `lower_volume_shadow_long + mid_high_vol/extreme_vol`.

Review tasks:
1. Confirm `12_causal_audit_summary.csv` has no lookahead/context leakage.
2. Use `04_wick_stop_summary.csv` to determine whether hard/buffered/close-below wick stops are valid structural failure lines.
3. Focus on stop-hit quality: high `stop_good_cut_rate`, low `stop_misfire_0p5_rate`, improved `no_stop_win_rate_60`, improved `no_stop_pf_60`, and enough `no_stop_trades`.
4. Do not treat `diagnostic_net_return` as a final strategy because profitable exit is not defined.
5. Decide which stop policy/direction should be carried into the next candidate backtest.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("[scope] lower shadow long + mid_high/extreme vol; wick-stop diagnostics; no final profit exit", flush=True)
    ...
    labels = build_profit_effect_labels(
        ...
        wick_stop_buffer_pct=float(args.wick_stop_buffer_pct),
        wick_stop_buffer_atr_mult=float(args.wick_stop_buffer_atr_mult),
    )
    ...
    print("[done] lower-shadow vol-regime wick-stop probe complete", flush=True)
    return 0
