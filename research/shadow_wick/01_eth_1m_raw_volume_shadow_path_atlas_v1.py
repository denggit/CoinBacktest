...
SCRIPT_NAME = "eth_1m_raw_volume_shadow_path_atlas_v1"
SCRIPT_VERSION = "1.0.1"
EXPERIMENT_ID = "ETH_MF_1M_RAW_VOLUME_SHADOW_PATH_ATLAS_V1"
EDGE_ID = "RESEARCH_ONLY_ETH_MF_1M_RAW_VOLUME_SHADOW_PATH_ATLAS_V1"
TITLE = "ETH 1m Raw Volume Shadow Path Atlas V1"
DEFAULT_OUT_DIR = "data/reports/research/eth_1m_raw_volume_shadow_path_atlas_v1"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Raw ETH 1m volume-shadow MFE/MAE path atlas for lower-long and upper-short signals.",
...
def _path_stats(part: pd.DataFrame) -> dict[str, float]:
    ...
    for col in ["mfe", "mae", "time_to_mfe_bars", "time_to_mae_bars", "mfe_capture_ratio", "adverse_sweep_count", "bars_beyond_adverse_level", "max_depth_beyond_adverse_pct"]:
        s = pd.to_numeric(part.get(col, np.nan), errors="coerce")
...
def write_reports(
    ...
    *,
    ...
    print("[aggregate] building path-atlas report tables", flush=True)
    ...
    manifest = {
        ...
        "path_row_count": int(len(paths)),
        "thresholds": th.__dict__,
        "causal_policy": "closed 1m event bar; next open entry; path diagnostics after causal entry; no MTF context",
    ...
    reports = {
        ...
        "02_path_summary.csv": build_path_summary(paths, horizons),
        "03_mfe_mae_by_outcome.csv": build_mfe_mae_by_outcome(paths),
        "04_sweep_reclaim_summary.csv": build_sweep_reclaim_summary(paths),
        "05_holdability_labels.csv": build_holdability_labels(paths),
        "06_direction_comparison.csv": build_direction_comparison(paths),
        "07_yearly_path_summary.csv": build_yearly_path_summary(paths),
        "08_session_regime_summary.csv": build_session_regime_summary(paths),
        "09_causal_audit.csv": build_causal_audit(paths),
        "10_upgrade_decision_map.csv": build_upgrade_decision_map(paths),
        "11_event_sample.csv": _sample(events, int(args.event_sample_size)),
        "12_path_sample.csv": _sample(paths, int(args.path_sample_size)),
    }
    for filename, df in reports.items():
        _write_csv(_pct_columns(df), out_dir / filename)

    if not bool(args.skip_full_path):
        slim_cols = [
            "event_id", "event_name", "family", "direction", "side", "strength", "entry_delay_bars",
            "event_bar_time", "signal_time", "entry_time", "event_low", "event_high", "event_open", "event_close",
            "entry_price", "primary_horizon", "primary_gross_return", "primary_net_return", "mfe", "mae",
            "time_to_mfe_bars", "time_to_mae_bars", "mfe_before_mae", "mfe_capture_ratio",
            "adverse_sweep_flag", "adverse_sweep_count", "sweep_bucket", "bars_to_first_adverse_sweep",
            "bars_beyond_adverse_level", "max_consecutive_bars_beyond_adverse", "max_depth_beyond_adverse_pct",
            "favorable_reclaim_flag", "bars_to_favorable_reclaim", "mid_reclaim_flag", "bars_to_mid_reclaim",
            "holdability_label", "session", "vol_regime", "trend_regime", "close_pos", "volume_ratio",
            "lower_wick_share", "upper_wick_share", "lower_wick_atr", "upper_wick_atr", "ret_30", "ret_120",
            "delta_ratio", "taker_buy_ratio", "lookahead_flag", "entry_not_expected_time_flag",
        ]
        for h in horizons:
            slim_cols.extend([f"gross_return_h{int(h)}", f"net_return_h{int(h)}", f"mfe_h{int(h)}", f"mae_h{int(h)}"])
        slim = paths[[c for c in slim_cols if c in paths.columns]].copy()
        _write_csv(_pct_columns(slim), out_dir / "13_full_path_trades.csv")

    prompt = f"""# GPT Review Prompt — {TITLE}

This is a raw path atlas, not a strategy-promotion report.

Review rules:
1. Do not start by optimizing conditions. First inspect MFE/MAE, adverse sweep, favorable reclaim, and holdability labels.
2. Compare lower-shadow LONG versus upper-shadow SHORT. Do not assume short has no edge until MFE/MAE and sweep/reclaim paths are reviewed.
3. `mean_net` remains important as per-trade expectancy, but it is not enough. Also inspect total_return, win_rate, PF, drawdown proxy, MFE/MAE, sweep buckets, reclaim share, yearly stability, and top-winner dependency.
4. Use `13_full_path_trades.csv` to inspect actual paths before proposing upgrades.
5. If raw shadow has MFE but poor net return, recommend exit/confirmation diagnostics rather than immediate rejection.
6. Any future candidate must preserve closed-bar signal and next-open execution timing.

Key files:
- 02_path_summary.csv: horizon-level MFE/MAE and net path summary.
- 03_mfe_mae_by_outcome.csv: winners versus losers path differences.
- 04_sweep_reclaim_summary.csv: adverse sweep and reclaim quality.
- 05_holdability_labels.csv: diagnostic path labels.
- 06_direction_comparison.csv: lower-long versus upper-short comparison.
- 10_upgrade_decision_map.csv: heuristic map for next research direction.
- 13_full_path_trades.csv: full slim path trades, unless --skip-full-path was used.
"""
    (out_dir / "GPT_REVIEW_PROMPT.md").write_text(prompt, encoding="utf-8")
    readme = f"""# {TITLE}

This report maps raw 1m volume-shadow paths for ETH, before narrowing into strict panic/downtrend candidates.

It intentionally answers path questions first:
- How much MFE/MAE exists after raw lower/upper volume shadows?
- Do adverse sweeps destroy trade quality or can they reclaim?
- Are upper-shadow shorts truly weak, or do they have MFE but poor exit/confirmation?
- Which trades look worth holding, quick to rebound, failed after sweep, or dead?

This is research-only and not a tradable edge.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("[scope] raw lower/upper volume-shadow path atlas; no condition-grid search", flush=True)
    bars = load_bars(args)
    ...
    bad = paths.loc[paths.get("lookahead_flag", pd.Series(False, index=paths.index)).astype(bool)] if not paths.empty else pd.DataFrame()
    if not bad.empty:
        raise RuntimeError(f"Lookahead flags detected in raw path atlas: {len(bad)}")
    print("[causal] no lookahead flags in raw path atlas", flush=True)
    write_reports(
        Path(args.out_dir),
        args=args,
        th=th,
    ...
    print("[done] raw volume-shadow path atlas completed; inspect gpt_review_pack.zip", flush=True)
    return 0
