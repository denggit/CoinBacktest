...
def test_timeframe_sized_step_jumps_one_30m_bar_without_lifecycle_scan(tmp_path: Path) -> None:
    seed_soxl(tmp_path)
    app = ReplayApplication(ReplayDataService(tmp_path), ReplayStore(tmp_path / "replay.sqlite3"))
    episode = app.create_episode({"symbol": "SOXL-USDT-SWAP", "mode": "specific", "start_date": "2026-06-02"})
    ...
    assert any(bar["time"] == "2026-06-02 07:30:00" and not bar["is_partial"] for bar in result["updates"]["30m"])
