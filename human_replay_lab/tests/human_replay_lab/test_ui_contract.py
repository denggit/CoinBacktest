...
def test_tradingview_style_replay_ui_contract() -> None:
    ...
    for tool in (
        "trend", "horizontal-ray", "ray", "rectangle", "vertical", "ruler",
        "long-position", "short-position", "callout",
    ):
    ...
    assert 'id="drawingColor"' in html
    assert 'id="drawingLockBtn"' in html
    assert 'id="autoScaleBtn"' in html
    assert 'id="colorPalette"' in html
    assert 'step="any" value="10000"' in html
    ...
    assert "history?timeframe=" in js
    assert "visibleOffset = Math.max(-maxBlank" in js
    assert "manualPriceRange" in js
    assert "syncEditedPosition" in js
    assert "update-order" in js
    assert "PRESET_COLORS" in js
    ...
    assert "step(timeframeMinutes())" in js
    ...
    assert '<option value="1000" selected>1 秒 / 根</option>' in html
    assert '<option value="16">极速</option>' in html
