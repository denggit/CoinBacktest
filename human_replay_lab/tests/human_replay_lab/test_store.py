...
def test_limit_modify_and_trade_bracket_events_reconstruct_current_state(tmp_path) -> None:
    store = ReplayStore(tmp_path / "replay.sqlite3")
    episode = store.create_episode("ETH-USDT-SWAP", "2026-01-02 07:30:00")
    store.add_event(episode.id, "LIMIT_ORDER", episode.cursor_time, timeframe="5m", price=100.0,
                    payload={"order_id": "order-1", "side": "LONG", "order_type": "limit", "stop_loss": 95.0, "take_profit": 110.0})
    store.add_event(episode.id, "LIMIT_MODIFY", "2026-01-02 07:31:00", timeframe="5m", price=101.0,
                    payload={"order_id": "order-1", "side": "LONG", "order_type": "limit", "stop_loss": 96.0, "take_profit": 112.0})
    pending = store.active_limit_orders(episode.id)
    assert len(pending) == 1
    assert pending[0]["price"] == 101.0
    assert pending[0]["payload"]["stop_loss"] == 96.0

    store.add_event(episode.id, "TRADE_OPEN", "2026-01-02 07:32:00", timeframe="5m", price=101.0,
                    payload={"trade_id": "trade-1", "side": "LONG", "entry_price": 101.0, "initial_stop_loss": 96.0, "initial_take_profit": 112.0})
    store.add_event(episode.id, "SL", "2026-01-02 07:33:00", timeframe="5m", price=97.0, payload={"trade_id": "trade-1"})
    store.add_event(episode.id, "TP", "2026-01-02 07:33:00", timeframe="5m", price=115.0, payload={"trade_id": "trade-1"})
    active = store.active_trades(episode.id)
    assert active[0]["payload"]["current_stop_loss"] == 97.0
    assert active[0]["payload"]["current_take_profit"] == 115.0
